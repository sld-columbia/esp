...
begin  -- architecture rtl
  ...
  -- rd/wr registers
  reg_rd_wr : process (apbi, status_reg, cmd_reg) is
  begin
          ...
          if (reg.word_cnt = BITS_PER_LINE / DMA_NOC_WIDTH - 1 or dma_preamble = PREAMBLE_TAIL) then
            if (llc_dma_req_in_ready = '1') then
              ...
              if (reg.dma32 = '1') then
                ...
                (DMA_NOC_WIDTH * reg.word_cnt) + (32 * reg.dma32_cnt)) := dma_rcv_data_out(31 downto 0);

                reg.dma32_cnt := (reg.dma32_cnt + 1) mod dma_words;
              else
                reg.line((DMA_NOC_WIDTH * reg.word_cnt) + DMA_NOC_WIDTH - 1 downto
                DMA_NOC_WIDTH * reg.word_cnt) := dma_rcv_data_out(DMA_NOC_WIDTH - 1 downto 0);
              end if;
  ...
  begin  -- process fsm_rsp_out
    ...
    case reg.state is
      ...
      when send_header =>
        ...
        if (llc_rsp_out_valid = '1') then
          ...
          if (coherence_rsp_snd_full = '0') then
            ...
            coherence_rsp_snd_data_in <= create_header(COH_NOC_FLIT_SIZE, local_y, local_x, dest_y, dest_x,
                                                       mix_msg, reserved);
      ...
      when send_header_stall =>
        ...
        if (coherence_rsp_snd_full = '0') then
          coherence_rsp_snd_wrreq   <= '1';
          coherence_rsp_snd_data_in <= create_header(COH_NOC_FLIT_SIZE, local_y, local_x, reg.dest_y, reg.dest_x,
      ...
      when send_data =>
        ...
        if (coherence_rsp_snd_full = '0') then
          ...
          if (reg.word_cnt = BITS_PER_LINE / COH_NOC_WIDTH - 1) then
            coherence_rsp_snd_data_in <= PREAMBLE_TAIL & read_coh_noc_word(reg.line, reg.word_cnt);

            reg.state := send_header;
          else
            coherence_rsp_snd_data_in <= PREAMBLE_BODY & read_coh_noc_word(reg.line, reg.word_cnt);
