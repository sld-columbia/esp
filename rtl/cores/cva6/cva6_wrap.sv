...
module cva6_wrap #(
    ...
    // Base addresses for CVA6
    function automatic config_pkg::cva6_user_cfg_t override_cfg(
    config_pkg::cva6_user_cfg_t base
    );
        override_cfg = base;
        override_cfg.NrNonIdempotentRules = unsigned'(1);
        override_cfg.NonIdempotentAddrBase = 1024'({64'b0});
        override_cfg.NonIdempotentLength   = 1024'({DRAMBase});
        override_cfg.NrExecuteRegionRules  = unsigned'(2);
        override_cfg.ExecuteRegionAddrBase = 1024'({DRAMBase, ROMBase});
        override_cfg.ExecuteRegionLength   = 1024'({DRAMCachedLength, ROMLength});
        override_cfg.NrCachedRegionRules   = unsigned'(1);
        override_cfg.CachedRegionAddrBase  = 1024'({DRAMBase});
        override_cfg.CachedRegionLength    = 1024'({DRAMCachedLength});
        override_cfg.DmBaseAddress         = 64'd0;
    endfunction

    localparam config_pkg::cva6_user_cfg_t cva6_cfg =
        override_cfg(cva6_config_pkg::cva6_cfg);

    // CVA6 configuration: use custom fg configuration package overrides
    localparam config_pkg::cva6_cfg_t Cva6Cfg = build_config_pkg::build_config(cva6_cfg);
