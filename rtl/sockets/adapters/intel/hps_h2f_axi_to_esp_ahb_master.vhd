...
--
-- Bridge behavior:
--   * AW and W are buffered independently so AXI write address/data beats may
--     arrive in either order or in the same cycle.
--   * HTRANS is held stable while an AHB address phase is stalled and returns
--     to IDLE after the single accepted transfer. This keeps ESP's AHB-to-NoC
--     proxy from sampling a phantom follow-up transfer during the data phase.
--
-- Notes / limitations:
--   * single outstanding transaction (no AXI pipelining beyond 1-deep AW/W buffering)
--   * AWLEN/ARLEN ignored; the HPS loader issues single-beat accesses
