...
void system_t::config_proc()
{
...

    {
        ...
        conf_info_t config(p_rows, q_cols, m_rows, data_bv, maxiter, p2p_in, p2p_out, p2p_iter,
                           store_state);
        wait();
...
void system_t::dump_memory()
{
    ...
    for (int i = 0; i < out_size / DMA_WORD_PER_BEAT; i++)
        for (int j = 0; j < DMA_WORD_PER_BEAT; j++) {
            ...
            // ESP_REPORT_INFO("value: %d, i: %d, X size is %d, Y size is %d", mem[offset +
            // i].range((j + 1) * DATA_WIDTH - 1, j * DATA_WIDTH).to_uint(), i, inX_size, inY_size);
        }
#endif
...
int system_t::validate()
{
    ...
    ESP_REPORT_INFO("sum of P: Actual = %f, Expected = %f, Error = %f ", sumP, 1.0,
                    100 * (1.0 - sumP) / 1.0);
    ESP_REPORT_INFO("Error of P: %f , not normalized: %f", sqrt(sumE) / (229 * 177), sumE);
    ESP_REPORT_INFO("CP_sum: Actual = %f, Expected = %f, Error = %f", CP_sum, 0.868033,
                    100 * (0.868033 - CP_sum) / 0.868033);

    float P_error     = 100 * (sqrt(sumE)) / (229 * 177);
    float sum_P_error = 100 * (1.0 - sumP) / 1.0;
    float CP_error    = 100 * (0.868033 - CP_sum) / 0.868033;

    if (P_error > 3 || CP_error > 3) errors++;
