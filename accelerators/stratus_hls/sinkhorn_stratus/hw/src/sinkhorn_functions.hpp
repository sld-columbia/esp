...
void sinkhorn::compute_P(uint32_t p, uint32_t q, uint32_t maxiter /*, FPDATA_WORD gamma*/)
{
    ...
    while (!converge) {
        ...
        counter = counter + 1;
        if (a_error <= limit || b_error <= limit || counter >= maxiter) converge = true;

#ifndef STRATUS_HLS
        float error_val;
        fp2native(a_error, error_val);
        ESP_REPORT_INFO("End iteration %d with %.20f a_error, min %f, max %f", counter, error_val,
                        min, max);
        fp2native(b_error, error_val);
        ESP_REPORT_INFO("End iteration %d with %.20f b_error, min %f, max %f", counter, error_val,
                        min, max);
#endif
    ...
    for (uint32_t i = 0; i < p; i++) {
        ...
        sum_a += a_fp1;
        if (i != p - 1) fp2native(a_fp2, a_val);
        // ESP_REPORT_INFO("a[%u] = %f ", i, a_val);
        sum_a += a_fp2;
    }
    for (uint32_t i = 0; i < q; i++) {
        ...
        sum_b += b_fp1;
        if (i != q - 1) fp2native(b_fp2, b_val);
        // ESP_REPORT_INFO("b[%u] = %f ", i, b_val);
        sum_b += b_fp2;
    }
    ...
    else // Do b = q / (K.T @ a)
    {
        ...
        for (uint8_t i = 0; i < Q_MAX; i++) {
            ...
            if (i >= q) {}
                ...
                //                     max_error = err_val;
                //                 //ESP_REPORT_INFO("inv b value: %f , taylor value: %f, error:
                //                 %f", 1/float_val, taylor_val, max_error);
                // #endif
