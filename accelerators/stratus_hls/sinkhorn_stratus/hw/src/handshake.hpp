...
class handshake_req_t {
  public:
    ...
    // Constructor
    handshake_req_t(sc_module_name name) : __req(std::string(name).append("_get").c_str()) {}
...
class handshake_ack_t {
  ...
  public:
    ...
    // Constructor
    handshake_ack_t(sc_module_name name) : __ack(std::string(name).append("_put").c_str()) {}
