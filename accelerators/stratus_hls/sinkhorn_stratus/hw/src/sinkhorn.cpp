            ...
            if ((store_state == 2 && p2p_iter == 1) || (store_state == 3 && b == 0)) {
                ...
                for (int rem = length; rem > 0; rem -= PLM_TOTAL_IN_WORD) {
                    ...
                    for (uint16_t i = 0; i < len; i++) {
                        ...
                        // Write to PLM
                        if (i < length_X) inputX[i] = dataBv.to_uint();
                        else
                        ...
                        for (uint16_t k = 0; k < DMA_WORD_PER_BEAT; k++) {
                            ...
                            if (i + k < length_X) {
                                ...
                                inputY[i + k - length_X] =
                                    dataBv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH)
                                        .to_uint();
                                // inputY[i + k - length_X] = dataBv.range(((k+1) << LOG_DATA_WIDTH)
                                // - 1, k << LOG_DATA_WIDTH).to_uint();
...
void sinkhorn::store_output()
{
...

    {
        ...
        p2p_iter = config.p2p_iter > 0 ? config.p2p_iter : 1;
        store_state =
            config.store_state; // 0 - store, 1 - don't store on the last iteration if p2p_out
    }
    ...
    // Store
    {
        ...
        for (uint16_t b = 0; b < p2p_iter; b++) {
            ...
            if ((store_state == 1 || store_state == 3) && b == p2p_iter - 1) {
                ...
                offset          = store_offset;
                if (p2p_out == 1) length = length_P;
#else
                uint32_t length = round_up(length_P + 1, DMA_WORD_PER_BEAT);
                offset = store_offset;
                if (p2p_out == 1) length = round_up(length_P, DMA_WORD_PER_BEAT);
#endif
                // Chunking
                for (int rem = length; rem > 0; rem -= PLM_OUT_WORD) {
                    ...
                    //     for (uint32_t i = plm_addr; i < /*PLM_OUTPUT_SIZE*/(plm_addr + p_rows *
                    //     q_cols); i++) {
                    //         wait();
                    ...
                    if (!(p2p_out == 1 && h == 0)) this->dma_write_chnl.put(dataBv);
                    pingpong = !pingpong;
    ...
    // Compute P
    {
        ...
        for (uint16_t b = 0; b < p2p_iter; b++) {
            ...
            if ((store_state == 1 || store_state == 3) && b == p2p_iter - 1) {
                    ...
                    if (i >= p_rows) {}
                        ...
                        for (j = 0; j < Q_MAX; j++) {
                            ...
                            if (j >= q_cols) {}
                                ...
                                // wait();
                                if (pingpong == false) Ping[j] = P_word;
                                else
                                    Pong[j] = P_word;
                ...
                if (pingpong == false) Ping[0] = CP_word;
                else
                    Pong[0] = CP_word;
                pingpong = !pingpong;
                this->computeP_store_handshake();
