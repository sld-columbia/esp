  ...
  // Load
  {
    ...
    for (uint16_t b = 0; b < p2p_iter; b++) {
      ...
      if ((store_state == 2 && p2p_iter == 1) || (store_state == 3 && b == 0)) {
        ...
        if (p2p_in == 1)
          length = round_up(length_X + length_Y + 1, DMA_WORD_PER_BEAT);
#endif
        wait();
        offset = 0;
        // Chunking
        for (int rem = length; rem > 0; rem -= PLM_TOTAL_IN_WORD) {
          ...
          dma_info_t dma_info(offset * DMA_BEAT_PER_WORD,
                              len * DMA_BEAT_PER_WORD, DMA_SIZE, 0);
#else
          dma_info_t dma_info(offset / DMA_WORD_PER_BEAT,
                              len / DMA_WORD_PER_BEAT, DMA_SIZE, 0);
          // dma_info_t dma_info(offset >> LOG_DMA_WORD_PER_BEAT, len >>
          // LOG_DMA_WORD_PER_BEAT, DMA_SIZE, 0);
#endif
          offset += len;
...
#ifndef STRATUS_HLS
          ESP_REPORT_INFO(
              "DMA INFO LOAD: index = %d, length = %d, size = %d \n",
              ...
              if (i + k < length_X) {
                inputX[i + k] =
                    dataBv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH)
                        .to_uint();
                // inputX[i + k] = dataBv.range(((k+1) << LOG_DATA_WIDTH) - 1, k
                // << LOG_DATA_WIDTH).to_uint();
                ...
                // ESP_REPORT_INFO("DMA read inputX = %.20f, offset is %d",
                // data_float, i+k);
#endif
                ...
                inputY[i + k - length_X] =
                    dataBv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH)
                        .to_uint();
                // inputY[i + k - length_X] = dataBv.range(((k+1) <<
                // LOG_DATA_WIDTH) - 1, k << LOG_DATA_WIDTH).to_uint();
                ...
                float data_float;
                fp2native(data_fp, data_float);
                // ESP_REPORT_INFO("DMA read inputY = %.20f, offset is %d",
                // data_float, i+k);
#endif
              }
            }
...
#ifndef STRATUS_HLS
        ESP_REPORT_INFO("DMA read inputX and inputY complete, offset is %d",
                        offset);
#endif
...
void sinkhorn::store_output() {
...

  {
    ...
    p2p_out = config.p2p_out;
    p2p_iter = config.p2p_iter > 0 ? config.p2p_iter : 1;
    store_state = config.store_state; // 0 - store, 1 - don't store on the last
                                      // iteration if p2p_out
  }
  ...
  // Store
  {
    ...
    uint32_t store_offset =
        round_up(length_X + length_Y, DMA_WORD_PER_BEAT) * 1;
#endif
    ...
    for (uint16_t b = 0; b < p2p_iter; b++) {
      ...
      if ((store_state == 1 || store_state == 3) && b == p2p_iter - 1) {
        ...
        for (int rem = length; rem > 0; rem -= PLM_OUT_WORD) {
...
#ifndef STRATUS_HLS
          ESP_REPORT_INFO(
              "DMA INFO STORE: index = %d, length = %d, size = %d \n",
              dma_info.index, dma_info.length, DMA_SIZE.to_uint());
#endif
          offset += len;
          ...
          //     for (uint32_t i = plm_addr; i < /*PLM_OUTPUT_SIZE*/(plm_addr +
          //     p_rows * q_cols); i++) {
          //         wait();
          ...
          for (uint16_t i = 0; i < length_P; i += q_cols) {
            ...
            for (uint16_t k = 0; k < q_cols; k++) {
              ...
              if (pingpong == false) {
                dataBv.range((h + 1) * DATA_WIDTH - 1, h * DATA_WIDTH) =
                    Ping[k];
                // dataBv.range(((h+1) << LOG_DATA_WIDTH) - 1, h <<
                // LOG_DATA_WIDTH) = Ping[k];
              }
          ...
          if (pingpong == false) {
            ...
            // dataBv.range(((h+1) << LOG_DATA_WIDTH) - 1, h << LOG_DATA_WIDTH)
            // = Ping[0];
          }
          // this->dma_write_chnl.put(dataBv_ping);
          else {
