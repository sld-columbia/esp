...
template <typename T, size_t FP_WL, size_t BV_L> void bv2fp(sc_dt::sc_bv<BV_L> data_in, T &data_out)
{
    // Copy data
...
template <typename T, size_t FP_WL, size_t BV_L> void fp2bv(T data_in, sc_dt::sc_bv<BV_L> &data_out)
{
    // Copy data
    for (unsigned i = 0; i < FP_WL; i++) {
...
inline void fp2native(const FPDATA &in, float &out) { out = in.to_double(); }
