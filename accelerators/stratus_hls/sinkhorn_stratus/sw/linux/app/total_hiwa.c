...
static int validate_final(token_t *out_svd, token_t *out_hiwa, float *final_svd, float final_hiwa, int32_t p_i,
                          int32_t q_i)
{
    int      i, j;
    unsigned errors = 0;
...
static void init_parameters(int32_t p_i, int32_t q_i)
{
    ...
    if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
        ...
        in_words_adj_svd =
            round_up(p_i * q_i + m_rows * p_i + m_rows * q_i + m_rows * m_rows + 1, DMA_WORD_PER_BEAT(sizeof(float)));
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++) {
            printf("\nResults from HiWA.%d:\n", j);
            init_parameters(p_tot[i], q_tot[j]);
            errors[i][j] = validate_final(&buf_svd[i * 4 + j][out_offset_svd], &buf[i * 4 + j][out_offset],
