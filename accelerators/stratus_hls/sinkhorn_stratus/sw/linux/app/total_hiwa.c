...
static int validate_final(token_t *out_svd, token_t *out_hiwa, float *final_svd,
                          float final_hiwa, int32_t p_i, int32_t q_i) {
  int i, j;
  unsigned errors = 0;
  float MAE, MAE_sum = 0;
...
static void init_buffer_svd(token_t *in, int32_t p_i, int32_t q_i, int32_t n,
                            int32_t l) {
  ...
  for (i = 0; i < 1; i++) {
    ...
    for (x = 0; x < m_rows * p_i; x++) // X
    {
      in[i * in_words_adj_svd + j + x] =
          (token_t)float_to_fixed32(X_tot[n][x], 11);
    }

    for (y = 0; y < m_rows * q_i; y++) // Y
    {
      in[i * in_words_adj_svd + j + x + y] =
          (token_t)float_to_fixed32(Y_tot[l][y], 11);
    }
...
static void init_parameters(int32_t p_i, int32_t q_i) {
  if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
    ...
    in_words_adj =
        round_up((p_i + q_i) * m_rows, DMA_WORD_PER_BEAT(sizeof(token_t)));
  ...
  if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
    in_words_adj_svd =
        p_i * q_i + m_rows * p_i + m_rows * q_i + m_rows * m_rows + 1;
    out_words_adj_svd = m_rows * m_rows + m_rows * p_i + m_rows * q_i;
  } else {
    in_words_adj_svd =
        round_up(p_i * q_i + m_rows * p_i + m_rows * q_i + m_rows * m_rows + 1,
...
int main(int argc, char **argv) {
  ...
  printf("\nResults from HiWA.0:\n");
  errors0 = validate_final(&buf_svd0[out_offset_svd], &buf0[out_offset],
                           R_tot[0], CP_tot[0], p_tot[0], q_tot[0]);
  printf("\nResults from HiWA.1:\n");
  errors1 = validate_final(&buf_svd1[out_offset_svd], &buf1[out_offset],
                           R_tot[0], CP_tot[0], p_tot[0], q_tot[0]);
  printf("\nResults from HiWA.2:\n");
  errors2 = validate_final(&buf_svd2[out_offset_svd], &buf2[out_offset],
                           R_tot[0], CP_tot[0], p_tot[0], q_tot[0]);
  printf("\nResults from HiWA.3:\n");
  errors3 = validate_final(&buf_svd3[out_offset_svd], &buf3[out_offset],
                           R_tot[0], CP_tot[0], p_tot[0], q_tot[0]);
  ...
  for (int i = 0; i < 4; i++)
    for (int j = 0; j < 4; j++) {
      printf("\nResults from HiWA.%d:\n", j);
      init_parameters(p_tot[i], q_tot[j]);
      errors[i][j] = validate_final(
          &buf_svd[i * 4 + j][out_offset_svd], &buf[i * 4 + j][out_offset],
