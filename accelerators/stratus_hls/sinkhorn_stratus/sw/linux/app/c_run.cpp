...
float c_run_hiwa(unsigned p, unsigned q, float *X, float *Y, float gamma,
                 unsigned maxiter, float outputX[], float outputY[], float C[],
...
void reshape_to_C_mat_with_RK(float *y, float *x, float *Y, float *X,
                              unsigned q, unsigned p, float *C, float *K,
                              float gamma) {
  // Takes two arrays with different dimensions
  // Array x has n elements, array y has m elements
  // Arrange x times m rows in X and y times n columns in Y
...
float sinkhorn_kernel_clean(unsigned iter, unsigned p, unsigned q, float *a,
                            float *b, float *b_prev, float *K, float *C,
                            float *P) {
...
void sink_kernel_operation(float *a, unsigned p, unsigned q, float *K, float *b,
                           bool transpose) {
...
void mult_diag_mat_diag(float *diag_vec_1, float *K, float *diag_vec_2,
...
float mult_sum_mats_elementwise(float *C, float *P, unsigned rows,
                                unsigned cols) {
...
void *c_hiwa_tot(void *pq) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    // Sinkhorn
    sum =
        total_sinkhorn_clean(p, q, X_sink, Y_sink, P_sink, gamma, iter,
                             outputX3, outputY3, C_sink, K_sink, a, b, b_prev);
