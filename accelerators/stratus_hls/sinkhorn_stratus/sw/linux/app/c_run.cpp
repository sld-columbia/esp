...
float c_run_hiwa(unsigned p, unsigned q, float *X, float *Y, float gamma, unsigned maxiter,
...
void reshape_to_C_mat_with_R2(float *y, float *x, float *Y, float *X, unsigned q, unsigned p,
                              float *C)
{
...
void reshape_to_C_mat_with_RK(float *y, float *x, float *Y, float *X, unsigned q, unsigned p,
                              float *C, float *K, float gamma)
{
    // Takes two arrays with different dimensions
    // Array x has n elements, array y has m elements
    // Arrange x times m rows in X and y times n columns in Y
...
float sinkhorn_kernel_clean(unsigned iter, unsigned p, unsigned q, float *a, float *b,
                            float *b_prev, float *K, float *C, float *P)
{
    ...
    else // Transpose multiplication
    {
        ...
        for (unsigned i = 0; i < p; i++) {
            ...
            for (unsigned k = 0; k < q; k++) {
                ...
                // a[i] += K[k * q + i] * b[k];
                a_alt[k] += K[i * q + k] * b[i];
                if (i == p - 1) { a[k] = 1.0 / ((float)q * a_alt[k]); }
            }
            // a[i] = 1.0/((float)q * a[i]);
        }
    }
...
void mult_diag_mat_diag(float *diag_vec_1, float *K, float *diag_vec_2, float *R, unsigned m,
                        unsigned n)
{
...
float total_sinkhorn_clean(unsigned p, unsigned q, float *X, float *Y, float *P, float gamma,
                           unsigned maxiter, float outputX[], float outputY[], float C[], float K[],
...
void *c_hiwa_tot(void *pq)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        // Sinkhorn
        sum = total_sinkhorn_clean(p, q, X_sink, Y_sink, P_sink, gamma, iter, outputX3, outputY3,
                                   C_sink, K_sink, a, b, b_prev);
