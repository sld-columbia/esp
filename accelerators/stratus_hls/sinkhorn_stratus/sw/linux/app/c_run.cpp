...
float c_run_hiwa(unsigned p, unsigned q, float *X, float *Y, float gamma, unsigned maxiter, float outputX[],
...
void reshape_to_C_mat_with_RK(float *y, float *x, float *Y, float *X, unsigned q, unsigned p, float *C, float *K,
                              float gamma)
{
    // Takes two arrays with different dimensions
    // Array x has n elements, array y has m elements
...
float sinkhorn_kernel_clean(unsigned iter, unsigned p, unsigned q, float *a, float *b, float *b_prev, float *K,
                            float *C, float *P)
{
...
float total_sinkhorn_clean(unsigned p, unsigned q, float *X, float *Y, float *P, float gamma, unsigned maxiter,
                           float outputX[], float outputY[], float C[], float K[], float a[], float b[], float b_prev[])
{
    float sum = 0;
...
void *c_hiwa_tot(void *pq)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        // Sinkhorn
        sum = total_sinkhorn_clean(p, q, X_sink, Y_sink, P_sink, gamma, iter, outputX3, outputY3, C_sink, K_sink, a, b,
                                   b_prev);
