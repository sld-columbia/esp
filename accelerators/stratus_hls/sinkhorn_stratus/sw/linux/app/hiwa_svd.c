...
static int validate_final(token_t *out_svd, token_t *out_hiwa, float *final_svd,
                          float *final_hiwa) {
  int i, j;
  unsigned errors = 0;
...
/* User-defined code */
static void init_buffer(token_t *in, token_t *gold, token_t *inX,
                        token_t *inY) {
  int i;
  int j;
  int k;
...
static void init_buffer_svd(token_t *in, token_t *gold) {
  ...
  for (i = 0; i < 1; i++) {
    ...
    {
      in[i * in_words_adj_svd + j + x] =
          (token_t)float_to_fixed32(svd_inputX[x], 11);
    }

    for (y = 0; y < m_rows * q_cols; y++) // Y
    {
      in[i * in_words_adj_svd + j + x + y] =
          (token_t)float_to_fixed32(inputY[y], 11);
    }

    for (t = 0; t < m_rows * m_rows; t++) // T
    {
      in[i * in_words_adj_svd + j + x + y + t] =
          (token_t)float_to_fixed32(inputT[t], 11);
    }

    in[i * in_words_adj_svd + j + x + y + t] =
        (token_t)float_to_fixed32(inputP, 11);
  }
  ...
  for (i = 0; i < 1; i++) {
    for (j = 0; j < m_rows * m_rows; j++)
      gold[i * out_words_adj_svd + j] =
          (token_t)float_to_fixed32(gold_out[j], 11);

    for (k = 0; k < m_rows * p_rows; k++)
      gold[i * out_words_adj_svd + j + k] =
          (token_t)float_to_fixed32(inputX[i * out_words_adj_svd + k], 11);

    for (h = 0; h < m_rows * q_cols; h++)
      gold[i * out_words_adj_svd + j + k + h] =
          (token_t)float_to_fixed32(inputYT[i * out_words_adj_svd + h], 11);
  }
...
static void init_parameters() {
  if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
    ...
    inX_words_adj =
        round_up(p_rows * m_rows, DMA_WORD_PER_BEAT(sizeof(token_t)));
    inY_words_adj =
        round_up(q_cols * m_rows, DMA_WORD_PER_BEAT(sizeof(token_t)));
    in_words_adj = round_up((p_rows + q_cols) * m_rows,
                            DMA_WORD_PER_BEAT(sizeof(token_t)));
    out_words_adj =
        round_up(p_rows * q_cols + 1, DMA_WORD_PER_BEAT(sizeof(token_t)));
  }
  ...
  // SVD parameters
  if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
    in_words_adj_svd = p_rows * q_cols + m_rows * p_rows + m_rows * q_cols +
                       m_rows * m_rows + 1;
    out_words_adj_svd = m_rows * m_rows + m_rows * p_rows + m_rows * q_cols;
  } else {
    in_words_adj_svd = round_up(p_rows * q_cols + m_rows * p_rows +
                                    m_rows * q_cols + m_rows * m_rows + 1,
                                DMA_WORD_PER_BEAT(sizeof(float)));
    out_words_adj_svd =
        round_up(m_rows * m_rows + m_rows * p_rows + m_rows * q_cols,
                 DMA_WORD_PER_BEAT(sizeof(float)));
  }
...
void inline reshape_to_C_mat_with_R(float *y, float *x, float *Y, float *X,
...
void inline exp_gamma_mat(float *C, float gamma, unsigned rows, unsigned cols,
                          float *K) {
  // Element-wise operation on C matrix. Divide each element by gamma and
  // exp(-result) Save the final result in K
...
void inline sink_kernel_operation(float *a, unsigned p, unsigned q, float *K,
                                  float *b, bool transpose) {
...
void inline mult_diag_mat_diag(float *diag_vec_1, float *K, float *diag_vec_2,
                               float *R, unsigned m, unsigned n) {
...
float inline mult_sum_mats_elementwise(float *C, float *P, unsigned rows,
                                       unsigned cols) {
...
float inline sinkhorn_kernel(unsigned iter, unsigned p, unsigned q, float *a,
                             float *b, float *K, float *C, float *tmp,
                             float *P) {
...
float inline sinkhorn(float p, float q, float *X, float *Y, float *P,
                      float gamma, unsigned maxiter, float outputX[],
                      float outputY[], float C[], float K[], float a[],
                      float b[], float tmp[]) {
  float sum = 0;
  unsigned m = 3;
  unsigned nX = p;
...
void c_run() {
  ...
  sw_ns = ts_subtract(&startn, &endn);
  printf("  > Optimized software test time: %llu ns , CP_sum = %f, Expected = "
         "%f\n",
...
int main(int argc, char **argv) {
  ...
  token_t *buf_inY =
      &buf_svd[out_offset_svd + m_rows * m_rows + m_rows * p_rows];
  ...
  ((struct sinkhorn_access *)cfg_000[1].esp_desc)->gamma =
  ...
  for (int i = 0; i < 50; i++) {
    ...
    // errors = validate_svd_2(&buf_svd[out_offset_svd], gold_out_2);
  ...
  //	errors = validate_svd_2(&buf_svd[out_offset_svd], gold_out_2);
  errors = validate_final(&buf_svd[out_offset_svd], &buf[out_offset], finalR,
                          finalQ);
  ...
  // One-side p2p test - SVD to Sinkhorn
  init_buffer_svd(buf_svd, gold_svd);
  ((struct svd_access *)cfg_p2p[0].esp_desc)->src_offset = size;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->gamma =
      float_to_fixed32(gamma_float, 11);
  cfg_p2p[0].hw_buf = buf;
  ...
  // One-side p2p test - Sinkhorn to SVD

  for (int i = 0; i < (size + size_svd) / sizeof(token_t); i++)
    buf[i] = 0;

  // SVD
  // Initialize the buffer for SVD
  printf("\nRun svd one time just to generate the input for Sinkhorn...\n");
  init_buffer_svd(buf_svd, gold_svd);

  // Run SVD one time to generate the input for Sinkhorn
  esp_run(&cfg_000[0], 1);
  errors = validate_buffer_svd(&buf_svd[out_offset_svd], gold_svd);

  // Set SVD configuration as destination for p2p
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_out =
      0; // send output to memory
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_in =
      1; // recieve input from sinkhorn
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_nsrcs = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_store = 0;
  strcpy(((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_srcs[0],
         cfg_p2p[1].devname);
  ((struct svd_access *)cfg_p2p[0].esp_desc)->dst_offset = size;

  // Sinkhorn
  buf_inX = &buf_svd[out_offset_svd + m_rows * m_rows];
  buf_inY = &buf_svd[out_offset_svd + m_rows * m_rows + m_rows * p_rows];
  init_buffer(buf, gold, buf_inX, buf_inY);
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->gamma =
      float_to_fixed32(gamma_float, 11);
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_out =
      1; // send output to svd
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_in =
      0; // get input from memory
  ((struct svd_access *)cfg_p2p[1].esp_desc)->esp.p2p_nsrcs = 0;
  ((struct svd_access *)cfg_p2p[1].esp_desc)->esp.p2p_store = 1;
  ...
  printf("\n====== %s-%s P2P 1 Iterations ======\n\n", cfg_p2p[0].devname,
         cfg_p2p[1].devname);

  for (int i = 0; i < (size + size_svd) / sizeof(token_t); i++)
    buf[i] = 0;

  // First load data into SVD
  init_buffer_svd(buf_svd, gold_svd);
  ((struct svd_access *)cfg_p2p[0].esp_desc)->src_offset = size;
  cfg_p2p[0].hw_buf = buf;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_out = 0;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_in = 0;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_iter = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_nsrcs = 0;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_store = 0;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->load_state = 1;
  ...
  // Run SVD-Sinkhorn with p2p no load from memory
  // SVD
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_out = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_in = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_iter = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_nsrcs = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_store = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->load_state = 2;
  // Sinkhorn
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_out = 0;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_in = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_iter = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->esp.p2p_nsrcs = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->esp.p2p_store = 0;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->store_state = 1;
  ...
  // Sinkhorn
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_out = 0;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_in = 0;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_iter = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->esp.p2p_nsrcs = 0;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->esp.p2p_store = 0;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->store_state = 2;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->dst_offset = 0;

  printf("\nStore data from Sinkhorn\n");
  esp_run(&cfg_p2p[1], 1);
  ...
  printf("\n====== %s-%s P2P 10 Iterations ======\n\n", cfg_p2p[0].devname,
         cfg_p2p[1].devname);

  // Cleanup memory and svd
  for (int i = 0; i < (size + size_svd) / sizeof(token_t); i++)
    buf[i] = 0;
  ...
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_nsrcs = 0;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_store = 0;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->load_state = 0;
  ...
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_out = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_in = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_iter = 10;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_nsrcs = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_store = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->load_state = 2;
  // Sinkhorn
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_out = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_in = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_iter = 10;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->esp.p2p_nsrcs = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->esp.p2p_store = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->store_state = 1;

  printf("\nRun HiWA = SVD->Sinkhorn for 10 iterations\n");
  esp_run(cfg_p2p, 2);
  ...
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_out = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_in = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->p2p_iter = 50;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_nsrcs = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->esp.p2p_store = 1;
  ((struct svd_access *)cfg_p2p[0].esp_desc)->load_state = 2;
  // Sinkhorn
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_out = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_in = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->p2p_iter = 50;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->esp.p2p_nsrcs = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->esp.p2p_store = 1;
  ((struct sinkhorn_access *)cfg_p2p[1].esp_desc)->store_state = 1;
