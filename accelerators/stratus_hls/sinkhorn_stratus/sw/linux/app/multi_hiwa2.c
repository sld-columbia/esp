...
static int validate_buffer_svd(token_t *out, token_t *gold)
{
    ...
    for (i = 0; i < 1; i++)
        for (j = 0; j < m_rows * m_rows + m_rows * p_rows /*+m_rows*q_cols*/; j++) {
            ...
            if (j < m_rows * m_rows) printf("%d: out = %f , gold = %f \n", j, val_out, val_gold);
...
int main(int argc, char **argv)
{
    ...
    printf("\nResults from HiWA.1:\n");
    errors1 = validate_final(&buf_svd0[out_offset_svd + out_words_adj_svd], &buf1[out_offset],
                             finalR, finalQ);
    /* printf("\nResults from HiWA.2:\n"); */
    /* errors2 = validate_final(&buf_svd0[out_offset_svd+2*out_words_adj_svd], &buf2[out_offset],
     * finalR, finalQ); */
    /* printf("\nResults from HiWA.3:\n"); */
    /* errors3 = validate_final(&buf_svd0[out_offset_svd+3*out_words_adj_svd], &buf3[out_offset],
     * finalR, finalQ); */
