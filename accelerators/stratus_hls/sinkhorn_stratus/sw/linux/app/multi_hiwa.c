...
int main(int argc, char **argv) {
  ...
  ((struct sinkhorn_access *)cfg_multi2[4].esp_desc)->gamma =
      float_to_fixed32(gamma_float, 11);
  ((struct sinkhorn_access *)cfg_multi2[5].esp_desc)->gamma =
      float_to_fixed32(gamma_float, 11);
  ((struct sinkhorn_access *)cfg_multi2[6].esp_desc)->gamma =
      float_to_fixed32(gamma_float, 11);
  ((struct sinkhorn_access *)cfg_multi2[7].esp_desc)->gamma =
      float_to_fixed32(gamma_float, 11);
  ...
  /* unsigned nthreads = 4; */
  /* esp_thread_info_t **cfg_ptrs = malloc(sizeof(esp_thread_info_t*) *
   * nthreads); */
  ...
  errors0 = validate_final(&buf_svd0[out_offset_svd], &buf0[out_offset], finalR,
                           finalQ);
  printf("\nResults from HiWA.1:\n");
  errors1 = validate_final(&buf_svd1[out_offset_svd], &buf1[out_offset], finalR,
                           finalQ);
  printf("\nResults from HiWA.2:\n");
  errors2 = validate_final(&buf_svd2[out_offset_svd], &buf2[out_offset], finalR,
                           finalQ);
  printf("\nResults from HiWA.3:\n");
  errors3 = validate_final(&buf_svd3[out_offset_svd], &buf3[out_offset], finalR,
                           finalQ);
