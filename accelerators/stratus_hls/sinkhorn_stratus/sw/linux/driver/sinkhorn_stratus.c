...
static int sinkhorn_probe(struct platform_device *pdev)
{
    ...
    if (sinkhorn == NULL) return -ENOMEM;
    esp         = &sinkhorn->esp;
...
static int __init sinkhorn_init(void) { return esp_driver_register(&sinkhorn_driver); }

static void __exit sinkhorn_exit(void) { esp_driver_unregister(&sinkhorn_driver); }

module_init(sinkhorn_init) module_exit(sinkhorn_exit)
