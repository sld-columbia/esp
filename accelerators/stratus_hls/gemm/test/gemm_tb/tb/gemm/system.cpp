...
int system_t::validate()
{
    ...
    int relu=cnn->layer_sets[l_n]->relu;
    int wr=cnn->W[l_n-1]->rows;
    float* out=new float[wr-1];
    float* layer_bias=cnn->layer_sets[l_n]->bias.x;

    //run programmer's view for the target layer
    run_pv(l_n,true);

    
    //validate layer
    float* out_gold=cnn->layer_sets[l_n]->node.x;
    ...
    validate_hw_gemm(l_n,out,layer_bias,cnn->layer_sets[l_n]->node.x,wr,size_out,relu);
    ...
    std::cout << "\n \nProgrammer's view + layer "<<l_n<<" accelerator invocation results\
:" << std::endl;
