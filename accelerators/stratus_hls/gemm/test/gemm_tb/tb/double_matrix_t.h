...
__attribute__((unused))
static int load_double_matrix_t1(double_matrix_t **matrix, float *mat, int batch, int rows,int cols,int is_tr)
{
    unsigned int i, size = 1;
...
//    if (!fp) { return -1; }
    ...
    *matrix = (double_matrix_t*)
    ...
    (*matrix)->dims[0]=(size_t)batch;
    (*matrix)->dims[1]=(size_t)rows;
    (*matrix)->dims[2]=(size_t)cols;

    size=((*matrix)->dims[0])*((*matrix)->dims[1])*((*matrix)->dims[2]);
    /* for (i = 0; i < (*matrix)->dim; ++i) */
    /* { */
    /*     fscanf(fp, "%zu\n", &((*matrix)->dims[i])); */
