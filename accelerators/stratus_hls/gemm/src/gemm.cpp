...
void gemm::load_input()
{
    ...
    for (uint24_t a = 0; a < ninputs; a++)
    {
    	...
    	for (uint24_t d1 = 0; d1 < matrix_d1; d1 += loadable_rows)
    	{
			...
			//     {
			// 	HLS_CONSTRAIN_LATENCY(0, 1, "load-matrix1-misaligned");
			// 	if (pingpong_m1) {
    	...
    	for (uint24_t d1 = 0; d1 < matrix_d1; d1 += loadable_rows)
    	{
			...
			accumulator = 0;
				...
				//HLS_CONSTRAIN_LATENCY(0, HLS_ACHIEVABLE, "constrain-mac");
 				HLS_PIPELINE_LOOP(HARD_STALL, 1, "pipeline-mac");
			...
			if (pingpong_out) {
			    ...
			    // std::cout << "COMPUTE OUTPUT0: " << accumulator << " count " << store_count << std::endl;
			} else {
			    output1[store_count] = FP2INT(accumulator);
			    // std::cout << "COMPUTE OUTPUT1: " << accumulator << " count " << store_count << std::endl;
			}
