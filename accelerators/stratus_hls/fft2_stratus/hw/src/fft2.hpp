...
class fft2 : public esp_accelerator_3P<DMA_WIDTH> {
  public:
    ...
    fft2(const sc_module_name &name) :
        esp_accelerator_3P<DMA_WIDTH>(name), cfg("config"), store_to_load("store-to-load")
    {
        // Signal binding
        cfg.bind_with(*this);
    ...
    inline void reset_load_to_store() { this->store_to_load.req.reset_req(); }

    inline void reset_store_to_load() { this->store_to_load.ack.reset_ack(); }
