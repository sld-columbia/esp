...
void fft2::compute_kernel()
{
    ...
    // Loop through the num_ffts successive FFT computations
    {
        ...
        printf("COMPUTE: in_len %u : max_in_ffts %u >> %u = %u\n", in_length, MAX_LOGN_SAMPLES,
               logn_samples, max_in_ffts);
...
FFT2_SINGLE_L3:
                        for (int j = 0; j < md2; j++) {
                            ...
                            // if ((k == 0) && (j == 0)) {
                            //     printf("  L3 : FFT %u = %u : k %u j %u md2 %u : kj %u kjm %u :
                            //     kji %u kjmi %u\n", fftn, ffts_done, k, j, md2, kj, kjm, 2*kj,
                            //     2*kjm);
                            // }
                            CompNum akj, akjm;
                            CompNum bkj, bkjm;
...

                            {
                                ...
                                // A0[kjm].re.to_hex() << " " << A0[kjm].im.to_hex() << endl; if ((k
                                // == 0) && (j == 0)) {
                                //         printf("  L3 : WROTE A0 %u and %u and %u and %u\n", 2*kj,
                                //         2*kj + 1, 2*kjm, 2*kjm + 1);
                                // }
                            }
                        } // for (j = 0 .. md2)
                    }     // for (k = 0 .. num_samples)
                ...
                /*cout << "ACCEL-END : FFT " << ffts_done << " : A0[0] = " << A0[0].to_double() <<
                  endl; cout << "ACCEL-END : FFT " << ffts_done << " : A0[1] = " <<
                  A0[1].to_double() << endl; cout << "ACCEL-END : FFT " << ffts_done << " : A0[2] =
                  " << A0[2].to_double() << endl; cout << "ACCEL-END : FFT " << ffts_done << " :
                  A0[3] = " << A0[3].to_double() << endl;*/
                ffts_done++;
