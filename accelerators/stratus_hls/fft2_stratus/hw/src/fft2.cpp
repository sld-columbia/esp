  ...
  int loads_done = 0;
  {
    ...
    for (int rem = length; rem > 0; rem -= PLM_IN_WORD) {
      ...
      printf("LOAD DMA INFO : rem %u : off = %u , len %u\n", rem,
             (offset / DMA_WORD_PER_BEAT), (len / DMA_WORD_PER_BEAT));
...
void fft2::compute_kernel() {
  ...
  // printf("COMPUTE: logn_samples %u num_samples %u num_ffts %u inverse %u
  ...
  // Loop through the num_ffts successive FFT computations
  {
    ...
    printf("COMPUTE: in_len %u : max_in_ffts %u >> %u = %u\n", in_length,
           MAX_LOGN_SAMPLES, logn_samples, max_in_ffts);
    ...
    for (int in_rem = in_length; in_rem > 0; in_rem -= PLM_IN_WORD) {
      ...
      printf(
          "COMPUTE INFO : in_rem %u : in_len %u :: out_rem %u : out_len %u\n",
      ...
      printf("COMPUTE has %u rem_ffts : proceeding to the next %u FFT "
             "computations...\n",
             rem_ffts, in_ffts);
      for (unsigned fftn = 0; fftn < in_ffts; fftn++) {
        unsigned offset =
            fftn * num_samples; // Offset into Mem for start of this FFT
      ...
      FFT2_SINGLE_L1:
        ...
        FFT2_SINGLE_L2:
          ...
          FFT2_SINGLE_L3:
            for (int j = 0; j < md2; j++) {
              ...
              // if ((k == 0) && (j == 0)) {
              //     printf("  L3 : FFT %u = %u : k %u j %u md2 %u : kj %u kjm
              //     %u : kji %u kjmi %u\n", fftn, ffts_done, k, j, md2, kj,
              //     kjm, 2*kj, 2*kjm);
              // }
              CompNum akj, akjm;
              CompNum bkj, bkjm;
...

              {
                ...
                // cout << "DFT: A0 " << kj << ": " << A0[kj].re.to_hex() << " "
                // << A0[kj].im.to_hex() << endl; cout << "DFT: A0 " << kjm <<
                // ": " << A0[kjm].re.to_hex() << " " << A0[kjm].im.to_hex() <<
                // endl; if ((k == 0) && (j == 0)) {
        ...
        /*cout << "ACCEL-END : FFT " << ffts_done << " : A0[0] = " <<
          A0[0].to_double() << endl; cout << "ACCEL-END : FFT " << ffts_done <<
          " : A0[1] = " << A0[1].to_double() << endl; cout << "ACCEL-END : FFT "
          << ffts_done << " : A0[2] = " << A0[2].to_double() << endl; cout <<
          "ACCEL-END : FFT " << ffts_done << " : A0[3] = " << A0[3].to_double()
          << endl;*/
        ffts_done++;
