...
class conf_info_t
{
  public:
    ...
    conf_info_t(
        /* <<--ctor-args-->> */
        int32_t logn_samples, int32_t num_ffts, int32_t do_inverse, int32_t do_shift, int32_t scale_factor)
    {
        /* <<--ctor-custom-->> */
    ...
    {
        /* <<--eq-->> */
        if (logn_samples != rhs.logn_samples)
            return false;
        if (num_ffts != rhs.num_ffts)
            return false;
        if (do_inverse != rhs.do_inverse)
            return false;
        if (do_shift != rhs.do_shift)
            return false;
        if (scale_factor != rhs.scale_factor)
            return false;
        return true;
    }
