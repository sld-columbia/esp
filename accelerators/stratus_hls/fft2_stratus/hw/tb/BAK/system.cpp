...
int system_t::validate() {
  ...
  for (int nf = 0; nf < num_ffts; nf++) {
    for (int j = 0; j < 2 * num_samples; j++) {
      ...
      if ((fabs(gold[idx] - out[idx]) / fabs(gold[idx])) > ERR_TH) {
        if (errors < 10) {
          ESP_REPORT_INFO(" ERROR %u : fft %u : idx %u : gold %g  out %g",
                          errors, nf, idx, gold[idx], out[idx]);
        }
        errors++;
      }
    }
  ...
  ESP_REPORT_INFO("DMA %u FX %u nFFT %u logn %u : Relative error > %f for %d "
                  "output values out of %d\n",
                  DMA_WIDTH, FX_WIDTH, num_ffts, logn_samples, ERR_TH, errors,
