...
static void init_buffer(token_t *in, float *gold) {
  ...
  fft2_comp(gold, num_ffts, (1 << logn_samples), logn_samples, do_inverse,
            do_shift);
}
...
static void init_parameters() {
  ...
  if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
    ...
    in_words_adj = round_up(2 * num_ffts * num_samples,
                            DMA_WORD_PER_BEAT(sizeof(token_t)));
    out_words_adj = round_up(2 * num_ffts * num_samples,
                             DMA_WORD_PER_BEAT(sizeof(token_t)));
  }
  in_len = in_words_adj;
...
int main(int argc, char **argv) {
  ...
  if ((float)(errors / (float)(2.0 * (float)num_ffts * (float)num_samples)) >
      ERROR_COUNT_TH)
