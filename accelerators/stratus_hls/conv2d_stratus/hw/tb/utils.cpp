...
void init_tensor(float *tensor, const int size, bool random)
{
    ...
    for (int i = 0; i < size; ++i) {
        if (random) value = rand() / float(RAND_MAX);
        else
            ...
            for (int c = 0; c < channels; c++) {
                ...
                for (int h = 0; h < height; h++) {
                    for (int w = 0; w < width; w++) {
                        if (fpdata) ss << FPDATA(image[c * +h * width + w]) << "  ";
                        else
...
bool check_error_threshold_for_neuron(float hw_data, float sw_data, float &error)
{
    ...
    if (sw_data != 0) error = fabs((float(sw_data) - float(hw_fdata)) / float(sw_data));
    else
...
int _validate(float *hw_data_array, float *sw_data_array, int batch_size, int filters, int output_h,
              int output_w)
{
        ...
        for (int f = 0; f < filters; f++) {
            for (int h = 0; h < output_h; h++) {
                for (int w = 0; w < output_w; w++) {
                    int idx_print = b * filters * output_h * output_w + f * output_h * output_w +
                        h * output_w + w;
