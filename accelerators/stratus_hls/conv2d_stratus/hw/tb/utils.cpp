...
void print_image(const char *name, float *image, const int batch_size,
...
void print_weights(const char *name, float *weights, const int filters,
                   const int channels, const int height, const int width,
                   const bool fpdata) {
        ...
        for (int h = 0; h < height; h++) {
          for (int w = 0; w < width; w++) {
            if (fpdata)
              ...
              ss << (float)weights[f * channels * height * width +
                                   c * height * width + h * width + w]
                 << "  ";
          }
          ss << "| ";
...
void print_bias(const char *name, float *bias, const int n_filters,
                const bool fpdata) {
...
bool check_error_threshold_for_neuron(float hw_data, float sw_data,
                                      float &error) {
  ...
  if (std::isinf(float(hw_fdata)) || std::isnan(float(hw_fdata))) {
    return true;
  }
...
int _validate(float *hw_data_array, float *sw_data_array, int batch_size,
              int filters, int output_h, int output_w) {
    ...
    for (int f = 0; f < filters; f++) {
      for (int h = 0; h < output_h; h++) {
        for (int w = 0; w < output_w; w++) {
          ...
          int idx =
              b * filters * round_up(output_h * output_w, DMA_WORD_PER_BEAT) +
              f * round_up(output_h * output_w, DMA_WORD_PER_BEAT) +
              h * output_w + w;
          if (check_error_threshold_for_neuron(float(hw_data_array[idx]),
