...
void system_t::config_proc()
{
    ...
    // Compute in SW
    sw_conv_layer(input, channels, height, width, kernel_h, kernel_w, pad_h, pad_w, stride_h,
                  stride_w, dilation_h, dilation_w, num_filters, weights, bias, sw_output, do_relu,
                  pool_type, batch_size);
    ESP_REPORT_INFO("SW done");

    // printf("Performance: data-in : moved %u (%u bytes) / memory footprint %u (%u bytes) / PLM
    // locality %.2f%%", 	   data_in_movement, data_in_movement * sizeof(FPDATA), channels * height *
    // width, channels * height * 	   width * sizeof(FPDATA),100*(channels * height *
    // width)/((float)data_in_movement)); printf("Performance: weights : moved %u (%u bytes) /
    // memory footprint %u (%u bytes) / PLM locality %.2f%%", 	   weights_movement, weights_movement *
    // sizeof(FPDATA), num_filters * channels * kernel_h * kernel_w, 	   num_filters * channels *
    // kernel_h * kernel_w * sizeof(FPDATA), 	   100*(num_filters * channels * kernel_h *
    // kernel_w)/((float)weights_movement)); printf("Performance: data-out: moved %u (%u bytes) /
    // memory footprint %u (%u bytes) / PLM locality %.2f%%", 	   data_out_movement, data_out_movement *
    // sizeof(FPDATA), num_filters * height * width,  num_filters * 	   height * width * sizeof(FPDATA),
...
#ifdef XS
        print_image("output-hw", hw_output, batch_size, num_filters, output_pool_h, output_pool_w,
                    true);
        print_image("output-sw", sw_output, batch_size, num_filters, output_pool_h, output_pool_w,
                    false);
        printf("\n");
#endif
...
void system_t::load_memory()
{
...
#else
    in_words_adj = round_up(
        batch_size *
            round_up(channels * round_up(height * width, DMA_WORD_PER_BEAT), DMA_WORD_PER_BEAT),
        DMA_WORD_PER_BEAT);
    ...
    out_words_adj =
        round_up(batch_size *
                     round_up(num_filters * round_up(output_h * output_w, DMA_WORD_PER_BEAT),
                              DMA_WORD_PER_BEAT),
                 DMA_WORD_PER_BEAT);
#endif
    ...
    for (; i < (in_size + weights_size + bias_size) / DMA_WORD_PER_BEAT; i++) {
        ...
        for (j = 0; j < DMA_WORD_PER_BEAT; j++)
            data_bv.range((j + 1) * DATA_WIDTH - 1, j * DATA_WIDTH) = fp2bv<FPDATA, DATA_WIDTH>(
