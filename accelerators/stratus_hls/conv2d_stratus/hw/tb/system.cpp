...
void system_t::config_proc()
{
    ...
    // Compute in SW
    sw_conv_layer(input, channels, height, width, kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, dilation_h,
                  dilation_w, num_filters, weights, bias, sw_output, do_relu, pool_type, batch_size);
...
void system_t::load_memory()
{
...
#else
    in_words_adj =
        round_up(batch_size * round_up(channels * round_up(height * width, DMA_WORD_PER_BEAT), DMA_WORD_PER_BEAT),
                 DMA_WORD_PER_BEAT);
    ...
    out_words_adj     = round_up(
        batch_size * round_up(num_filters * round_up(output_h * output_w, DMA_WORD_PER_BEAT), DMA_WORD_PER_BEAT),
        DMA_WORD_PER_BEAT);
#endif
