...
class conf_info_t
{
  public:
    ...
    conf_info_t(
        /* <<--ctor-args-->> */
        int32_t n_channels, int32_t n_filters, int32_t filter_dim, int32_t stride, int32_t is_padded,
        int32_t feature_map_height, int32_t feature_map_width, int32_t do_relu, int32_t pool_type, int32_t batch_size)
    {
        /* <<--ctor-custom-->> */
    ...
    {
        /* <<--eq-->> */
        if (n_channels != rhs.n_channels)
            return false;
        if (n_filters != rhs.n_filters)
            return false;
        if (filter_dim != rhs.filter_dim)
            return false;
        if (stride != rhs.stride)
            return false;
        if (is_padded != rhs.is_padded)
            return false;
        if (feature_map_height != rhs.feature_map_height)
            return false;
        if (feature_map_width != rhs.feature_map_width)
            return false;
        if (do_relu != rhs.do_relu)
            return false;
        if (pool_type != rhs.pool_type)
            return false;
