...
class conv2d : public esp_accelerator_3P<DMA_WIDTH> {
  public:
    ...
    conv2d(const sc_module_name &name) :
        ...
        load_compute_cfg_done("load_compute_cfg_done"), load_store_cfg_done("load_store_cfg_done")
    {
        // Signal binding
        cfg.bind_with(*this);
    ...
    void compute_dimensions(const uint16_t height, const uint16_t width, const uint16_t n_channels,
                            ...
                            const uint16_t n_filters, const uint2_t pool_type,
                            const uint16_t batch_size, uint16_t *output_w, uint4_t *pad,
                            ...
                            uint32_t *feature_offset_start_base, uint12_t *loadable_chan,
                            uint12_t *chan_iters, uint12_t *chan_rem, uint16_t *loadable_chan_sz,
    ...
    void patch_extractor(const uint16_t channels, const uint16_t height, const uint16_t width,
                         ...
                         const uint16_t output_row, const uint16_t output_col, const uint4_t pad,
                         const uint4_t kernel_dim);
    void multiple_multiplier_accumulator(const uint16_t ping_weights, const uint16_t ping_bias,
                                         ...
                                         const uint16_t cacheable_filters,
                                         const uint16_t plm_bias_index,
