...
void conv2d::load_input()
{
    ...
    compute_dimensions(height, width, n_channels, (bool)is_padded, stride, (uint8_t)filter_dim, n_filters, pool_type,
                       batch_size, &output_w, &pad, &feature_size, &filter_size, &filters_size, &max_cacheable_rows,
                       &max_cacheable_rows_init, &max_cacheable_size, &max_cacheable_size_init, &max_cacheable_filters,
                       &max_cacheable_filters_size, &max_cacheable_bias_chunks, &max_cacheable_bias_size,
                       &total_input_chunks, &total_filters_chunks, &feature_offset_incr, &feature_offset_incr_init,
                       &channel_offset_incr, &out_channel_offset_incr, &out_channel_pool_offset_incr,
                       &filters_offset_start_base, &bias_offset_start_base, &feature_offset_start_base, &loadable_chan,
    ...
    // Load
    {
        ...
        for (uint16_t filter_chunk = 0; filter_chunk < total_filters_chunks; filter_chunk++) {
            ...
            if (!bias_chunk) {
                uint16_t n_words_to_load   = min(n_filters - bias_offset_start_virt, max_cacheable_bias_size);
                    ...
                    for (uint12_t in_i = 0; in_i < chan_iters; in_i++) {
                        ...
                        for (uint16_t ch = 0; ch < channels; ch++) {
                            ...
                            uint16_t n_words_to_load =
                                min(feature_size - infeature_offset_start_virt, max_cacheable_size_i);
    ...
    for (uint16_t filter_chunk = 0; filter_chunk < total_filters_chunks; filter_chunk++) {
        ...
        for (uint16_t b = 0; b < batch_size; b++) {
            ...
            for (uint16_t input_chunk = 0; input_chunk < total_input_chunks; input_chunk++) {
                ...
                uint16_t cached_filters = min(max_cacheable_filters, n_filters - filter_chunk * max_cacheable_filters);
                ...
                for (uint16_t filter_i = 0; filter_i < cached_filters; filter_i++) {
                    ...
                    if (pool_type) {
                        uint16_t pool_i_in1_base = filter_i * round_up(rows_to_load_adj * output_w, DMA_WORD_PER_BEAT);
                        uint16_t pool_i_in2_base = pool_i_in1_base + output_w;
                        uint16_t pool_i_out      = filter_i * round_up(n_words_to_store, DMA_WORD_PER_BEAT);
...
void conv2d::compute_kernel()
{
    ...
    uint16_t plm_bias_i = 0;
    for (uint16_t filter_chunk = 0; filter_chunk < total_filters_chunks; filter_chunk++) {
        uint16_t loadable_filters = min(n_filters - filter_chunk * max_cacheable_filters, max_cacheable_filters);
        ...
        for (uint16_t b = 0; b < batch_size; b++) {
            ...
            for (uint16_t input_chunk = 0; input_chunk < total_input_chunks; input_chunk++) {
                ...
                uint16_t loadable_out_size =
                    round_up(((uint16_t)(rows_to_load + stride - 1) >> ilog2(stride)) * output_w, DMA_WORD_PER_BEAT);
