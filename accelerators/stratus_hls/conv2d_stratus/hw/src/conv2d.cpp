...
void conv2d::load_input()
{
    ...
    compute_dimensions(
        height, width, n_channels, (bool)is_padded, stride, (uint8_t)filter_dim, n_filters,
        pool_type, batch_size, &output_w, &pad, &feature_size, &filter_size, &filters_size,
        &max_cacheable_rows, &max_cacheable_rows_init, &max_cacheable_size,
        &max_cacheable_size_init, &max_cacheable_filters, &max_cacheable_filters_size,
        &max_cacheable_bias_chunks, &max_cacheable_bias_size, &total_input_chunks,
        &total_filters_chunks, &feature_offset_incr, &feature_offset_incr_init,
        &channel_offset_incr, &out_channel_offset_incr, &out_channel_pool_offset_incr,
        &filters_offset_start_base, &bias_offset_start_base, &feature_offset_start_base,
        &loadable_chan, &chan_iters, &chan_rem, &loadable_chan_sz, &chan_rem_sz);
    ...
    // Load
    {
        ...
        for (uint16_t filter_chunk = 0; filter_chunk < total_filters_chunks; filter_chunk++) {
            ...
            uint16_t n_words_to_load =
                min(filters_size - filters_offset_start_virt, max_cacheable_filters_size);
...
#else
                if (ping_weights) { plm_weights_ping[plm_weights_index++] = dataBv.to_int(); }
                else {
            ...
            if (!bias_chunk) {
                uint16_t n_words_to_load =
                    min(n_filters - bias_offset_start_virt, max_cacheable_bias_size);
                ...
                for (uint16_t i = 0; i < adj_words_to_load; i += DMA_WORD_PER_BEAT) {
                    ...
                    if (!(!i && misaligned)) {
                        if (ping_bias) {
                            plm_bias_ping[plm_bias_i++] = dataBv.range(31, 0).to_int();
                            if (i + 1 < adj_words_to_load)
                                plm_bias_ping[plm_bias_i++] = dataBv.range(63, 32).to_int();
                        }
                        else {
                            plm_bias_pong[plm_bias_i++] = dataBv.range(31, 0).to_int();
                            if (i + 1 < adj_words_to_load)
                                plm_bias_pong[plm_bias_i++] = dataBv.range(63, 32).to_int();
                        }
                    }
#else
                    if (ping_bias) { plm_bias_ping[plm_bias_i++] = dataBv.to_int(); }
                    else {
            ...
            for (uint16_t b = 0; b < batch_size; b++) {
                ...
                for (uint16_t input_chunk = 0; input_chunk < total_input_chunks; input_chunk++) {
                    ...
                    for (uint12_t in_i = 0; in_i < chan_iters; in_i++) {
                        ...
                        if (in_i < chan_iters - 1) channels = loadable_chan;
                        else
                            channels = chan_rem;
                        ...
                        for (uint16_t ch = 0; ch < channels; ch++) {
                            ...
                            uint16_t n_words_to_load = min(
                                feature_size - infeature_offset_start_virt, max_cacheable_size_i);
...
#else
                                if (ping_input) { plm_in_ping[plm_in_index++] = dataBv.to_int(); }
                                else {
    ...
    for (uint16_t filter_chunk = 0; filter_chunk < total_filters_chunks; filter_chunk++) {
        ...
        for (uint16_t b = 0; b < batch_size; b++) {
            ...
            for (uint16_t input_chunk = 0; input_chunk < total_input_chunks; input_chunk++) {
                ...
                if (!input_chunk) { max_cacheable_rows_i = max_cacheable_rows_init; }
                else {
                ...
                uint16_t loadable_rows = min(height - first_row_to_load, max_cacheable_rows_i);
                uint16_t rows_to_load  = loadable_rows - (no_first_row * pad) - (no_last_row * pad);
                uint16_t rows_to_load_adj = (uint16_t)(rows_to_load + stride - 1) >> ilog2(stride);
                uint16_t rows_to_load_adj_pool = (uint16_t)rows_to_load_adj >> is_pool;
                uint16_t n_words_to_store = rows_to_load_adj_pool * ((uint16_t)output_w >> is_pool);

                uint16_t plm_out_index = 0;
                uint16_t cached_filters =
                    min(max_cacheable_filters, n_filters - filter_chunk * max_cacheable_filters);
                ...
                for (uint16_t filter_i = 0; filter_i < cached_filters; filter_i++) {
                    ...
                    if (pool_type) {
                        uint16_t pool_i_in1_base =
                            filter_i * round_up(rows_to_load_adj * output_w, DMA_WORD_PER_BEAT);
                        uint16_t pool_i_in2_base = pool_i_in1_base + output_w;
                        uint16_t pool_i_out =
                            filter_i * round_up(n_words_to_store, DMA_WORD_PER_BEAT);
                        ...
                        for (uint16_t out_h = 0; out_h < rows_to_load_adj - 1; out_h += 2) {
                            ...
                            for (uint16_t out_w = 0; out_w < output_w - 1; out_w += 2) {
                                ...
                                if (ping_output) { plm_out_ping[pool_i_out++] = FP2INT(res); }
                                else {
...

                    {
                        ...
                        dma_info_t dma_info(offset_start >> DMA_WORD_PER_BEAT_LOG2,
                                            ...
                                            DMA_SIZE);
#ifndef STRATUS_HLS
                        ESP_REPORT_INFO("store dma info. offset: %u len %u", offset_start,
                                        n_words_to_store);
#endif
                        ...
                        for (uint16_t i = 0; i < n_words_to_store; i += DMA_WORD_PER_BEAT) {
                            ...
                            // Write to PLM (all DMA_WORD_PER_BEAT words in one cycle)
                            if (ping_output) {
                                std::cout << "store ping " << INT2FP(plm_out_ping[plm_out_index])
                                          << std::endl;
                                ...
                                std::cout << "store pong " << INT2FP(plm_out_pong[plm_out_index])
                                          << std::endl;
...
void conv2d::compute_kernel()
{
    ...
    uint16_t plm_bias_i = 0;
    for (uint16_t filter_chunk = 0; filter_chunk < total_filters_chunks; filter_chunk++) {
        uint16_t loadable_filters =
            min(n_filters - filter_chunk * max_cacheable_filters, max_cacheable_filters);
        ...
        for (uint16_t b = 0; b < batch_size; b++) {
            ...
            for (uint16_t input_chunk = 0; input_chunk < total_input_chunks; input_chunk++) {
                ...
                uint16_t loadable_out_size =
                    round_up(((uint16_t)(rows_to_load + stride - 1) >> ilog2(stride)) * output_w,
                             DMA_WORD_PER_BEAT);
                ...
                for (uint12_t in_i = 0; in_i < chan_iters; in_i++) {
...
#ifndef STRATUS_HLS
                    ESP_REPORT_INFO("compute_load_handshake %u %u %u %u", filter_chunk, b,
                                    input_chunk, (unsigned)in_i);
#endif
                    uint12_t channels;
                    ...
                    for (uint16_t out_r = 0; out_r < rows_to_load; out_r += stride) {
                        for (uint16_t out_c = 0; out_c < (output_w << ilog2(stride));
                             out_c += stride) {
                            uint16_t ch = 0, ch_base = 0, ch_size = loadable_rows * width;
                            uint16_t k_r = 0, k_c = 0;
                            int16_t in_r_base         = out_r + (no_first_row * pad) - pad,
                                    in_c_base         = out_c - pad;
                            int16_t out_r_i           = out_r + (no_first_row * pad);
                            uint16_t start_addr_base1 = start_addr_base;
                            uint16_t compute_iters =
                                ((uint16_t)(filt_sz_loc - 1) >> (uint4_t)PARAL_LOG2) + 1;
                            for (uint16_t i = 0; i < compute_iters; i++) {
                                ...
                                for (uint16_t j = 0; j < PARALLELISM; j++) {
                                    ...
                                    uint16_t plm_i =
                                        (uint16_t)ch_base + ((int16_t)in_r * width) + in_c;

                                    if (in_r >= 0 && in_r < loadable_rows && in_c >= 0 &&
                                        in_c < width && ch < channels) {
                                        if (ping_input) val = plm_in_ping[plm_i];
                                        else
                                ...
                                for (uint16_t f = 0; f < loadable_filters; f++) {
                                    ...
                                    if (!i && !in_i) { res_partial = FPDATA(0.0); }
                                    else {
                                        if (ping_output) {
                                    ...
                                    res_mac =
                                        ((reg_mac[0] + reg_mac[1]) + (reg_mac[2] + reg_mac[3])) +
                                        ((reg_mac[4] + reg_mac[5]) + (reg_mac[6] + reg_mac[7])) +
                                        res_partial;

                                    if (i + 1 == compute_iters && in_i + 1 == chan_iters) {
                                        if (ping_bias) bias = INT2FP(plm_bias_ping[plm_bias_i + f]);
                                        else
                                            bias = INT2FP(plm_bias_pong[plm_bias_i + f]);
                                        res_bias = res_mac + bias;
                                        if (do_relu && res_bias < 0) res_relu = FPDATA(0);
                                        else
                                            res_relu = res_bias;
                                    }
                                    else {
                    ...
                    if (total_input_chunks != 1 || batch_size != 1) ping_input = !ping_input;
                }
                first_row_to_load += max_cacheable_rows_i - (filter_dim - 1);
                ping_output = !ping_output;
