...
// NOTE
// - most datatypes are custom (e.g. uint4_t), make sure you don't need more
// bits!
...
void conv2d::load_input() {
  ...
  compute_dimensions(
      height, width, n_channels, (bool)is_padded, stride, (uint8_t)filter_dim,
      n_filters, pool_type, batch_size, &output_w, &pad, &feature_size,
      &filter_size, &filters_size, &max_cacheable_rows,
      &max_cacheable_rows_init, &max_cacheable_size, &max_cacheable_size_init,
      &max_cacheable_filters, &max_cacheable_filters_size,
      ...
      &bias_offset_start_base, &feature_offset_start_base, &loadable_chan,
      &chan_iters, &chan_rem, &loadable_chan_sz, &chan_rem_sz);
  ...
  // Load
  {
    ...
    ESP_REPORT_INFO("max_cacheable_filters_size %u",
    ...
    for (uint16_t filter_chunk = 0; filter_chunk < total_filters_chunks;
      ...
      dma_info_t dma_info(
          filters_offset_start_phys >> DMA_WORD_PER_BEAT_LOG2,
      ...
      if (!bias_chunk) {
        ...
        dma_info_t dma_info(
            bias_offset_start_phys >> DMA_WORD_PER_BEAT_LOG2,
            (n_words_to_load + misaligned + DMA_WORD_PER_BEAT_LOG2) >>
                DMA_WORD_PER_BEAT_LOG2,
            DMA_SIZE, 0);
...
#ifndef STRATUS_HLS
              ESP_REPORT_INFO(
                  "load_input load features dma_info. offset: %u len %u",
    ...
    for (uint16_t b = 0; b < batch_size; b++) {
      ...
      for (uint16_t input_chunk = 0; input_chunk < total_input_chunks;
        ...
        uint16_t loadable_rows =
            min(height - first_row_to_load, max_cacheable_rows_i);
        uint16_t rows_to_load =
            loadable_rows - (no_first_row * pad) - (no_last_row * pad);
        uint16_t rows_to_load_adj =
            (uint16_t)(rows_to_load + stride - 1) >> ilog2(stride);
        uint16_t rows_to_load_adj_pool = (uint16_t)rows_to_load_adj >> is_pool;
        uint16_t n_words_to_store =
            rows_to_load_adj_pool * ((uint16_t)output_w >> is_pool);

        uint16_t plm_out_index = 0;
        uint16_t cached_filters =
            min(max_cacheable_filters,
        ...
        for (uint16_t filter_i = 0; filter_i < cached_filters; filter_i++) {
          uint32_t offset_start =
              out_channel_offset + feature_offset_start_phys;
      ...
      feature_offset_start_base_tmp +=
          (n_filters * out_channel_pool_offset_incr);
    }
...
void conv2d::compute_kernel() {
  ...
  for (uint16_t filter_chunk = 0; filter_chunk < total_filters_chunks;
    ...
    uint16_t loadable_filters =
        min(n_filters - filter_chunk * max_cacheable_filters,
    ...
    for (uint16_t b = 0; b < batch_size; b++) {
      ...
      for (uint16_t input_chunk = 0; input_chunk < total_input_chunks;
        ...
        uint16_t loadable_out_size = round_up(
            ((uint16_t)(rows_to_load + stride - 1) >> ilog2(stride)) * output_w,
            DMA_WORD_PER_BEAT);
