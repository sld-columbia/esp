...
#endif
        if (!gold[j] && val ||
            (((gold[j] - val) / gold[j]) > REL_ERROR_THRESHOLD || ((gold[j] - val) / gold[j]) < -REL_ERROR_THRESHOLD)) {
            errors++;
...
#include "gold.h"
    ...
    size = (n_channels * feature_map_height * feature_map_width) +
           (n_channels * n_filters * filter_height * filter_width) + n_filters;
}
...
int main(int argc, char *argv[])
{
    ...
    if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
        ...
        in_words_adj =
            round_up(n_channels * feature_map_height * feature_map_width, DMA_WORD_PER_BEAT(sizeof(token_t)));
        weights_words_adj =
            round_up(n_filters * n_channels * filter_height * filter_width, DMA_WORD_PER_BEAT(sizeof(token_t)));
        bias_words_adj = round_up(n_filters, DMA_WORD_PER_BEAT(sizeof(token_t)));
        out_words_adj =
            round_up(n_filters * feature_map_height * feature_map_width, DMA_WORD_PER_BEAT(sizeof(token_t)));
    }
