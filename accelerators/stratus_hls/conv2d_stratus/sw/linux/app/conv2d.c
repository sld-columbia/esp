...
static void sw_run(int32_t n_channels, int32_t feature_map_height, int32_t feature_map_width, int32_t n_filters,
...
int main(int argc, char **argv)
{
    ...
    for (test = start_test - 1; test < n_tests + start_test - 1; ++test) {
        ...
        init_parameters(test, n_channels[test], feature_map_height[test], feature_map_width[test], n_filters[test],
                        filter_dim[test], is_padded[test], stride[test], do_relu[test], pool_type[test],
                        batch_size[test], &in_len, &weights_len, &bias_len, &out_len, &in_size, &weights_size,
                        &bias_size, &out_size, &weights_offset, &bias_offset, &out_offset, &size);
        ...
        sw_run(n_channels[test], feature_map_height[test], feature_map_width[test], n_filters[test], filter_dim[test],
               is_padded[test], stride[test], do_relu[test], pool_type[test], batch_size[test], sw_buf,
