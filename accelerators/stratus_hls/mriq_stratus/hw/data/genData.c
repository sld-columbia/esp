...
int main(int argc, char **argv)
{
 ...
 2. numK\n");
    ...
    inputData(in_Parboil, &numK, &numX, &kx, &ky, &kz, &x, &y, &z, &phiR, &phiI);
    ...
    if (ret != numK) { printf("not writing correctly!\n"); }
    ...
    if (ret != numX) { printf("not writing correctly!\n"); }
    ...
    ComputeQ(numK, numX, kx, ky, kz, x, y, z, phiR, phiI, Qr, Qi);

    FILE *fout = fopen(goldenName, "wr");
    if (fout == NULL) { printf("can't open file"); }
    // write numX to the golden output file
    ret = fwrite(&numX, sizeof(float), 1, fout);

    if (ret != 1) { printf("not writing correctly!\n"); }
