...
void write_in(FILE *fp, unsigned len, float *val, unsigned batch,
              unsigned base_in) {
...
void write_gold(FILE *fp, unsigned len, float *val, unsigned batch,
                unsigned base_in) {
  unsigned base = batch * len;
...
6. num_batch_x\n");
  ...
  inputData(inputName, &numK, &numX, &kx, &ky, &kz, &x, &y, &z, &phiR, &phiI);
