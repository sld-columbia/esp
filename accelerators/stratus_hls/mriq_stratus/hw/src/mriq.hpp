...
class mriq : public esp_accelerator_3P<DMA_WIDTH> {
  public:
    // Constructor
    SC_HAS_PROCESS(mriq);
    mriq(const sc_module_name &name) : esp_accelerator_3P<DMA_WIDTH>(name), cfg("config")
    {
        // Signal binding
        cfg.bind_with(*this);
    ...
    void ComputeQ(FPDATA_S x, FPDATA_S y, FPDATA_S z, uint16_t batch_size_k, bool pingpong_k,
