...
template <typename T> void mriq::dma_read(T array[], uint32_t dma_index, uint32_t dma_length)
{
    ...
    for (uint16_t i = 0; i < dma_length; i++) {
        ...
        for (int k = 0; k < DMA_WORD_PER_BEAT; k++) {
            HLS_UNROLL_SIMPLE;
            array[i * DMA_WORD_PER_BEAT + k] =
                data.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH).to_int64();
        }
    }
}

template <typename T> void mriq::dma_write(T array[], uint32_t dma_index, uint32_t dma_length)
{
    dma_info_t dma_info(dma_index, dma_length, DMA_SIZE, 0);
...
void mriq::ComputeQ(FPDATA_S x, FPDATA_S y, FPDATA_S z, uint16_t batch_size_k, bool pingpong_k,
                    FPDATA_S *sin_table, FPDATA_L *Qr, FPDATA_L *Qi)
{
    int indexK, i;
    ...
    #elif (PARAL == 8)
        Qracc += Qracc_p[0] + Qracc_p[1] + Qracc_p[2] + Qracc_p[3] + Qracc_p[4] + Qracc_p[5] +
            Qracc_p[6] + Qracc_p[7];
        Qiacc += Qiacc_p[0] + Qiacc_p[1] + Qiacc_p[2] + Qiacc_p[3] + Qiacc_p[4] + Qiacc_p[5] +
            Qiacc_p[6] + Qiacc_p[7];
    #elif (PARAL == 16)
        Qracc += Qracc_p[0] + Qracc_p[1] + Qracc_p[2] + Qracc_p[3] + Qracc_p[4] + Qracc_p[5] +
            Qracc_p[6] + Qracc_p[7] + Qracc_p[8] + Qracc_p[9] + Qracc_p[10] + Qracc_p[11] +
            Qracc_p[12] + Qracc_p[13] + Qracc_p[14] + Qracc_p[15];
        Qiacc += Qiacc_p[0] + Qiacc_p[1] + Qiacc_p[2] + Qiacc_p[3] + Qiacc_p[4] + Qiacc_p[5] +
            Qiacc_p[6] + Qiacc_p[7] + Qiacc_p[8] + Qiacc_p[9] + Qiacc_p[10] + Qiacc_p[11] +
            Qiacc_p[12] + Qiacc_p[13] + Qiacc_p[14] + Qiacc_p[15];
    #endif

    #if (0) // the following implementation can't have a valid scheduling when generating RTL for
            // PARAL=8 and 16.
        for (i = 0; i < unroll_factor; i++) {
            HLS_UNROLL_SIMPLE;
...
FPDATA_S mriq::mySinf(FPDATA_L angle, FPDATA_S *sin_table)
{
    ...
    angle_within_2pi = angle -
        PI * 2 *
            floor(cynw_fixed<FPDATA_L_WL, FPDATA_L_IL, SC_TRN, SC_WRAP, 1>(angle * REC_PI * 0.5));

    q = floor(
            cynw_fixed<FPDATA_S_WL, FPDATA_S_IL, SC_TRN, SC_WRAP, 1>(angle_within_2pi * 2 * REC_PI))
            .to_int();

    angle_within_halfpi = angle_within_2pi - (HALF_PI)*q;

    switch (q) {
        case 0: radian = angle_within_2pi; break;
        case 1: radian = PI - angle_within_2pi; break;
        case 2: radian = angle_within_2pi - PI; break;
        case 3: radian = PI * 2 - angle_within_2pi; break;
        default: radian = angle_within_2pi; break;
    }

    i = floor(cynw_fixed<FPDATA_L_WL, FPDATA_L_IL, SC_TRN, SC_WRAP, 1>(radian * REC_PI * 40))
            .to_int();

    if (i >= 19) ans_sin = sin_table[19] + (1 - sin_table[19]) * (radian * REC_PI * 40 - 19);
    else
