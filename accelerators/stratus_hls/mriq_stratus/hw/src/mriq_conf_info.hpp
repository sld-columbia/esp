...
class conf_info_t
{
  public:
    ...
    conf_info_t(
        ...
        int32_t num_batch_k, int32_t batch_size_k, int32_t num_batch_x, int32_t batch_size_x)
    {
        /* <<--ctor-custom-->> */
    ...
    {
        /* <<--eq-->> */
        if (num_batch_k != rhs.num_batch_k)
            return false;
        if (batch_size_k != rhs.batch_size_k)
            return false;
        if (num_batch_x != rhs.num_batch_x)
            return false;
        if (batch_size_x != rhs.batch_size_x)
            return false;
        return true;
    }
