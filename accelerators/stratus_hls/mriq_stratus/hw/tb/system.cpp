...
void system_t::load_mem(float *in, int offset, int32_t base_index,
                        int32_t size) {
  FPDATA_S data_fp;
  ...
  for (int16_t i = 0; i < size; i += DMA_WORD_PER_BEAT) {
    for (int k = 0; k < DMA_WORD_PER_BEAT; k++) {
      ...
      mem[base_index + mem_i].range((k + 1) * FPDATA_S_WL - 1,
                                    k * FPDATA_S_WL) = data_bv;
    }
    mem_i++;
  }
}
...
void system_t::load_memory() {
  ...
  init_parameters(batch_size_x, num_batch_x, batch_size_k, num_batch_k,
                  &in_words_adj, &out_words_adj, &in_len, &out_len, &in_size,
                  &out_size, &out_offset, &mem_size, DMA_WORD_PER_BEAT);

  // load input data to array "in" and golden output to array "gold"
  in = new float[in_len];
  gold = new float[out_len];

  init_buffer(in, gold, esc_argv()[1], esc_argv()[2], batch_size_x, num_batch_x,
  ...
  for (int r = 0; r < num_batch_x; r++) {
    ...
    for (int i = 0; i < batch_size_x / DMA_WORD_PER_BEAT; i++) {
      ...
      for (int j = 0; j < DMA_WORD_PER_BEAT; j++) {
        ...
        data_bv =
            mem[offset + i].range((j + 1) * FPDATA_L_WL - 1, j * FPDATA_L_WL);
