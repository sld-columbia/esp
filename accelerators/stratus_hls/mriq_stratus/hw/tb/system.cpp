...
void system_t::load_memory()
{
    ...
    init_parameters(batch_size_x, num_batch_x, batch_size_k, num_batch_k, &in_words_adj, &out_words_adj, &in_len,
                    &out_len, &in_size, &out_size, &out_offset, &mem_size, DMA_WORD_PER_BEAT);

    // load input data to array "in" and golden output to array "gold"
    in   = new float[in_len];
    gold = new float[out_len];

    init_buffer(in, gold, esc_argv()[1], esc_argv()[2], batch_size_x, num_batch_x, batch_size_k, num_batch_k);
