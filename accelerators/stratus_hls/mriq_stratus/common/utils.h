...
void init_parameters(int32_t batch_size_x, int32_t num_batch_x,
                     ...
                     unsigned *in_len, unsigned *out_len, unsigned *in_size,
                     unsigned *out_size, unsigned *out_offset,
                     unsigned *mem_size, unsigned dma_word_per_beat) {
  if (dma_word_per_beat == 0) {
    *in_words_adj =
        3 * batch_size_x * num_batch_x + 5 * batch_size_k * num_batch_k;
    *out_words_adj = 2 * batch_size_x * num_batch_x;
  } else {
    *in_words_adj = round_up(3 * batch_size_x * num_batch_x +
                                 5 * batch_size_k * num_batch_k,
                             dma_word_per_beat);
    *out_words_adj =
        round_up(2 * batch_size_x * num_batch_x, dma_word_per_beat);
  }
