...
void sw_comp(int batch_size_k, int num_batch_k, int batch_size_x, int num_batch_x, float *kx,
             float *ky, float *kz, float *x, float *y, float *z, float *phiR, float *phiI,
             float *out)
{
    float expArg;
    float cosArg;
    ...
    for (int b = 0; b < num_batch_x; b++) {
        ...
        for (indexX = 0; indexX < batch_size_x; indexX++) {
            ...
            for (indexK = 0; indexK < numK; indexK++) {
                ...
                expArg = PIx2 *
                    (kx[indexK] * x[base + indexX] + ky[indexK] * y[base + indexX] +
...
unsigned long long sw_exec(float *gold, float *in, int32_t batch_size_x, int32_t num_batch_x,
                           int32_t batch_size_k, int32_t num_batch_k)
{
    ...
    sw_comp(batch_size_k, num_batch_k, batch_size_x, num_batch_x, kx, ky, kz, x, y, z, phiR, phiI,
            gold);
    gettime(&ts_end);
