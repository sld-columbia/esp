...
void inputData(const char *fName, int *_numK, int *_numX, float **kx, float **ky, float **kz, float **x, float **y,
               float **z, float **phiR, float **phiI);

void outputData(const char *fName, float **outR, float **outI, int *numX);

void ComputeQ(int numK, int numX, float *plm_kx, float *plm_ky, float *plm_kz, float *plm_x, float *plm_y, float *plm_z,
              ...
              float *plm_phiR, float *plm_phiI, float *plm_Qr, float *plm_Qi)
{
    ...
    for (indexX = 0; indexX < numX; indexX++) {
        ...
        for (indexK = 0; indexK < numK; indexK++) {
            ...
            expArg = Pix2 *
                     (plm_kx[indexK] * plm_x[indexX] + plm_ky[indexK] * plm_y[indexX] + plm_kz[indexK] * plm_z[indexX]);
            cosArg = cosf(expArg);
            sinArg = sinf(expArg);
