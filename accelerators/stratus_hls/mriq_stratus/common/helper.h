...
void inputData(const char *fName, int *_numK, int *_numX, float **kx,
               float **ky, float **kz, float **x, float **y, float **z,
               float **phiR, float **phiI);

void outputData(const char *fName, float **outR, float **outI, int *numX);

void ComputeQ(int numK, int numX, float *plm_kx, float *plm_ky, float *plm_kz,
              float *plm_x, float *plm_y, float *plm_z, float *plm_phiR,
              float *plm_phiI, float *plm_Qr, float *plm_Qi);
...
void ComputeQ(int numK, int numX, float *plm_kx, float *plm_ky, float *plm_kz,
  ...
  for (indexX = 0; indexX < numX; indexX++) {
    ...
    for (indexK = 0; indexK < numK; indexK++) {
      phiMag = plm_phiR[indexK] * plm_phiR[indexK] +
               plm_phiI[indexK] * plm_phiI[indexK];
