...
static void prepare_dummy(int *fd, contig_handle_t *mem, accelerator_thread_info_t *info,
                          struct dummy_stratus_access *desc, const char *device_name, unsigned id,
                          unsigned do_p2p)
{
    *fd = open(device_name, O_RDWR, 0);
    ...
    switch (id) {
        case 0:
            ...
            if (do_p2p) desc->esp.p2p_store = 1;
            break;
        case 1:
            desc->batch      = BATCH / 2;
