...
static void prepare_dummy(int *fd, contig_handle_t *mem,
                          accelerator_thread_info_t *info,
                          struct dummy_stratus_access *desc,
                          const char *device_name, unsigned id,
                          unsigned do_p2p) {
...
static void init_buffer(contig_handle_t *hwbuf) {
  ...
  for (j = 0; j < BATCH; j++)
    for (i = 0; i < TOKENS; i++)
      contig_write64(0xFEED0BAC00000000L | (u64)i, *hwbuf,
                     (j * TOKENS + i) * sizeof(u64));
...
static void validate_buffer(contig_handle_t *hwbuf) {
  ...
  for (j = 0; j < BATCH; j++)
    for (i = 0; i < TOKENS; i++) {
      u64 token = contig_read64(*hwbuf, 2 * out_offset +
                                            (j * TOKENS + i) * sizeof(u64));
...
static void print_time_info(accelerator_thread_info_t *info,
                            unsigned long long hw_ns, int nthreads) {
  int i;
