...
static bool vitbfly2_xfer_input_ok(struct esp_device *esp, void *arg) { return true; }
...
static int vitbfly2_probe(struct platform_device *pdev)
{
    ...
    if (vitbfly2 == NULL) return -ENOMEM;
    esp         = &vitbfly2->esp;
...
static int __init vitbfly2_init(void) { return esp_driver_register(&vitbfly2_driver); }

static void __exit vitbfly2_exit(void) { esp_driver_unregister(&vitbfly2_driver); }

module_init(vitbfly2_init) module_exit(vitbfly2_exit)
