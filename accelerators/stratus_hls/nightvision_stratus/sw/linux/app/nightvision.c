...
#define CLIP_INRANGE(LOW, VAL, HIGH)                                           \
  ((VAL) < (LOW) ? (LOW) : ((VAL) > (HIGH) ? (HIGH) : (VAL)))

int dwt53_row_transpose(algPixel_t *data, algPixel_t *data2, int nrows,
                        int ncols) {
  int i, j, cur;
...
int dwt53_row_transpose_inverse(algPixel_t *data, algPixel_t *data2, int nrows,
                                int ncols) {
  int i, j, cur;
  for (i = 0; i < nrows; i++) {
...
int dwt53(algPixel_t *data, int nrows, int ncols) {
  ...
  algPixel_t *data2 = (algPixel_t *)calloc(nrows * ncols, sizeof(algPixel_t));
  if (!data2) {
    fprintf(stderr, "File %s, Line %d, Memory Allocation Error", __FILE__,
            __LINE__);
    return -1;
  }
...
int hist(algPixel_t *streamA, int *h, int nRows, int nCols, int nBpp) {
  ...
  for (i = 0; i < nPxls; i++) {
    if (streamA[i] >= nBins) {
      fprintf(stderr,
              "File %s, Line %d, Range Error in hist() -- using max val ---",
              __FILE__, __LINE__);
      h[nBins - 1]++;
    } else {
      h[(int)streamA[i]]++;
...
int histEq(algPixel_t *streamA, algPixel_t *out, int *h, int nRows, int nCols,
           int nInpBpp, int nOutBpp) {
...
int readImage(void *image, char *srcDir, char *fileName, int nRows, int nCols,
              int nFrames, int nBytesPerPxl, bool bSwap) {
  char *origDir = NULL;
  __int64 *p64 = (__int64 *)image;
  unsigned long *p32 = (unsigned long *)image;
  unsigned short *p16 = (unsigned short *)image;
...
int saveFrame(void *image, char *dstDir, char *baseFileName, int nRows,
              int nCols, int frameNo, int nBytesPerPxl, bool bSwap) {
  // char *origDir = NULL;
  __int64 *p64 = (__int64 *)image;
  unsigned long *p32 = (unsigned long *)image;
  unsigned short *p16 = (unsigned short *)image;
  ...
  if (fp == (FILE *)NULL) {
    fprintf(stderr, "File %s, Line %d, Failed fopen() on file: %s\n", __FILE__,
            __LINE__, fullFileName);
    return -1;
  }
  if (fwrite((void *)image, nBytesPerPxl, nPxlsToWrite, fp) !=
      (size_t)nPxlsToWrite) {
    fclose(fp);
...
int ensemble_1(algPixel_t *streamA, algPixel_t *out, int nRows, int nCols,
               int nInpBpp, int nOutBpp) {
  // NF
  // H
  // HE
...
static const char usage_str[] =
    "usage: ./nightvision_stratus.exe coherence infile [nimages] [ncols] "
    "[nrows] [-v]\n"
...
static void nightvision_comp(struct test_info *info) {
  ...
  for (i = 0; i < t->nimages; i++) {
    ...
    algErr |=
        ensemble_1(buf_in, buf_out, t->rows, t->cols, t->nbpp_in, t->nbpp_out);
    assert(algErr == 0);
  }

  if (t->infile_is_raw) {
    // save raw output image
    numOut = saveFrame(buf_out, ".", "ensemble1", t->rows, t->cols, 0,
                       sizeof(int), false);
...
static bool nightvision_diff_ok(struct test_info *info) {
  ...
  err = check_gold(t->sbuf_out, t->hbuf, t->rows * t->cols * t->nimages,
                   t->verbose);
...
/*
 * The app currently has some hard-coded parameters, that should be made
 * configurable.:
 * - do_dwt = false. DWT always disabled. This means that the output of the
 * accelerator is expected to be unsigned.
 * - in/out images in TXT format only, although the RAW format is supported as
 * well
 * - word bitwidth and related data type, it should be extracted from the HLS
 * config of the accelerator. NBPP_IN and NBPP_OUT.
 */
