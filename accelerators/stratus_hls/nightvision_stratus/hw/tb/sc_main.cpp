...
extern void esc_elaborate() {
  ...
  testbench =
      new system_t("testbench", image_A_path, image_out_path, n_Images, n_Rows,
                   n_Cols, image_gold_path, do_validation, do_dwt);
}
...
int sc_main(int argc, char *argv[]) {
  ...
  if (argc == 6 || argc == 7) {
    ...
    fprintf(stderr, "Expected arguments: image_A_path (string), image_out_path "
                    "(string),\n");
    fprintf(
        stderr,
        "n_Images, n_Rows, n_Cols, [optional: image_gold_path (string)].\n");
