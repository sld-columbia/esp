  ...
  for (i = 0; i < n_Images; i++) {
    for (j = 0; j < n_Pixels; j += WORDS_PER_DMA) {
      for (k = 0; k < WORDS_PER_DMA; k++) {
        mem[mem_i].range(((k + 1) << MAX_PXL_WIDTH_LOG) - 1,
                         k << MAX_PXL_WIDTH_LOG) =
...
int system_t::validate() {
  ...
  for (j = 0; j < n_Images; j++) {
    for (i = 0; i < n_Pixels; i++) {
      if (imgOut[i] != imgGold[i]) {
        //                ESP_REPORT_INFO("Error: i = %d\tOut = %d\tGold =
  ...
  ESP_REPORT_INFO("====================================================");
  if (errors) {
    ESP_REPORT_INFO("Errors: %d out of %d pixel(s) not match.", errors,
                    n_Pixels);
  } else {
