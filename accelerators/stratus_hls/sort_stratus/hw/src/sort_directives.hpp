...
#define NUM   (1 << lgNUM)
    ...
    // Memory allocation
        ...
        #define HLS_MAP_A0 HLS_MAP_TO_MEMORY(A0, "sort_plm_block_1w1r")
        #define HLS_MAP_A1 HLS_MAP_TO_MEMORY(A1, "sort_plm_block_1w1r")
        #define HLS_MAP_B0 HLS_MAP_TO_MEMORY(B0, "sort_plm_block_1w1r")
        #define HLS_MAP_B1 HLS_MAP_TO_MEMORY(B1, "sort_plm_block_1w1r")

    #elif (DMA_WIDTH == 64)

        #define HLS_MAP_A0 HLS_MAP_TO_MEMORY(A0, "sort_plm_block_2w1r")
        #define HLS_MAP_A1 HLS_MAP_TO_MEMORY(A1, "sort_plm_block_2w1r")
        #define HLS_MAP_B0 HLS_MAP_TO_MEMORY(B0, "sort_plm_block_1w2r")
        #define HLS_MAP_B1 HLS_MAP_TO_MEMORY(B1, "sort_plm_block_1w2r")
    #endif

    #define HLS_MAP_C0 HLS_MAP_TO_MEMORY(C0, "sort_plm_block_1w1r")
    #define HLS_MAP_C1 HLS_MAP_TO_MEMORY(C1, "sort_plm_block_1w1r")
    ...
    #define HLS_RB_RESET  HLS_DEFINE_PROTOCOL("rb-reset")
    #define HLS_RB_CONFIG HLS_DEFINE_PROTOCOL("rb-config")

    #define HLS_MERGE_RESET  HLS_DEFINE_PROTOCOL("merge-reset")
    #define HLS_MERGE_CONFIG HLS_DEFINE_PROTOCOL("merge-config")

    // Datapath micro-architecture

    #if defined(HLS_DIRECTIVES_BASIC)

        #define HLS_RB_SORT_LOOP HLS_UNROLL_LOOP(OFF)
        #define HLS_RB_MAP_REGS  HLS_FLATTEN_ARRAY(regs)
        #define HLS_RB_MAIN      HLS_UNROLL_LOOP(OFF)
        ...
        #define HLS_RB_INSERTION_OUTER HLS_UNROLL_LOOP(OFF)
        #define HLS_RB_INSERTION_EVEN  HLS_UNROLL_LOOP(ON)
        #define HLS_RB_INSERTION_ODD   HLS_UNROLL_LOOP(ON)
        ...
        #define HLS_MERGE_SORT_LOOP HLS_UNROLL_LOOP(OFF)
        ...
        #define HLS_MERGE_INIT_ZERO_FIDX HLS_UNROLL_LOOP(ON)
        ...
        #define HLS_MERGE_MAIN                                    \
            ...
            HLS_UNROLL_LOOP(OFF)
        #define HLS_MERGE_COMPARE     HLS_UNROLL_LOOP(ON)
        #define HLS_MERGE_SHIFT_ARRAY HLS_UNROLL_LOOP(ON)
        #define HLS_MERGE_DEBRUIJN32  HLS_FLATTEN_ARRAY(DeBruijn32)
        #define HLS_MERGE_SHIFT_REV   HLS_UNROLL_LOOP(ON)
        #define HLS_MERGE_SHIFT       HLS_UNROLL_LOOP(ON)
        #define HLS_MERGE_POP_ARRAY   HLS_UNROLL_LOOP(ON)
        #define HLS_MERGE_SEQ         HLS_UNROLL_LOOP(ON)
        ...
        #define HLS_MERGE_DO_POP            \
            HLS_BREAK_ARRAY_DEPENDENCY(C0); \
            HLS_BREAK_ARRAY_DEPENDENCY(C1)
        #define HLS_MERGE_ZERO           HLS_UNROLL_LOOP(ON)
        #define HLS_MERGE_NO_MERGE_OUTER HLS_UNROLL_LOOP(OFF)
