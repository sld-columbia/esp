        ...
        for (uint16_t b = 0; b < bursts; b++) {
            ...
            if (chunk_max > 1) // MERGE is needed
            {
                ...
                while (true) {
                    ...
                    const int DeBruijn32[32] = {0,  1,  28, 2,  29, 14, 24, 3,  30, 22, 20,
                    ...
                    for (uint8_t chunk = 0; chunk < LEN / NUM; chunk++) {
                        ...
                        if (chunk < cur) regs[chunk] = regs_in[chunk];
                    }

                    if (cur == chunk_max) {
                        HLS_MERGE_WR_LAST_ELEMENTS;
                        // write output
                        if (ping) B0[cnt] = regs[chunk_max - 1];
                        else
                    ...
                    if (pop_idx != -1) {
                        HLS_MERGE_DO_POP;
                        if (ping) head[pop_idx] = C0[pop_idx][fidx[pop_idx]];
                        else
