...
void system_t::load_memory()
{
    ...
    if (!(s1 >> x)) { ESP_REPORT_ERROR("Invalid number %s \n", esc_argv()[1]); }
    SORT_LEN = x;
    std::istringstream s2(esc_argv()[2]);
    if (!(s2 >> x)) { ESP_REPORT_ERROR("Invalid number %s \n", esc_argv()[2]); }
    SORT_BATCH = x;
#else
        ...
        for (uint32_t i = 0; i < SORT_LEN / ratio; i++)
            for (uint32_t k = 0; k < ratio; k++) {
                sc_dt::sc_bv<32> data_bv =
                    mem[SORT_LEN * j / ratio + i].range(32 * (k + 1) - 1, 32 * k);
...
int system_t::validate()
{
    ...
    for (int j = 0; j < SORT_BATCH; j++) {
        ...
        if (local_errors) { ESP_REPORT_INFO("sort[%d] faild\n", j); }
        errors += local_errors;
    }
