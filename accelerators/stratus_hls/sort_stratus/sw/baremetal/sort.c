...
#define NCHUNK      ((SORT_BUF_SIZE % CHUNK_SIZE == 0) ? (SORT_BUF_SIZE / CHUNK_SIZE) : (SORT_BUF_SIZE / CHUNK_SIZE) + 1)
...
int main(int argc, char *argv[])
{
    ...
    for (n = 0; n < ndev; n++) {
...
#else
        {
            ...
            printf("******************** %s.%d ********************\n", DEV_NAME, n);
            // Check access ok
            if (SORT_LEN < sort_len_min || SORT_LEN > sort_len_max || SORT_BATCH < 1 || SORT_BATCH > sort_batch_max) {
                printf("  Error: unsopported configuration parameters for %s.%d\n", DEV_NAME, n);
                printf("         device can sort up to %d fp-vectors of size [%d, %d]\n", sort_batch_max, sort_len_min,
                       sort_len_max);
                break;
            }
