...
int main(int argc, char * argv[])
{
	...
	for (n = 0; n < ndev; n++) {
	    ...
	    sw_buf = aligned_malloc(SORT_BUF_SIZE);
	    ...
	    init_buf((float *) sw_buf, SORT_LEN, SORT_BATCH);
		...
		quicksort((float*)&sw_buf[i * SORT_LEN], SORT_LEN);
	    ...
	    for (j = 0; j < SORT_BATCH; j++) {
		int err = validate_sorted((float *) &sw_buf[j * SORT_LEN], SORT_LEN);
		/* if (err != 0) */
		/* 	printf("  Error: %s.%d mismatch on batch %d\n", DEV_NAME, n, j); */
		errors += err;
	    }
