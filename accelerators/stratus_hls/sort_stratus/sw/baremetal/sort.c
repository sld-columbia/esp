...
#define NCHUNK                                                                 \
  ((SORT_BUF_SIZE % CHUNK_SIZE == 0) ? (SORT_BUF_SIZE / CHUNK_SIZE)            \
...
static void init_buf(float *buf, unsigned sort_size, unsigned sort_batch) {
  ...
  for (j = 0; j < sort_batch; j++)
    for (i = 0; i < sort_size; i++) {
      ...
      /* float M = 100000.0; */
      /* buf[sort_size * j + i] =  M * ((float) rand() / (float) RAND_MAX) -
       * M/2; */
      /* /\* Easyto debug...! *\/ */
...
int main(int argc, char *argv[]) {
  ...
  for (n = 0; n < ndev; n++) {
...
#else
    {
      ...
      printf("******************** %s.%d ********************\n", DEV_NAME, n);
      // Check access ok
      if (SORT_LEN < sort_len_min || SORT_LEN > sort_len_max ||
          SORT_BATCH < 1 || SORT_BATCH > sort_batch_max) {
        printf("  Error: unsopported configuration parameters for %s.%d\n",
               DEV_NAME, n);
        printf(
            "         device can sort up to %d fp-vectors of size [%d, %d]\n",
