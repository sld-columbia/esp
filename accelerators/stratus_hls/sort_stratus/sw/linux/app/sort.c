...
static const char usage_str[] =
    "usage: sort coherence cmd [n_elems] [n_batches] [-v]\n"
...
static int partition(float *array, int low, int high)
{
    ...
    if ((array[low] - array[mid]) * (array[high] - array[low]) >= 0) pivot = low;
...
static int check_gold(float *gold, float *array, int len, bool verbose)
{
    ...
    for (i = 0; i < len; i++) {
        if (array[i] != gold[i]) {
            if (verbose) printf("A[%d]: array=%.15g; gold=%.15g\n", i, array[i], gold[i]);
            rtn++;
        }
    }
    return rtn;
...
static void sort_alloc_buf(struct test_info *info)
{
    ...
    if (!strcmp(info->cmd, "test")) t->sbuf = malloc0_check(sort_size(t));
}
...
static void sort_alloc_contig(struct test_info *info)
{
    ...
    if (contig_alloc(sort_size(t), &info->contig) == NULL) die_errno(__func__);
}
...
static void sort_init_bufs(struct test_info *info)
{
    ...
    if (!strcmp(info->cmd, "test")) memcpy(t->sbuf, t->hbuf, sort_size(t));
}
...
static bool sort_diff_ok(struct test_info *info)
{
    ...
    for (i = 0; i < t->n_batches; i++) {
        ...
        if (err) printf("Batch %d: %d mismatches\n", i, err);
        total_err += err;
    }
    if (t->verbose) {
        for (i = 0; i < t->n_batches; i++) {
...
int main(int argc, char *argv[])
{
    ...
    if (!strcmp(argv[2], "run") || !strcmp(argv[2], "flush")) n_argc = 3;
    else
        n_argc = 5;
