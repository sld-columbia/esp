...
static bool sort_xfer_input_ok(struct esp_device *esp, void *arg)
{
    ...
    if (a->size & size_mask || a->size > sort->max_len || a->size < sort->min_len ||
        a->batch > sort->max_batch || a->batch < 1)
        return false;
    return true;
}
...
static int __init sort_init(void) { return esp_driver_register(&sort_driver); }

static void __exit sort_exit(void) { esp_driver_unregister(&sort_driver); }

module_init(sort_init) module_exit(sort_exit)
