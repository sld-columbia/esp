...
static int cholesky_probe(struct platform_device *pdev)
{
    ...
    if (cholesky == NULL) return -ENOMEM;
    esp         = &cholesky->esp;
...
static int __init cholesky_init(void) { return esp_driver_register(&cholesky_driver); }

static void __exit cholesky_exit(void) { esp_driver_unregister(&cholesky_driver); }

module_init(cholesky_init) module_exit(cholesky_exit)
