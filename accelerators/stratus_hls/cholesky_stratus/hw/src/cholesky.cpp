    ...
    for (uint16_t b = 0; b < 1; b++) {
      ...
      for (int rem = length; rem > 0; rem -= rows) {
        ...
        if (start_fetching_output >= start_fetching_output_thresh) {
          ...
          for (int fetch_index = 0; fetch_index < fetch_time + 1;
               fetch_index++) {
            wait();

            if (skip) {
              uint32_t fetch_len = rows;
              fetch_len = (DMA_WORD_PER_BEAT == 2 && !even_rows) ? fetch_len + 1
                                                                 : fetch_len;
...
void cholesky::store_output() {
  ...
  // Store
  {
    ...
    uint32_t offset = (DMA_WORD_PER_BEAT == 2 && !even_rows) ? store_offset + 1
                                                             : store_offset;
