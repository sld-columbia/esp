                ...
                if (start_fetching_output >= start_fetching_output_thresh) {
                    ...
                    for (int fetch_index = 0; fetch_index < fetch_time + 1; fetch_index++) {
                        ...
                        if (skip) {
                            ...
                            dma_info_t dma_info(fetch_offset / DMA_WORD_PER_BEAT, fetch_len / DMA_WORD_PER_BEAT,
                                                DMA_SIZE);
                                        ...
                                        if (fetch_ping) {
                                            if (i == 0 && k == 0)
                                                plm_fetch_outdata_ping[i] =
                                                    data_Bv.range(2 * DATA_WIDTH - 1, DATA_WIDTH).to_int64();
                                            else if (i != 0)
                                                plm_fetch_outdata_ping[i + k - 1] =
                                                    data_Bv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH).to_int64();
                                        } else {
        ...
        for (uint16_t b = 0; b < 1; b++) {
            ...
            for (int in_rem = in_length; in_rem > 0; in_rem -= rows) {
                ...
                for (int j = 0; j < i + 1; j++) {
                    ...
                    plm_in_data =
                        ping ? int2fp<FPDATA, WORD_SIZE>(plm_in_ping[j]) : int2fp<FPDATA, WORD_SIZE>(plm_in_pong[j]);
