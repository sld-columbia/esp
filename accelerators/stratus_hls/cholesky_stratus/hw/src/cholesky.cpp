                    ...
                    if (!even_rows) {
                        for (uint16_t k = 0; k < DMA_WORD_PER_BEAT; k++) {
                            ...
                            if (ping) {
                                ...
                                if (i == 0 && k == 0)
                                    plm_in_pong[i] =
                                        dataBv.range(2 * DATA_WIDTH - 1, DATA_WIDTH).to_int64();
                                else if (i != 0)
                                    plm_in_pong[i + k - 1] =
                                        dataBv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH)
                                            .to_int64();
                ...
                if (start_fetching_output >= start_fetching_output_thresh) {
                    ...
                    for (int fetch_index = 0; fetch_index < fetch_time + 1; fetch_index++) {
                        ...
                        if (skip) {
                            ...
                            fetch_len =
                                (DMA_WORD_PER_BEAT == 2 && !even_rows) ? fetch_len + 1 : fetch_len;

                            dma_info_t dma_info(fetch_offset / DMA_WORD_PER_BEAT,
                                                fetch_len / DMA_WORD_PER_BEAT, DMA_SIZE, 0);
                            fetch_offset += (DMA_WORD_PER_BEAT == 2 && !even_rows) ?
                                (fetch_ping ? fetch_len : fetch_len - 2) :
                                fetch_len;
                            this->dma_read_ctrl.put(dma_info);
                            ...
                            for (uint16_t i = 0; i < fetch_len; i += DMA_WORD_PER_BEAT) {
                                ...
                                if (!even_rows) {
                                    for (uint16_t k = 0; k < DMA_WORD_PER_BEAT; k++) {
                                        ...
                                        if (fetch_ping) {
                                            if (i == 0 && k == 0)
                                                plm_fetch_outdata_ping[i] =
                                                    data_Bv.range(2 * DATA_WIDTH - 1, DATA_WIDTH)
                                                        .to_int64();
                                            else if (i != 0)
                                                plm_fetch_outdata_ping[i + k - 1] =
                                                    data_Bv
                                                        .range((k + 1) * DATA_WIDTH - 1,
                                                               k * DATA_WIDTH)
                                                        .to_int64();
                                        }
                                        else {
                                            if (i == (fetch_len - 2) && k == 0)
                                                plm_fetch_outdata_pong[i] =
                                                    data_Bv.range(DATA_WIDTH - 1, 0).to_int64();
                                            else if (i != (fetch_len - 2))
                                                plm_fetch_outdata_pong[i + k] =
                                                    data_Bv
                                                        .range((k + 1) * DATA_WIDTH - 1,
                                                               k * DATA_WIDTH)
                                                        .to_int64();
                                    ...
                                    for (uint16_t k = 0; k < DMA_WORD_PER_BEAT; k++) {
                                        HLS_UNROLL_SIMPLE;
                                        if (fetch_ping)
                                            plm_fetch_outdata_ping[i + k] =
                                                data_Bv
                                                    .range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH)
                                                    .to_int64();
                                        else
                                            plm_fetch_outdata_pong[i + k] =
                                                data_Bv
                                ...
                                for (uint16_t k = 0; k < DMA_WORD_PER_BEAT; k++) {
                                    ...
                                    if (fetch_ping)
                                        plm_fetch_outdata_ping[i + k] =
                                            data_Bv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH)
                                                .to_int64();
                                    else
                                        plm_fetch_outdata_pong[i + k] =
                                            data_Bv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH)
                    ...
                    if (!even_rows) {
                        for (uint16_t k = 0; k < DMA_WORD_PER_BEAT; k++) {
                            ...
                            if (ping) {
                                ...
                                if (i == 0) {
                                    if (k == 0) dataBv.range(DATA_WIDTH - 1, 0) = dma_rem_data;
                                    else
                                        dataBv.range(2 * DATA_WIDTH - 1, DATA_WIDTH) =
                                            plm_out_pong[i];
                                }
                                else
                                    dataBv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH) =
    ...
    uint32_t start_fetching_output_thresh = 3;
    {
        for (uint16_t b = 0; b < 1; b++) {
            ...
            for (int in_rem = in_length; in_rem > 0; in_rem -= rows) {
                ...
                if (i < start_fetching_output_thresh) this->compute_load_handshake();
                ...
                for (int j = 0; j < i + 1; j++) {
                    ...
                    plm_in_data = ping ? int2fp<FPDATA, WORD_SIZE>(plm_in_ping[j]) :
                                         int2fp<FPDATA, WORD_SIZE>(plm_in_pong[j]);
                    ...
                    for (int k = 0; k < j; k++) {
                        ...
                        if (i >= start_fetching_output_thresh) {
                            if (j < (i - 1)) {
                                if (fetch_out_ping)
                                    plm_fetch_data =
                                        int2fp<FPDATA, WORD_SIZE>(plm_fetch_outdata_ping[k]);
                                else
                                    plm_fetch_data =
                                        int2fp<FPDATA, WORD_SIZE>(plm_fetch_outdata_pong[k]);
                            ...
                            if (i != j) sum += plm_out_data * plm_temp_data;
                            else
                                sum += plm_out_data * plm_out_data;
                        }
                    } // for(k)
                    ...
                    if (i == j) {
                        ...
                        if (ping) plm_out_ping[j] = index_sqrt_data;
                        else
                        ...
                        if (plm_diag_data != 0) {
                            output_data = fp2int<FPDATA, WORD_SIZE>((1.0 / plm_diag_data) *
                                                                    (plm_in_data - sum));
