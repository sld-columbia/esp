...
void system_t::load_memory() {
  ...
  for (int i = 0; i < out_size; i++) {
    ...
    gold_bv.range(DATA_WIDTH - 1, 0) =
        fp2bv<FPDATA, WORD_SIZE>(FPDATA(gold[i]));
  ...
  for (int i = 0; i < in_size / DMA_WORD_PER_BEAT; i++) {
    ...
    for (int j = 0; j < DMA_WORD_PER_BEAT; j++) {
      ...
      // cout << "INPUT DATA FPDATA" <<  (FPDATA( in[i * DMA_WORD_PER_BEAT +
...
int system_t::validate() {
  ...
  for (int i = 0; i < 1; i++) {
    for (int j = 0; j < rows * rows; j++) {
      ...
      if ((fabs(gold[i * out_words_adj + j] - out[i * out_words_adj + j]) /
           fabs(gold[i * out_words_adj + j])) > ERR_TH) {
        errors++;
