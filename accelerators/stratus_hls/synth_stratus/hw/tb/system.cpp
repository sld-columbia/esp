...
void system_t::report_latency()
{
    ...
    do {
        if (load.nb_can_get()) { load.nb_get(load_exec); }
        if (compute.nb_can_get()) { compute.nb_get(compute_exec); }
        if (store.nb_can_get()) { store.nb_get(store_exec); }
        fout << load_exec << ",";
        fout << compute_exec << ",";
        fout << store_exec << "\n";
        wait();
...
int system_t::validate_acc_id(int acc_id)
{
    ...
    if (!configs[acc_id].in_place) offset = configs[acc_id].in_size;
