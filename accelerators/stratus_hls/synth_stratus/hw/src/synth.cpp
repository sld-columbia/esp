...
void synth::latency_collector()
{
    ...
    while (true) {
        ...
        while (!load.nb_can_put())
            wait();
        load.nb_put(load_exec);

        while (!compute.nb_can_put())
            wait();
        compute.nb_put(compute_exec);

        while (!store.nb_can_put())
            wait();
...
void synth::load_input()
{
    ...
    // Config
    {
        ...
        if (config.pattern == IRREGULAR) nwords = in_size >> access_factor;
        else
            nwords = in_size;
        ...
        if (pattern == IRREGULAR) { in_place = 0; }

        if (ld_st_ratio > 1 && in_place == 1) reuse_factor = 1;
    }
        ...
        for (uint32_t r = 0; r < reuse_factor; r++) {
            ...
            for (uint32_t b = 0; b < ntrans; b++) {
...

                {
                    ...
                    dma_info_t dma_info(index + offset, burst_len, SIZE_WORD, 0);
                ...
                for (uint32_t i = 0; i < burst_len; i++) {
                    HLS_LOAD_INPUT_LOOP;
                    {
                        ...
                        // validate read data
                        if ((!in_place || r == 0) && data != rd_data) { rd_err = data; }
                ...
                // Index calculation
                if (pattern == STREAMING) { index += burst_len; }
        ...
        for (uint32_t r = 0; r < reuse_factor; r++) {
            ...
            for (uint32_t b = 0; b < ntrans; b++) {
                ...
                for (uint32_t i = 0; i < burst_len; i++) {
                    ...
                    {
                        HLS_STORE_DMA;
                        if (r == reuse_factor - 1 && b == ntrans - 1 && i == burst_len - 1 &&
                            rd_err > 0) {
