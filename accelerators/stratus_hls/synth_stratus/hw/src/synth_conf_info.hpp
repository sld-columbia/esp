...
class conf_info_t
{
  public:
    ...
    conf_info_t(uint32_t in_size, uint32_t out_size, uint32_t access_factor, uint32_t burst_len,
                uint32_t compute_bound_factor, uint32_t reuse_factor, uint32_t in_place, uint32_t pattern,
                uint32_t irregular_seed, uint32_t ld_st_ratio, uint32_t stride_len, uint32_t offset, uint32_t wr_data,
                uint32_t rd_data)
        : in_size(in_size)
        , out_size(out_size)
    ...
    {
        return (rhs.in_size == in_size) && (rhs.out_size == out_size) && (rhs.access_factor == access_factor) &&
               (rhs.burst_len == burst_len) && (rhs.compute_bound_factor == compute_bound_factor) &&
               (rhs.reuse_factor == reuse_factor) && (rhs.in_place == in_place) && (rhs.pattern == pattern) &&
               (rhs.irregular_seed == irregular_seed) && (rhs.ld_st_ratio == ld_st_ratio) &&
               (rhs.stride_len == stride_len) && (rhs.offset == offset) && (rhs.wr_data == wr_data) &&
               (rhs.rd_data == rd_data);
    }
    ...
    friend ostream &operator<<(ostream &os, conf_info_t const &conf_info)
    {
        os << "{ in_size = " << conf_info.in_size << ", out_size = " << conf_info.out_size
           << ", access_factor = " << conf_info.access_factor << ", burst_len = " << conf_info.burst_len
           << ", compute_bound_factor = " << conf_info.compute_bound_factor
           << ", reuse_factor = " << conf_info.reuse_factor << ", in_place = " << conf_info.in_place
           << ", pattern = " << conf_info.pattern << ", irregular_seed = " << conf_info.irregular_seed
           << ", ld_st_ratio = " << conf_info.ld_st_ratio << ", stride_len = " << conf_info.stride_len
           << ", offset = " << conf_info.offset << ", wr_data = " << conf_info.wr_data
