...
char *devnames[] = {
    "synth_stratus.0",  "synth_stratus.1",  "synth_stratus.2",
    "synth_stratus.3",  "synth_stratus.4",  "synth_stratus.5",
    "synth_stratus.6",  "synth_stratus.7",  "synth_stratus.8",
    "synth_stratus.9",  "synth_stratus.10", "synth_stratus.11",
    "synth_stratus.12", "synth_stratus.13", "synth_stratus.14",
    "synth_stratus.15",
};
...
static void read_soc_config(FILE *f, soc_config_t *soc_config) {
  ...
  soc_config->ddr_hops =
      malloc(sizeof(int) * soc_config->nmem * soc_config->nsynth);
...
static void config_threads(FILE *f, accelerator_thread_info_t **thread_info,
                           ...
                           struct synth_stratus_access ***synth_cfg, int phase,
                           int *nthreads, int coherence_mode,
  ...
  *synth_cfg = malloc(sizeof(struct synth_stratus_access *) * *nthreads);
  ...
  for (int t = 0; t < *nthreads; t++) {
    ...
    (*synth_cfg)[t] =
        malloc(sizeof(struct synth_stratus_access) * thread_info[t]->ndev);
    ...
    for (int d = 0; d < thread_info[t]->ndev; d++) {
      ...
      fscanf(f, "%d %d %d %d %d %d %d %d %d %s",
             &(*synth_cfg)[t][d].access_factor, &(*synth_cfg)[t][d].burst_len,
             ...
             &(*synth_cfg)[t][d].wr_data, &(*synth_cfg)[t][d].rd_data,
             coh_choice);
...
static void alloc_phase(accelerator_thread_info_t **thread_info,
                        esp_thread_info_t ***cfg,
                        struct synth_stratus_access ***synth_cfg, int nthreads,
                        soc_config_t soc_config, int alloc_mode,
                        enum alloc_effort alloc, uint32_t **buffers,
                        int phase) {
  ...
  for (int i = 0; i < nthreads; i++) {
    ...
    if (buffers[i] == NULL) {
      die_errno("error: cannot allocate %zu contig bytes",
                thread_info[i]->memsz);
    }
...
static void free_phase(accelerator_thread_info_t **thread_info,
                       esp_thread_info_t **cfg,
                       struct synth_stratus_access **synth_cfg, int nthreads) {
  for (int i = 0; i < nthreads; i++) {
...
static void dump_results(FILE *out_file,
                         accelerator_thread_info_t **thread_info,
                         esp_thread_info_t **cfg,
                         struct synth_stratus_access **synth_cfg,
                         soc_config_t *soc_config, int phase, int nthreads,
                         char **argv, int test_no) {
  int t, d;
...
int main(int argc, char **argv) {
  ...
  if (access(out_name, F_OK) != -1) {
    ...
    fprintf(out_file, "Device/Thread/Phase name, devID/nacc/nthreads, "
                      "coherence, allocation, footprint, DDR#, time\n");
  }
  ...
  if (!strcmp(argv[2], "none")) {
    ...
    printf("Valid coherence choices include none, llc, recall, full, auto, or "
           "cfg\n");
    return 1;
  }
  ...
  if (!strcmp(argv[3], "preferred")) {
    ...
    printf("Valid alloc choices include preferred, lloaded, balanced, auto, "
           "and cfg\n");
    return 1;
  }
  ...
  for (int p = 0; p < nphases; p++) {
    config_threads(f, thread_info[p], &cfg, &synth_cfg, p, &nthreads,
                   coherence_mode, coherence, &nacc);
    alloc_phase(thread_info[p], &cfg, &synth_cfg, nthreads, *soc_config,
                alloc_mode, alloc, buffers, p);
    ...
    dump_results(out_file, thread_info[p], cfg, synth_cfg, soc_config, p,
                 nthreads, argv, test_no);
