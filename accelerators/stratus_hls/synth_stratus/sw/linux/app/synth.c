    ...
    for (int s = 0; s < soc_config->nsynth; s++) {
        for (int m = 0; m < soc_config->nmem; m++) {
            soc_config->ddr_hops[s * soc_config->nmem + m] =
                abs(soc_config->synth_y[s] - soc_config->mem_y[m]) + abs(soc_config->synth_x[s] - soc_config->mem_x[m]);
        }
    }
}

static void config_threads(FILE *f, accelerator_thread_info_t **thread_info, esp_thread_info_t ***cfg,
                           struct synth_stratus_access ***synth_cfg, int phase, int *nthreads, int coherence_mode,
    ...
    *synth_cfg = malloc(sizeof(struct synth_stratus_access *) * *nthreads);
    ...
    for (int t = 0; t < *nthreads; t++) {
        ...
        for (int d = 0; d < thread_info[t]->ndev; d++) {
            ...
            fscanf(f, "%d %d %d %d %d %d %d %d %d %s", &(*synth_cfg)[t][d].access_factor, &(*synth_cfg)[t][d].burst_len,
                   &(*synth_cfg)[t][d].compute_bound_factor, &(*synth_cfg)[t][d].reuse_factor,
                   &(*synth_cfg)[t][d].ld_st_ratio, &(*synth_cfg)[t][d].stride_len, &(*synth_cfg)[t][d].in_place,
            ...
            unsigned int out_size      = (in_size >> (*synth_cfg)[t][d].access_factor) / (*synth_cfg)[t][d].ld_st_ratio;
...
static void alloc_phase(accelerator_thread_info_t **thread_info, esp_thread_info_t ***cfg,
                        struct synth_stratus_access ***synth_cfg, int nthreads, soc_config_t soc_config, int alloc_mode,
                        enum alloc_effort alloc, uint32_t **buffers, int phase)
{
...
static int validate_buffer(accelerator_thread_info_t *thread_info, struct synth_stratus_access **synth_cfg,
                           uint32_t *buf)
{
    int errors = 0;
...
static void dump_results(FILE *out_file, accelerator_thread_info_t **thread_info, esp_thread_info_t **cfg,
                         struct synth_stratus_access **synth_cfg, soc_config_t *soc_config, int phase, int nthreads,
...
int main(int argc, char **argv)
{
    ...
    if (access(out_name, F_OK) != -1) {
        ...
        fprintf(out_file,
                "Device/Thread/Phase name, devID/nacc/nthreads, coherence, allocation, footprint, DDR#, time\n");
    }
