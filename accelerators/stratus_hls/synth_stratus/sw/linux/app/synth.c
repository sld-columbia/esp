...
{
    if (!strncmp(size, "M8", 2)) { return 8388608; }
...
char *devnames[] = {
    "synth_stratus.0",  "synth_stratus.1",  "synth_stratus.2",  "synth_stratus.3",
    "synth_stratus.4",  "synth_stratus.5",  "synth_stratus.6",  "synth_stratus.7",
    "synth_stratus.8",  "synth_stratus.9",  "synth_stratus.10", "synth_stratus.11",
    "synth_stratus.12", "synth_stratus.13", "synth_stratus.14", "synth_stratus.15",
};
...
static void read_soc_config(FILE *f, soc_config_t *soc_config)
{
    ...
    for (int s = 0; s < soc_config->nsynth; s++) {
        for (int m = 0; m < soc_config->nmem; m++) {
            soc_config->ddr_hops[s * soc_config->nmem + m] =
                abs(soc_config->synth_y[s] - soc_config->mem_y[m]) +
                abs(soc_config->synth_x[s] - soc_config->mem_x[m]);
        }
    }
}

static void config_threads(FILE *f, accelerator_thread_info_t **thread_info,
                           esp_thread_info_t ***cfg, struct synth_stratus_access ***synth_cfg,
                           int phase, int *nthreads, int coherence_mode,
                           enum accelerator_coherence coherence, unsigned **nacc)
{
    fscanf(f, "%d", nthreads);
    ...
    *synth_cfg = malloc(sizeof(struct synth_stratus_access *) * *nthreads);
    ...
    for (int t = 0; t < *nthreads; t++) {
        ...
        if (!strcmp(flow_choice, "p2p")) { thread_info[t]->p2p = 1; }
        else {
            thread_info[t]->p2p = 0;
        }
        ...
        for (int d = 0; d < thread_info[t]->ndev; d++) {
            ...
            fscanf(f, "%d %d %d %d %d %d %d %d %d %s", &(*synth_cfg)[t][d].access_factor,
                   &(*synth_cfg)[t][d].burst_len, &(*synth_cfg)[t][d].compute_bound_factor,
                   &(*synth_cfg)[t][d].reuse_factor, &(*synth_cfg)[t][d].ld_st_ratio,
                   &(*synth_cfg)[t][d].stride_len, &(*synth_cfg)[t][d].in_place,
                   &(*synth_cfg)[t][d].wr_data, &(*synth_cfg)[t][d].rd_data, coh_choice);
            ...
            unsigned int out_size =
                (in_size >> (*synth_cfg)[t][d].access_factor) / (*synth_cfg)[t][d].ld_st_ratio;
            ...
            if (!(*synth_cfg)[t][d].in_place &&
                (!thread_info[t]->p2p || d == thread_info[t]->ndev - 1))
                footprint += out_size;

            if (thread_info[t]->p2p) { (*synth_cfg)[t][d].esp.coherence = ACC_COH_NONE; }
...
static void alloc_phase(accelerator_thread_info_t **thread_info, esp_thread_info_t ***cfg,
                        struct synth_stratus_access ***synth_cfg, int nthreads,
                        soc_config_t soc_config, int alloc_mode, enum alloc_effort alloc,
                        uint32_t **buffers, int phase)
{
    ...
    for (int t = 0; t < nthreads; t++) {
        ...
        for (int d = 0; d < thread_info[t]->ndev; d++) {
            for (int m = 0; m < soc_config.nmem; m++) {
                ddr_node_cost[m] +=
                    soc_config.ddr_hops[thread_info[t]->chain[d] * soc_config.nmem + m];
            }
        }
    ...
    for (int i = 0; i < nthreads; i++) {
        ...
        if (alloc_mode == CFG) { params.policy = thread_info[i]->alloc_choice; }
...
static int validate_buffer(accelerator_thread_info_t *thread_info,
                           struct synth_stratus_access **synth_cfg, uint32_t *buf)
{
    int errors = 0;
    for (int i = 0; i < thread_info->ndev; i++) {
        if (thread_info->p2p && i != thread_info->ndev - 1) continue;
        ...
        if (i != thread_info->ndev - 1) {
            ...
            if (next_in_place) continue;
        }

        if (!in_place && !thread_info->p2p) offset += in_size;
...
static void free_phase(accelerator_thread_info_t **thread_info, esp_thread_info_t **cfg,
                       struct synth_stratus_access **synth_cfg, int nthreads)
{
    for (int i = 0; i < nthreads; i++) {
...
static void dump_results(FILE *out_file, accelerator_thread_info_t **thread_info,
                         esp_thread_info_t **cfg, struct synth_stratus_access **synth_cfg,
                         soc_config_t *soc_config, int phase, int nthreads, char **argv,
                         int test_no)
{
    int t, d;
    ...
    for (t = 0; t < nthreads; t++) {
        ...
        for (d = 0; d < thread_info[t]->ndev; d++) {
            ...
            if (synth_cfg[t][d].esp.coherence == ACC_COH_FULL) fprintf(out_file, "full,");
        ...
        if (thread_info[t]->alloc_choice == CONTIG_ALLOC_BALANCED) fprintf(out_file, "balanced,");
        else if (thread_info[t]->alloc_choice == CONTIG_ALLOC_PREFERRED)
            fprintf(out_file, "preferred,");
        else if (thread_info[t]->alloc_choice == CONTIG_ALLOC_LEAST_LOADED)
            fprintf(out_file, "lloaded,");
...
int main(int argc, char **argv)
{
    ...
    if (argc != 4) { printf("Usage: ./synth.exe file coherence alloc\n"); }
    // command line args
    FILE *f;
    f = fopen(argv[1], "r");
    ...
    FILE *out_file;
    if (access(out_name, F_OK) != -1) { out_file = fopen(out_name, "a"); }
    else {
        out_file = fopen(out_name, "w");
        fprintf(out_file,
                "Device/Thread/Phase name, devID/nacc/nthreads, coherence, allocation, footprint, "
                "DDR#, time\n");
    }
    ...
    for (int p = 0; p < nphases; p++) {
        config_threads(f, thread_info[p], &cfg, &synth_cfg, p, &nthreads, coherence_mode, coherence,
                       &nacc);
        alloc_phase(thread_info[p], &cfg, &synth_cfg, nthreads, *soc_config, alloc_mode, alloc,
                    buffers, p);
        ...
        for (int t = 0; t < nthreads; t++) {
            ...
            if (errors) printf("[FAIL] Thread %d.%d : %u errors\n", p, t, errors);
            else
        ...
        dump_results(out_file, thread_info[p], cfg, synth_cfg, soc_config, p, nthreads, argv,
                     test_no);
