...
static int vitdodec_probe(struct platform_device *pdev)
{
    ...
    if (vitdodec == NULL) return -ENOMEM;
    esp         = &vitdodec->esp;
...
static int __init vitdodec_init(void) { return esp_driver_register(&vitdodec_driver); }

static void __exit vitdodec_exit(void) { esp_driver_unregister(&vitdodec_driver); }

module_init(vitdodec_init) module_exit(vitdodec_exit)
