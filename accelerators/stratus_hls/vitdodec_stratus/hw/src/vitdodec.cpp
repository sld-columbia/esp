  ...
  // Load
  {
    ...
    for (uint16_t b = 0; b < 1; b++) {
      ...
      for (int rem = length; rem > 0; rem -= PLM_IN_WORD) {
        ...
        dma_info_t dma_info(offset * DMA_BEAT_PER_WORD, len * DMA_BEAT_PER_WORD,
                            DMA_SIZE, 0);
#else
        dma_info_t dma_info(offset / DMA_WORD_PER_BEAT, len / DMA_WORD_PER_BEAT,
                            DMA_SIZE, 0);
#endif
        offset += len;
          ...
          for (uint16_t k = 0; k < DMA_WORD_PER_BEAT; k++) {
            ...
            plm_in_ping[i + k] =
                dataBv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH)
                    .to_int64();
            // else
            //    plm_in_pong[i + k] = dataBv.range((k+1) * DATA_WIDTH - 1, k *
            //    DATA_WIDTH).to_int64();
          }
        }
#if (0)
    ...
    for (uint16_t b = 0; b < 1; b++) {
      ...
      // for (int in_rem = in_length; in_rem > 0; in_rem -= PLM_IN_WORD)
      {
...

        {
          ...
          //    cbps                  : INPUT  :     X  : int = 4 bytes
          //    (REGISTER) ntraceback            : INPUT  :     X  : int = 4
          //    bytes (REGISTER) data_bits             : INPUT  :     X  : int =
          //    4 bytes (REGISTER) d_branchtab27_generic : INPUT  :     0  :
          //    uint8_t[2][32] = 64 bytes in_depuncture_pattern : INPUT  :    64
          //    : uint8_t[8] (max is 6 bytes + 2 padding bytes) depd_data :
          //    INPUT  :    72  : uint8_t[MAX_ENCODED_BITS == 24780]
          //    (depunctured data) <return_val>          : OUTPUT :     0  :
          //    uint8_t[MAX_ENCODED_BITS * 3 / 4 == 18585 ] : The decoded data
          //    stream
          ...
          //    l_store_pos           : INPUT  : int (position in circular
          //    traceback buffer?) l_mmresult            : OUTPUT : uint8_t[64]
          //    l_ppresult            : OUTPUT : uint8_t[ntraceback_MAX][ 64
          //    bytes ]
          ...
          //  Do this before the real operation so local memories are "cleared
          //  to zero"
          ...
          while (n_decoded < data_bits) {
            ...
            if ((in_count % 4) == 0) { // 0 or 3
               ...
               * operation because of the way it looks on a trellis diagram.
               *Each butterfly involves an Add-Compare-Select (ACS) operation on
               *the two nodes where the 0 and 1 paths from the current node
               *merge at the next step of the trellis.
               *
               * The code polynomials are assumed to have 1's on both ends.
               *Given a function encode_state() that returns the two symbols for
               *a given encoder state in the low two bits, such a code will have
               *the following identities for even 'n' < 64:
               *
               * 	encode_state(n) = encode_state(n+65)
               *	encode_state(n+1) = encode_state(n+64) = (3 ^
               *encode_state(n))
               *
               * Any convolutional code you would actually want to use will have
               * these properties, so these assumptions aren't too limiting.
               *
               * Doing this as a macro lets the compiler evaluate at compile
               *time the many expressions that depend on the loop index and
               *encoder state and emit them as immediate arguments. This makes
               *an enormous difference on register-starved machines such as the
               *Intel x86 family where evaluating these expressions at runtime
               * would spill over into memory.
               */

              // INPUTS/OUTPUTS:  All are 64-entry (bytes) arrays randomly
              // accessed.
              ...
              /* 				 unsigned char *mm0, unsigned
               * char *mm1, */
              /* 				 unsigned char *pp0, unsigned
               * char *pp1) */
              {
                ...
                unsigned char
                    symbols[4]; // = (unsigned char)plm_in_ping[(idx_depd_data +
                                // in_count) & 0xfffffffc];
                HLS_FLAT(symbols);

                symbols[0] = (unsigned char)
                    plm_in_ping[((idx_depd_data + in_count) & 0xfffffffc) + 0];
                symbols[1] = (unsigned char)
                    plm_in_ping[((idx_depd_data + in_count) & 0xfffffffc) + 1];
                symbols[2] = (unsigned char)
                    plm_in_ping[((idx_depd_data + in_count) & 0xfffffffc) + 2];
                symbols[3] = (unsigned char)
                    plm_in_ping[((idx_depd_data + in_count) & 0xfffffffc) + 3];

                // These are used to "virtually" rename the uses below (for
                // symmetry; reduces code size)
                //  Really these are functionally "offset pointers" into the
                //  above arrays....
                ...
                for (int j = 0; j < 16; j++) {
                  ...
                  // sym0v[j] = (unsigned char)plm_in_ping[((idx_depd_data +
                  // in_count) & 0xfffffffc) + first_symbol]; sym1v[j] =
                  // (unsigned char)plm_in_ping[((idx_depd_data + in_count) &
                  // 0xfffffffc) + second_symbol];
                }

                for (int s = 0; s < 2;
                     s++) { // iterate across the 2 symbol groups
                  ...
                  for (int i = 0; i < 2; i++) {
                    ...
                    if (symbols[first_symbol] == 2) {
                      for (int j = 0; j < 16; j++) {
                        // metsvm[j] = d_branchtab27_generic[1].c[(i*16) + j] ^
                        // sym1v[j]; metsvm[j] = d_brtab27[1][(i*16) + j] ^
                        // sym1v[j];
                        metsvm[j] =
                            (unsigned char)
                                plm_in_ping[idx_brtab27_1 + (i * 16) + j] ^
                            sym1v[j];
                        metsv[j] = 1 - metsvm[j];
                      }
                    } else if (symbols[second_symbol] == 2) {
                      for (int j = 0; j < 16; j++) {
                        // metsvm[j] = d_branchtab27_generic[0].c[(i*16) + j] ^
                        // sym0v[j]; metsvm[j] = d_brtab27[0][(i*16) + j] ^
                        // sym0v[j]; metsvm[j] = d_brtab27_0[(i*16) + j] ^
                        // sym0v[j];
                        metsvm[j] =
                            (unsigned char)
                                plm_in_ping[idx_brtab27_0 + (i * 16) + j] ^
                            sym0v[j];
                        metsv[j] = 1 - metsvm[j];
                      }
                    } else {
                      for (int j = 0; j < 16; j++) {
                        // metsvm[j] = (d_branchtab27_generic[0].c[(i*16) + j] ^
                        // sym0v[j]) + (d_branchtab27_generic[1].c[(i*16) + j] ^
                        // sym1v[j]); metsvm[j] = (d_brtab27[0][(i*16) + j] ^
                        // sym0v[j]) + (d_brtab27[1][(i*16) + j] ^ sym1v[j]);
                        metsvm[j] =
                            ((unsigned char)
                                 plm_in_ping[idx_brtab27_0 + (i * 16) + j] ^
                             sym0v[j]) +
                    ...
                    for (int j = 0; j < 16; j++) {
                      ...
                      survivor0[j] =
                          (decision0[j] & m0[j]) | ((~decision0[j]) & m1[j]);
                      survivor1[j] =
                          (decision1[j] & m2[j]) | ((~decision1[j]) & m3[j]);
                    }
              ...
              viterbi_butterfly_calls++; // Do not increment until after the
                                         // comparison code.
              ...
              if ((in_count > 0) && (in_count % 16) == 8) { // 8 or 11
                ...
                //    ntraceback  : INPUT         : int (I think effectively
                //    const for given run type; here 5 I think) outbuf      :
                //    OUTPUT        : 1 byte l_store_pos : GLOBAL IN/OUT : int
                //    (position in circular traceback buffer?)

                //    l_mmresult  : GLOBAL OUTPUT : Array [ 64 bytes ]
                //    l_ppresult  : GLOBAL OUTPUT : Array [ntraceback][ 64 bytes
                //    ]

                // CALL : viterbi_get_output_generic(l_metric0_generic,
                // l_path0_generic, ntraceback, &c); unsigned char
                // viterbi_get_output_generic(unsigned char *mm0, unsigned char
                // *pp0, int ntraceback, unsigned char *outbuf)
                {
                  unsigned char *mm0 = l_metric0_generic;
                  unsigned char *pp0 = l_path0_generic;
                ...
                if (out_count >= ntraceback) {
                  for (int i = 0; i < 8; i++) {
                    // l_decoded[(out_count - ntraceback) * 8 + i] = (c >> (7 -
                    // i)) & 0x1;
                    plm_out_ping[(out_count - ntraceback) * 8 + i] =
                        (c >> (7 - i)) & 0x1;
                    // if (n_decoded < 16) {
                    //     printf("plm_out_ping[%2u] written as %u = %u\n",
                    //     (out_count - ntraceback) * 8 + i, (c >> (7 - i)) &
                    //     0x1, (unsigned char)plm_out_ping[(out_count -
                    //     ntraceback) * 8 + i]);
                    // }
                    n_decoded++;
                  }
