        ...
        for (uint16_t b = 0; b < 1; b++) {
            ...
            // for (int in_rem = in_length; in_rem > 0; in_rem -= PLM_IN_WORD)
            {
...

                {
                    ...
                    //    depd_data             : INPUT  :    72  : uint8_t[MAX_ENCODED_BITS == 24780] (depunctured
                    //    data) <return_val>          : OUTPUT :     0  : uint8_t[MAX_ENCODED_BITS * 3 / 4 == 18585 ] :
                    ...
                    while (n_decoded < data_bits) {
                        ...
                        if ((in_count % 4) == 0) { // 0 or 3
                            ...
                            /* 				 unsigned char *pp0, unsigned char *pp1) */
                            {
                                ...
                                unsigned char  symbols[4]; // = (unsigned char)plm_in_ping[(idx_depd_data + in_count) &
                                                           // 0xfffffffc];
                                HLS_FLAT(symbols);
                                ...
                                for (int j = 0; j < 16; j++) {
                                    ...
                                    // sym0v[j] = (unsigned char)plm_in_ping[((idx_depd_data + in_count) & 0xfffffffc) +
                                    // first_symbol]; sym1v[j] = (unsigned char)plm_in_ping[((idx_depd_data + in_count) &
                                    // 0xfffffffc) + second_symbol];
                                }

                                for (int s = 0; s < 2; s++) { // iterate across the 2 symbol groups
                                    // This is the basic viterbi butterfly for 2 symbols (we need therefore 2 passes for
                                    // 4 total symbols)
                                    // printf("for s = %u :\n", s);
                                    for (int i = 0; i < 2; i++) {
                                        // printf("  for i = %u :\n", i);
                                        if (symbols[first_symbol] == 2) {
                                            ...
                                            for (int j = 0; j < 16; j++) {
                                                ...
                                                metsvm[j] =
                                                    (unsigned char)plm_in_ping[idx_brtab27_0 + (i * 16) + j] ^ sym0v[j];
                                                metsv[j] = 1 - metsvm[j];
                                            }
                                        } else {
                                            for (int j = 0; j < 16; j++) {
                                                // metsvm[j] = (d_branchtab27_generic[0].c[(i*16) + j] ^ sym0v[j]) +
                                                // (d_branchtab27_generic[1].c[(i*16) + j] ^ sym1v[j]); metsvm[j] =
                                                // (d_brtab27[0][(i*16) + j] ^ sym0v[j]) + (d_brtab27[1][(i*16) + j] ^
                                                // sym1v[j]);
                                                metsvm[j] = ((unsigned char)plm_in_ping[idx_brtab27_0 + (i * 16) + j] ^
                                                             sym0v[j]) +
                                                            ((unsigned char)plm_in_ping[idx_brtab27_1 + (i * 16) + j] ^
                                                             sym1v[j]);
                                                metsv[j] = 2 - metsvm[j];
                                            }
                                        }
                            ...
                            if ((in_count > 0) && (in_count % 16) == 8) { // 8 or 11
                                ...
                                //    ntraceback  : INPUT         : int (I think effectively const for given run type;
                                //    here 5 I think) outbuf      : OUTPUT        : 1 byte l_store_pos : GLOBAL IN/OUT :
                                //    int (position in circular traceback buffer?)

                                //    l_mmresult  : GLOBAL OUTPUT : Array [ 64 bytes ]
                                //    l_ppresult  : GLOBAL OUTPUT : Array [ntraceback][ 64 bytes ]

                                // CALL : viterbi_get_output_generic(l_metric0_generic, l_path0_generic, ntraceback,
                                // &c); unsigned char viterbi_get_output_generic(unsigned char *mm0, unsigned char *pp0,
                                ...
                                if (out_count >= ntraceback) {
                                    for (int i = 0; i < 8; i++) {
                                        ...
                                        //     * 8 + i, (c >> (7 - i)) & 0x1, (unsigned char)plm_out_ping[(out_count -
                                        //     ntraceback) * 8 + i]);
                                        // }
                                        n_decoded++;
                                    }
