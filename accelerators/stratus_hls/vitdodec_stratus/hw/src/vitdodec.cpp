                ...
                for (uint16_t i = 0; i < len; i += DMA_WORD_PER_BEAT) {
                    ...
                    for (uint16_t k = 0; k < DMA_WORD_PER_BEAT; k++) {
                        ...
                        dataBv.range((k + 1) * DATA_WIDTH - 1, k * DATA_WIDTH) =
                            plm_out_ping[i + k];
                        // else
                        //    dataBv.range((k+1) * DATA_WIDTH - 1, k * DATA_WIDTH) = plm_out_pong[i
                        //    + k];
                    }
        ...
        for (uint16_t b = 0; b < 1; b++) {
            ...
            // for (int in_rem = in_length; in_rem > 0; in_rem -= PLM_IN_WORD)
            {
...

                {
                    ...
                    //    in_depuncture_pattern : INPUT  :    64  : uint8_t[8] (max is 6 bytes + 2
                    //    padding bytes) depd_data             : INPUT  :    72  :
                    //    uint8_t[MAX_ENCODED_BITS == 24780] (depunctured data) <return_val> :
                    //    OUTPUT :     0  : uint8_t[MAX_ENCODED_BITS * 3 / 4 == 18585 ] : The
                    //    decoded data stream
                    ...
                    //    l_store_pos           : INPUT  : int (position in circular traceback
                    //    buffer?) l_mmresult            : OUTPUT : uint8_t[64] l_ppresult : OUTPUT
                    //    : uint8_t[ntraceback_MAX][ 64 bytes ]
                    ...
                    while (n_decoded < data_bits) {
                        ...
                        if ((in_count % 4) == 0) { // 0 or 3
                             ...
                             * encoder state in the low two bits, such a code will have the
                             *following identities for even 'n' < 64:
                             *
                            ...
                            /* 				 unsigned char *pp0, unsigned char *pp1) */
                            {
                                ...
                                for (int j = 0; j < 16; j++) {
                                    ...
                                    // in_count) & 0xfffffffc) + first_symbol]; sym1v[j] = (unsigned
                                    // char)plm_in_ping[((idx_depd_data + in_count) & 0xfffffffc) +
                                    // second_symbol];
                                }
                                ...
                                for (int s = 0; s < 2; s++) { // iterate across the 2 symbol groups
                                    ...
                                    for (int i = 0; i < 2; i++) {
                                        ...
                                        if (symbols[first_symbol] == 2) {
                                            for (int j = 0; j < 16; j++) {
                                                // metsvm[j] = d_branchtab27_generic[1].c[(i*16) +
                                                // j] ^ sym1v[j]; metsvm[j] = d_brtab27[1][(i*16) +
                                                // j] ^ sym1v[j];
                                                metsvm[j] =
                                                    (unsigned char)
                                            ...
                                            for (int j = 0; j < 16; j++) {
                                                // metsvm[j] = d_branchtab27_generic[0].c[(i*16) +
                                                // j] ^ sym0v[j]; metsvm[j] = d_brtab27[0][(i*16) +
                                                // j] ^ sym0v[j]; metsvm[j] = d_brtab27_0[(i*16) + j]
                                                // ^ sym0v[j];
                                                metsvm[j] =
                                                    (unsigned char)
                                            ...
                                            for (int j = 0; j < 16; j++) {
                                                // metsvm[j] = (d_branchtab27_generic[0].c[(i*16) +
                                                // j] ^ sym0v[j]) +
                                                // (d_branchtab27_generic[1].c[(i*16) + j] ^
                            ...
                            if ((in_count > 0) && (in_count % 16) == 8) { // 8 or 11
                                ...
                                //    ntraceback  : INPUT         : int (I think effectively const
                                //    for given run type; here 5 I think) outbuf      : OUTPUT : 1
