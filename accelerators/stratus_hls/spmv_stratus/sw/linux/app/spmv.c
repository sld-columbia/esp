...
static const char usage_str[] =
    "usage: spmv_stratus coherence cmd [plm_size] [fit_plm] [in_file] [-v]\n"
...
static inline size_t spmv_out_size(struct spmv_test *test) { return sizeof(int) * test->out_size; }

static inline size_t spmv_out_addr(struct spmv_test *test) { return sizeof(int) * test->out_addr; }
...
static void spmv_alloc_buf(struct test_info *info)
{
    ...
    if (!fp) die_errno("%s: cannot open file %s", __func__, test->in_file);
...
static void spmv_alloc_contig(struct test_info *info)
{
    ...
    if (contig_alloc(size, &info->contig) == NULL) die_errno(__func__);
}
...
static void spmv_init_bufs(struct test_info *info)
{
    ...
    contig_copy_to(info->contig, sizeof(int) * test->vals_addr, test->in_vals_fx_buf,
                   sizeof(int) * test->vals_size);
    contig_copy_to(info->contig, sizeof(int) * test->cols_addr, test->in_cols_buf,
                   sizeof(int) * test->cols_size);
    contig_copy_to(info->contig, sizeof(int) * test->rows_addr, test->in_rows_buf,
                   sizeof(int) * test->rows_size);
    contig_copy_to(info->contig, sizeof(int) * test->vect_addr, test->in_vect_fx_buf,
                   sizeof(int) * test->vect_size);
}
...
int check_error_threshold(float out, float gold)
{
    ...
    if (fabs(gold) >= 1) { return (error > MAX_REL_ERROR); }
    else {
...
static int check_gold(float *gold, float *out, int nrows, bool verbose)
{
    ...
    for (i = 0; i < nrows; i++)
        if (check_error_threshold(out[i], gold[i])) {
            if (verbose) printf("out[%d]: result=%.15g; gold=%.15g\n", i, out[i], gold[i]);
            rtn++;
        }
...
int main(int argc, char *argv[])
{
    ...
    if (!strcmp(argv[2], "run")) n_argc = 3;
    else if (!strcmp(argv[2], "test"))
        n_argc = 6;
    else
        n_argc = 5;

    if (argc < n_argc) usage();
