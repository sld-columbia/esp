...
static const char usage_str[] =
    ...
    "  fit_plm   : when not zero the dense vector is kept in the PLM if cols < "
    "8K\n"
    "  in_file   : input matrix, input vector and configuration parameters\n"
    "\n"
    "Other optional arguments:\n"
    "  -v        : 'test' prints mismatched values in case of errors above "
    "threshold\n";
...
static inline size_t spmv_in_size(struct spmv_test *test) {
  return sizeof(int) * (test->vals_size + test->cols_size + test->rows_size +
                        test->vect_size);
}
...
static void spmv_alloc_buf(struct test_info *info) {
  ...
  // Read configuration
  fscanf(fp, "%u %u %u %u\n", &test->nrows, &test->ncols, &test->max_nonzero,
         &test->mtx_len);
...
static void spmv_init_bufs(struct test_info *info) {
  ...
  contig_copy_to(info->contig, sizeof(int) * test->vals_addr,
                 test->in_vals_fx_buf, sizeof(int) * test->vals_size);
  ...
  contig_copy_to(info->contig, sizeof(int) * test->vect_addr,
                 test->in_vect_fx_buf, sizeof(int) * test->vect_size);
}
...
static bool spmv_diff_ok(struct test_info *info) {
  ...
  contig_copy_from(t->out_fx_buf, info->contig, spmv_out_addr(t),
                   spmv_out_size(t));
