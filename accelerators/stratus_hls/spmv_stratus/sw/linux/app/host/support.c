...
char *find_section_start(char *s, int n) {
  ...
  while (i < n && (*s) != (char)0) {
    // This comparison will short-circuit before overrunning the string, so no
    // length check.
    if (s[0] == '%' && s[1] == '%' && s[2] == '\n') {
      i++;
    }
  ...
  if (*s != (char)0)
    return s +
           2; // Skip the section header itself, return pointer to the content
...
#define generate_parse_TYPE_array(TYPE, STRTOTYPE)                             \
  int parse_##TYPE##_array(char *s, TYPE *arr, int n) {                        \
    ...
    while (line != NULL && i < n) {                                            \
      ...
      /*if( errno!=0 ) {                                                       \
        fprintf(stderr, "Couldn't convert string \"%s\": line %d of            \
      section\n", line, i);                                                    \
      }*/                                                                      \
