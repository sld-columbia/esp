...
static bool spmv_xfer_input_ok(struct esp_device *esp, void *arg)
{
    ...
    if (!is_power_of_2(a->nrows)) return false;

    if (!is_power_of_2(a->ncols)) return false;

    if (!is_power_of_2(a->max_nonzero)) return false;

    if (a->max_nonzero > 32 || a->max_nonzero < 4) return false;

    if (a->mtx_len == 0 || ((long long unsigned)a->mtx_len) > mtx_max) return false;

    if (!is_power_of_2(a->vals_plm_size)) return false;

    if (a->vals_plm_size < 16 || a->vals_plm_size > 1024) return false;

    if (a->vect_fits_plm && a->ncols > 8192) return false;
...
static int __init spmv_init(void) { return esp_driver_register(&spmv_driver); }

static void __exit spmv_exit(void) { esp_driver_unregister(&spmv_driver); }
