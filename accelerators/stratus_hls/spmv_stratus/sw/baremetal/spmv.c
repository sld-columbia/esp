...
void spmv_comp(unsigned nrows, unsigned ncols, float *in_vals_buf,
               unsigned *in_cols_buf, unsigned *in_rows_buf, float *in_vect_buf,
               float *gold_buf) {
  long i, j;
  double sum, Si;
...
int main(int argc, char *argv[]) {
  ...
  if (!(in_vals_buf && in_cols_buf && in_rows_buf && in_vect_buf && out_buf &&
        in_vals_fx_buf && in_vect_fx_buf && out_fx_buf)) {
...
#include "data.h"
  ...
  spmv_comp(nrows, ncols, in_vals_buf, in_cols_buf, in_rows_buf, in_vect_buf,
            gold_buf);
  ...
  for (n = 0; n < ndev; n++) {
...
#else
    {
      ...
      if (ioread32(dev, PT_NCHUNK_MAX_REG) == 0) {
        printf("  -> scatter-gather DMA is disabled; revert to contiguous "
               "buffer.\n");
        scatter_gather = 0;
      } else {
      ...
      size = sizeof(int) *
             (vals_size + cols_size + rows_size + vect_size + out_size);

      if (scatter_gather)
        if (ioread32(dev, PT_NCHUNK_MAX_REG) < NCHUNK(size)) {
          printf(
              "  Trying to allocate %lu chunks on %d TLB available entries\n",
      ...
      if (scatter_gather) {
        ...
        iowrite32(dev, DST_OFFSET_REG,
                  (unsigned long)mem); // Sort runs in place
      }
