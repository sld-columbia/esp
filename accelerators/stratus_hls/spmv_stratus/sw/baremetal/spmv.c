...
void spmv_comp(unsigned nrows, unsigned ncols, float *in_vals_buf, unsigned *in_cols_buf,
               unsigned *in_rows_buf, float *in_vect_buf, float *gold_buf)
{
    long i, j;
    double sum, Si;
...
int main(int argc, char *argv[])
{
    ...
    if (!(in_vals_buf && in_cols_buf && in_rows_buf && in_vect_buf && out_buf && in_vals_fx_buf &&
          in_vect_fx_buf && out_fx_buf)) {
