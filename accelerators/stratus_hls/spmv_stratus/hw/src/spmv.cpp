...
void spmv::load_input()
{
    ...
    if (vect_fits_plm) {

        {
            ...
            dma_info_t dma_info(index_vect, ncols, SIZE_WORD, 0);
    ...
    for (uint32_t b = 0; b < bursts; b++) {
        ...
        // Load row delimiters
        {
            ...
            dma_info_t dma_info(index_rows, len_rows, SIZE_WORD, 0);
            index_rows += len_rows;
            this->dma_read_ctrl.put(dma_info);
        }
        ...
        for (uint16_t i = 0; i < len_rows; i++) {
...

            {
                ...
                if (ping) ROWS0[i] = data.to_int() - rows_last;
                else
...

        {
            ...
            dma_info_t dma_info(index_vals, len_vals, SIZE_WORD, 0);
            index_vals += len_vals;
            this->dma_read_ctrl.put(dma_info);
        }
...

        {
            ...
            dma_info_t dma_info(index_cols, len_cols, SIZE_WORD, 0);
            index_cols += len_cols;
            this->dma_read_ctrl.put(dma_info);
        }
        ...
        for (uint16_t i = 0; i < len_cols; i++) {
...

            {
                ...
                if (ping | !vect_fits_plm) COLS0[i] = data.to_int();
                else
                    COLS1[i] = data.to_int();
        ...
        if (!vect_fits_plm) {
            ...
            for (int i = 0; i < len_vect; i++) {
...

                {
                    ...
                    dma_info_t dma_info(local_index_vect, 1, SIZE_WORD, 0);
                    this->dma_read_ctrl.put(dma_info);
                }
...
void spmv::store_output()
{
    ...
    for (uint16_t b = 0; b < bursts; b++) {
...

        {
            ...
            dma_info_t dma_info(index_out, len_out, SIZE_WORD, 0);
            index_out += len_out;
        ...
        for (uint16_t i = 0; i < len_out; i++) {
...

            {
                ...
                if (ping) data = sc_bv<WORD_SIZE>(OUT0[i]);
                else
...
void spmv::compute_kernel()
{
    ...
    for (unsigned b = 0; b < bursts; b++) {
        ...
        for (unsigned r = 0; r < len_rows; r++) {
            ...
            if (ping) OUT0[r] = fp2int<FPDATA, WORD_SIZE>(sum);
            else
