...
class conf_info_t {
  public:
    ...
    //
    conf_info_t() :
        nrows(0), ncols(0), max_nonzero(0), mtx_len(0), vals_plm_size(0), vect_fits_plm(0)
    {
    }

    conf_info_t(uint32_t r, uint32_t c, uint32_t max, uint32_t mtx_len, uint32_t vals_plm_size,
                bool vect_fits_plm) :
        nrows(r),
        ncols(c), max_nonzero(max), mtx_len(mtx_len), vals_plm_size(vals_plm_size),
    ...
    {
        return (rhs.nrows == nrows) && (rhs.ncols == ncols) && (rhs.max_nonzero == max_nonzero) &&
            (rhs.mtx_len == mtx_len) && (rhs.vals_plm_size == vals_plm_size) &&
    ...
    {
        os << "{ nrows = " << conf_info.nrows << ", ncols = " << conf_info.ncols
           << ", mtx_len = " << conf_info.mtx_len << ", vals_plm_size = " << conf_info.vals_plm_size
