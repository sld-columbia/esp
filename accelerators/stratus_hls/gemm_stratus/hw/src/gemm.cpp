...
void gemm::load_input() {
...

  {
    calculate_config(ninputs, matrix_d1, matrix_d2, matrix_d3, transpose,
                     size_matrix1, size_matrix2, size_matrix_out, matrix_chk_in,
                     matrix_rem_in1, matrix_rem_in2, matrix_chk_out,
                     matrix_rem_out, load_cfg, loadable_rows, loadable_chunk,
                     index_d1_incr, m2_loop_iters, m2_plm_incr);
    ...
    // 		(unsigned) matrix_chk_in, (unsigned) matrix_rem_in1, (unsigned)
    // matrix_rem_in2, 		(unsigned) matrix_chk_out, (unsigned) matrix_rem_out,
        ...
        for (uint24_t chk = 0; chk < matrix_chk_in; ++chk) {
          ...
          if (!(d2 && load_cfg == LESS_THAN_MATRIX2)) {

            {
              ...
              // 		    (unsigned) index_d1_tmp, (unsigned)
              // round_up(length1, WORDS_PER_DMA));
            }
            ...
            for (uint16_t k = 0;
                 k < round_up(length1 + misaligned, WORDS_PER_DMA) >>
                 WORDS_PER_DMA_LOG;
                 ++k) {
          ...
          for (uint16_t t = 0; t < m2_loop_iters; ++t) {
            i = base_i;
            {
              ...
              // 		    (unsigned) index_m2_dma, (unsigned)
              // round_up(length_m2_dma, WORDS_PER_DMA));
            }
      ...
      for (uint24_t d2 = 0; d2 < matrix_d3; d2 += loadable_rows) {
        ...
        for (uint16_t d1i = 0; d1i < loaded_rows_d1; d1i++) {
          for (uint24_t chk = 0; chk < matrix_chk_out; ++chk) {
            ...
            // 		(unsigned) d1, (unsigned) d2, (unsigned) d1i, (unsigned)
            // chk);
            ...
            i = 0;
            for (uint16_t k = 0;
                 k < round_up(length, WORDS_PER_DMA) >> WORDS_PER_DMA_LOG;
                 ++k) {
...
void gemm::compute_kernel() {
  ...
  // Config
  {
    ...
    // 		(unsigned) matrix_chk_in, (unsigned) matrix_rem_in1, (unsigned)
    // matrix_rem_in2, 		(unsigned) load_cfg, (unsigned) loadable_rows);
  }
      ...
      for (uint24_t d2 = 0; d2 < matrix_d3; d2 += loadable_rows) {
        ...
        for (uint16_t d1i = 0; d1i < loaded_rows_d1; d1i++) {
          ...
          for (uint16_t d2i = 0; d2i < loaded_rows_d3; d2i++) {
            ...
            // Call the store_output process and wait for the store_output
            // process
