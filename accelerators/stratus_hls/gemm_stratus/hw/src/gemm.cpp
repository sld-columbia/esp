...
void gemm::load_input()
{
    ...
    {
        calculate_config(ninputs, matrix_d1, matrix_d2, matrix_d3, transpose, size_matrix1,
                         size_matrix2, size_matrix_out, matrix_chk_in, matrix_rem_in1,
                         matrix_rem_in2, matrix_chk_out, matrix_rem_out, load_cfg, loadable_rows,
                         loadable_chunk, index_d1_incr, m2_loop_iters, m2_plm_incr);
            ...
            for (uint24_t d2 = 0; d2 < matrix_d3; d2 += loadable_rows) {
                ...
                for (uint24_t chk = 0; chk < matrix_chk_in; ++chk) {
                    ...
                    if (load_cfg == LESS_THAN_ROW && chk == matrix_chk_in - 1 &&
                        ...
                        if (d1 + loadable_rows > matrix_d1) length1 = matrix_rem_in1;
                        if (d2 + loadable_rows > matrix_d3) length2 = matrix_rem_in2;
                    }
                    ...
                    if (!(d2 && load_cfg == LESS_THAN_MATRIX2)) {

                        {
                            ...
                            dma_info_t dma_info(
                                index_d1_tmp >> WORDS_PER_DMA_LOG,
                                round_up(length1, WORDS_PER_DMA) >> WORDS_PER_DMA_LOG, SIZE_WORD);
                            this->dma_read_ctrl.put(dma_info);

                            // ESP_REPORT_INFO("load m1 %u %u",
                            // 		    (unsigned) index_d1_tmp, (unsigned) round_up(length1,
                            // WORDS_PER_DMA));
                        }
                        ...
                        for (uint16_t k = 0;
                             k<round_up(length1 + misaligned, WORDS_PER_DMA)> > WORDS_PER_DMA_LOG;
                             ++k) {
...

                            {
                                ...
                                if (pingpong_m1) {
                                    ...
                                    if (i < DMA_CHUNK) input0[i++] = data.range(63, 32).to_uint();
                                }
                                else {
                                    if (!(misaligned && !k))
                                        input1[i++] = data.range(31, 0).to_uint();
                                    if (i < DMA_CHUNK) input1[i++] = data.range(63, 32).to_uint();
                                }
#else
                                if (pingpong_m1) { input0[i++] = data.to_uint(); }
                                else {
                    ...
                    if (transpose || matrix_d3 == 1) {
                        ...
                        if (load_cfg == LESS_THAN_ROW) { m2_loop_iters = length2; }
                    ...
                    for (uint16_t t = 0; t < m2_loop_iters; ++t) {
                        i = base_i;
                        {
                            ...
                            dma_info_t dma_info(index_m2_dma >> WORDS_PER_DMA_LOG,
                                                round_up(length_m2_dma, WORDS_PER_DMA) >>
                                                    WORDS_PER_DMA_LOG,
                                                SIZE_WORD);
                            this->dma_read_ctrl.put(dma_info);

                            // ESP_REPORT_INFO("load m2 %u %u",
                            // 		    (unsigned) index_m2_dma, (unsigned) round_up(length_m2_dma,
                            // WORDS_PER_DMA));
                        }
                        ...
                        for (uint16_t k = 0;
                             k<round_up(length_m2_dma + misaligned, WORDS_PER_DMA)> >
                             WORDS_PER_DMA_LOG;
                             ++k) {
                            sc_dt::sc_bv<DMA_WIDTH> data = this->dma_read_chnl.get();
...
#if (WORDS_PER_DMA != 2)
                                if (pingpong_m2) input2[i] = data.to_uint();
                                else
                                    input3[i] = data.to_uint();
                                i += m2_plm_incr;
#else
                                if (!(misaligned && !k)) {
                                    if (pingpong_m2) input2[i] = data.range(31, 0).to_uint();
                                    else
                                        input3[i] = data.range(31, 0).to_uint();
                                    i += m2_plm_incr;
                                }
                                if (i < DMA_CHUNK) {
                                    if (m2_plm_incr != 1) { wait(); }
                                    if (pingpong_m2) input2[i] = data.range(63, 32).to_uint();
                                    else
                    ...
                    if (transpose) index_d2_tmp += length_m2_dma;
                }
                if (!transpose) index_d2_tmp += loadable_rows;
            }
...
void gemm::store_output()
{
    ...
    // Config
    {
        ...
        if (load_cfg == LESS_THAN_MATRIX2 && loadable_rows != 1) { matrix_chk_out = 1; }
        else {
            ninputs       = 1;
            matrix_d1     = 1;
            matrix_d3     = 1;
                ...
                for (uint16_t d1i = 0; d1i < loaded_rows_d1; d1i++) {
                    for (uint24_t chk = 0; chk < matrix_chk_out; ++chk) {
...

                        {
                            ...
                            dma_info_t dma_info(
                                index >> WORDS_PER_DMA_LOG,
                                round_up(length, WORDS_PER_DMA) >> WORDS_PER_DMA_LOG, SIZE_WORD);
                        ...
                        i = 0;
                        for (uint16_t k = 0; k<round_up(length, WORDS_PER_DMA)> > WORDS_PER_DMA_LOG;
                             ++k) {
    ...
    for (uint24_t a = 0; a < ninputs; a++) {
        ...
        for (uint24_t d1 = 0; d1 < matrix_d1; d1 += loadable_rows) {
            ...
            for (uint24_t d2 = 0; d2 < matrix_d3; d2 += loadable_rows) {
                ...
                if (load_cfg != LESS_THAN_ROW) compute_load_handshake();
                ...
                for (uint16_t d1i = 0; d1i < loaded_rows_d1; d1i++) {
                    ...
                    for (uint16_t d2i = 0; d2i < loaded_rows_d3; d2i++) {
                        ...
                        for (uint24_t chk = 0; chk < matrix_chk_in; ++chk) {
                            ...
                            for (uint16_t k = 0; k < (length + PARALLELISM - 1) / PARALLELISM;
                                 ++k) {
                                ...
                                if (plm_i < length) mult_out[1] = row[1] * col[1];
                                else
                                    mult_out[1] = 0;
#if (PARALLELISM >= 4)
                                if (plm_i + 1 < length) mult_out[2] = row[2] * col[2];
                                else
                                    mult_out[2] = 0;
                                if (plm_i + 2 < length) mult_out[3] = row[3] * col[3];
                                else
                                    mult_out[3] = 0;
#endif
#if (PARALLELISM >= 8)
                                if (plm_i + 3 < length) mult_out[4] = row[4] * col[4];
                                else
                                    mult_out[4] = 0;
                                if (plm_i + 4 < length) mult_out[5] = row[5] * col[5];
                                else
                                    mult_out[5] = 0;
                                if (plm_i + 5 < length) mult_out[6] = row[6] * col[6];
                                else
                                    mult_out[6] = 0;
                                if (plm_i + 6 < length) mult_out[7] = row[7] * col[7];
                                else
                                    mult_out[7] = 0;
#endif
#if (PARALLELISM >= 16)
                                if (plm_i + 7 < length) mult_out[8] = row[8] * col[8];
                                else
                                    mult_out[8] = 0;
                                if (plm_i + 8 < length) mult_out[9] = row[9] * col[9];
                                else
                                    mult_out[9] = 0;
                                if (plm_i + 9 < length) mult_out[10] = row[10] * col[10];
                                else
                                    mult_out[10] = 0;
                                if (plm_i + 10 < length) mult_out[11] = row[11] * col[11];
                                else
                                    mult_out[11] = 0;
                                if (plm_i + 11 < length) mult_out[12] = row[12] * col[12];
                                else
                                    mult_out[12] = 0;
                                if (plm_i + 12 < length) mult_out[13] = row[13] * col[13];
                                else
                                    mult_out[13] = 0;
                                if (plm_i + 13 < length) mult_out[14] = row[14] * col[14];
                                else
                                    mult_out[14] = 0;
                                if (plm_i + 14 < length) mult_out[15] = row[15] * col[15];
                                else
                                    mult_out[15] = 0;
#endif
                        ...
                        // ReLU
                        accumulator =
                            (do_relu && accumulator < (FPDATA)0) ? (FPDATA)0 : accumulator;

                        // Write to output PLM
                        if (pingpong_out) { output0[store_count] = FP2INT(accumulator); }
                        else {
