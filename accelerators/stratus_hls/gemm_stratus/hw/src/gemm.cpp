...
void gemm::load_input()
{
    ...
    {
        calculate_config(ninputs, matrix_d1, matrix_d2, matrix_d3, transpose, size_matrix1, size_matrix2,
                         size_matrix_out, matrix_chk_in, matrix_rem_in1, matrix_rem_in2, matrix_chk_out, matrix_rem_out,
                         load_cfg, loadable_rows, loadable_chunk, index_d1_incr, m2_loop_iters, m2_plm_incr);
            ...
            for (uint24_t d2 = 0; d2 < matrix_d3; d2 += loadable_rows) {
                ...
                for (uint24_t chk = 0; chk < matrix_chk_in; ++chk) {
                    ...
                    if (!(d2 && load_cfg == LESS_THAN_MATRIX2)) {
                        ...
                        for (uint16_t k = 0; k < round_up(length1 + misaligned, WORDS_PER_DMA) >> WORDS_PER_DMA_LOG;
                             ++k) {
                    ...
                    for (uint16_t t = 0; t < m2_loop_iters; ++t) {
                        ...
                        for (uint16_t k = 0;
                             k < round_up(length_m2_dma + misaligned, WORDS_PER_DMA) >> WORDS_PER_DMA_LOG; ++k) {
                            sc_dt::sc_bv<DMA_WIDTH> data = this->dma_read_chnl.get();
            ...
            for (uint24_t d2 = 0; d2 < matrix_d3; d2 += loadable_rows) {
                ...
                for (uint16_t d1i = 0; d1i < loaded_rows_d1; d1i++) {
                    ...
                    for (uint16_t d2i = 0; d2i < loaded_rows_d3; d2i++) {
                        ...
                        sync_compute_store(store_count, loaded_rows_d3, load_cfg, loadable_rows, pingpong_out);
                    }
                }
            }
        }
