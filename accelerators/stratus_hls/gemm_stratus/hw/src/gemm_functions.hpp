...
inline void gemm::calculate_config(uint24_t ninputs, uint24_t matrix_d1, uint24_t matrix_d2,
                                   ...
                                   uint16_t &m2_plm_incr)
{
    ...
    if ((matrix_d2 > DMA_CHUNK) || (is_less_than_matrix2 && d3_odd)) {
        ...
        if (size_matrix2 > DMA_CHUNK) {
            ...
            if (loadable_rows != 1) loadable_rows = (loadable_rows >> 1) << 1;
        }
        else {
...
inline void gemm::calculate_chunks(uint24_t &matrix_chk, uint16_t &matrix_rem, uint32_t matrix_d2,
                                   bool in_or_out)
{
    uint32_t matrix_mul;
    {
...
inline void gemm::sync_compute_store(uint16_t &count, uint16_t loaded_rows, uint8_t load_cfg,
