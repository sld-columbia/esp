...
void gemm_pvt(int l_n,int prev_soft,float*layer_in_sw,float* layer_in_hw,float*out,float* W,float*bias, bool relu,bool soft,int wr,int wc,int ws)
{
...
void gemm_pp(float* out,float* layer_bias,int wr,int relu)
{
// Relu
        for (int i = 0; i < wr; i++) {
                if (relu) {
