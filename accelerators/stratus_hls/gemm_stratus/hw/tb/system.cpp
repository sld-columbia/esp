...
void system_t::dump_memory() {
  ...
  for (uint32_t i = 0;
       i <
       round_up((batch_size * rowsA * colsB), WORDS_PER_DMA) / WORDS_PER_DMA;
       ++i) {
    data = this->mem[indexB++];

    for (uint32_t w = 0; w < WORDS_PER_DMA; ++w) {

      bv2fp<FPDATA, WORD_SIZE>(
          elem, data.range((w + 1) * WORD_SIZE - 1, w * WORD_SIZE));
...
int system_t::validate() {
  ...
  for (uint32_t d2 = 0; d2 < size_out * batch_size; ++d2) {
    if (check_error_threshold(matrix_out->data[d2], matrix_out_gold->data[d2],
                              rel_error)) {
      if (tot_errors < REPORT_THRESHOLD) {
        ESP_REPORT_INFO("gemm[%d] = %lf (%lf) error: %.2lf%%", d2,
                        matrix_out->data[d2], matrix_out_gold->data[d2],
                        rel_error * 100);
      }
    ...
    // ESP_REPORT_INFO("gemm[%d] = %lf (%lf) error: %.2lf%%", d2,
    // 		matrix_out->data[d2], matrix_out_gold->data[d2], rel_error *
    // 100);
