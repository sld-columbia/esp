    ...
    for (uint32_t b = 0; b < batch_size; ++b) {
        for (uint32_t d1 = 0; d1 < rowsA; ++d1) {
            for (uint32_t d2 = 0; d2 < colsA; ++d2) {
                ...
                if (!((k - 1) % WORDS_PER_DMA)) (this->mem[indexA]) = data | this->mem[indexA];
                else
                    (this->mem[indexA]) = (data << WORD_SIZE) | this->mem[indexA];

                if ((k - 1) % WORDS_PER_DMA == WORDS_PER_DMA - 1) indexA++;
            }
        }
    }
    ...
    for (uint32_t b = 0; b < batch_size; ++b) {
        for (uint32_t d1 = 0; d1 < rowsB_actual; ++d1) {
            for (uint32_t d2 = 0; d2 < colsB_actual; ++d2) {
                ...
                // TODO works only for WORDS_PER_DMA = {1, 2}
                if (!((k - 1) % WORDS_PER_DMA)) (this->mem[indexB]) = data | this->mem[indexB];
                else
                    (this->mem[indexB]) = (data << WORD_SIZE) | this->mem[indexB];

                if ((k - 1) % WORDS_PER_DMA == WORDS_PER_DMA - 1) indexB++;
            }
        }
    }
...
void system_t::dump_memory()
{
    ...
    uint32_t k = 0;
    for (uint32_t i = 0; i < round_up((batch_size * rowsA * colsB), WORDS_PER_DMA) / WORDS_PER_DMA;
         ++i) {
        ...
        for (uint32_t w = 0; w < WORDS_PER_DMA; ++w) {
            ...
            if (k < batch_size * rowsA * colsB) matrix_out->data[k++] = elem.to_double();
        }
    }
...
int system_t::validate()
{
    ...
    for (uint32_t d2 = 0; d2 < size_out * batch_size; ++d2) {
        if (check_error_threshold(matrix_out->data[d2], matrix_out_gold->data[d2], rel_error)) {
            if (tot_errors < REPORT_THRESHOLD) {
                ESP_REPORT_INFO("gemm[%d] = %lf (%lf) error: %.2lf%%", d2, matrix_out->data[d2],
    ...
    if (tot_errors > 0) { ESP_REPORT_ERROR("validation failed!"); }
    else {
        ESP_REPORT_INFO("validation succeeded!");
    }

    ESP_REPORT_INFO("total memory #%d bytes", (unsigned int)(indexB * 4UL) / 1000UL);
