...
static void adder_alloc_buf(struct test_info *info)
{
    ...
    if (!strcmp(info->cmd, "test")) { t->sbuf = malloc0_check(adder_size(t)); }
}
...
static void adder_alloc_contig(struct test_info *info)
{
    ...
    if (contig_alloc(adder_size(t), &info->contig) == NULL) die_errno(__func__);
}
...
static bool adder_diff_ok(struct test_info *info)
{
    ...
    if (err) printf("Test FAILED: %d mismatches\n", err);
    else
