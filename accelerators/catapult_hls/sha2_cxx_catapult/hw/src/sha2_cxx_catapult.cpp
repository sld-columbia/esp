...
void compute_wrapper(uint32_t in_bytes, uint32_t out_bytes, T1 &input, T2 &output) {
    ESP_REPORT_INFO(VOFF, "in_bytes: %u", ESP_TO_UINT32(in_bytes));
    ESP_REPORT_INFO(VOFF, "out_bytes: %u", ESP_TO_UINT32(out_bytes));
...
void sha2_cxx_catapult(
#else
void CCS_BLOCK(sha2_cxx_catapult)(
#endif
    ...
    out_bytes = config.out_bytes;

    ESP_REPORT_INFO(VOFF, "conf_info.in_bytes = %u", ESP_TO_UINT32(in_bytes));
    ESP_REPORT_INFO(VOFF, "conf_info.out_bytes = %u", ESP_TO_UINT32(out_bytes));
...
LOAD_LOOP:
    ...
    compute_wrapper<plm_in_t, plm_out_t>(in_bytes, out_bytes, plm_in, plm_out);
