...
const int32_t mac_vec = 64;
const int32_t mac_len = 10;
...
int main(int argc, char *argv[])
{
    ...
    if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
        in_words_adj  = mac_vec*mac_len;
        out_words_adj = mac_vec;
    }
    else {
        in_words_adj  = round_up(mac_vec*mac_len, DMA_WORD_PER_BEAT(sizeof(token_t)));
    ...
    for (n = 0; n < ndev; n++) {
        ...
        for (coherence = ACC_COH_NONE; coherence <= ACC_COH_RECALL; coherence++) {
            ...
            /* <<--regs-config-->> */
			iowrite32(dev, MACFIN_MAC_N_REG, mac_n);
			iowrite32(dev, MACFIN_MAC_VEC_REG, mac_vec);
			iowrite32(dev, MACFIN_MAC_LEN_REG, mac_len);
