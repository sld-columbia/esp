...
CCS_MAIN(int argc, char **argv) {
    ...
    ESP_REPORT_INFO(VON, "  - SHA1 DMA width: %u", DMA_WIDTH);
    ...
    for (unsigned t = 1; t < N_TESTS; t++) {
        unsigned sha1_in_words = 0; 

#ifdef SHA1_ALGO
        conf_info_data.crypto_algo = SHA1_ALGO;
        conf_info_data.sha1_in_bytes = sha1_raw_in_bytes[t];
        sha1_in_words = (ESP_TO_UINT32(conf_info_data.sha1_in_bytes) + 4 - 1)/4;
#endif

        ESP_REPORT_INFO(VON, "Configuration:");
        ESP_REPORT_INFO(VON, "  - in_bytes: %u (words %u)", ESP_TO_UINT32(conf_info_data.sha1_in_bytes), sha1_in_words);
