...
AES_EXPAND_L_3:
        for (j = 0; j < 40; j += 4) {
            ...
            temp        = ekey[j + 3];
            ekey[j + 4] = ekey[j + 0] ^ (Te2[__range(temp, 23, 16)] & 0xff000000) ^
                (Te3[__range(temp, 15, 8)] & 0x00ff0000) ^ (Te0[__range(temp, 7, 0)] & 0x0000ff00) ^
...
AES_EXPAND_L_4:
        for (j = 0; j < 48; j += 6) {
            ...
            temp        = ekey[j + 5];
            ekey[j + 6] = ekey[j + 0] ^ (Te2[__range(temp, 23, 16)] & 0xff000000) ^
                (Te3[__range(temp, 15, 8)] & 0x00ff0000) ^ (Te0[__range(temp, 7, 0)] & 0x0000ff00) ^
                (Te1[__range(temp, 31, 24)] & 0x000000ff) ^ rcon[i++];
...
AES_EXPAND_L_5:
        for (j = 0; j < 56; j += 8) {
            ...
            temp        = ekey[j + 7];
            ekey[j + 8] = ekey[j + 0] ^ (Te2[__range(temp, 23, 16)] & 0xff000000) ^
                (Te3[__range(temp, 15, 8)] & 0x00ff0000) ^ (Te0[__range(temp, 7, 0)] & 0x0000ff00) ^
                (Te1[__range(temp, 31, 24)] & 0x000000ff) ^ rcon[i++];
            ...
            if (j != 48) {
                temp         = ekey[j + 11];
                ekey[j + 12] = ekey[j + 4] ^ (Te2[__range(temp, 31, 24)] & 0xff000000) ^
...
#pragma HLS unroll complete
            ...
            ekey[j] = Td0[Te1[(ekey[j] >> 24) & 0xff] & 0xff] ^
                Td1[Te1[(ekey[j] >> 16) & 0xff] & 0xff] ^ Td2[Te1[(ekey[j] >> 8) & 0xff] & 0xff] ^
...
#pragma HLS unroll complete
                ...
                ekey[j] = Td0[Te1[(ekey[j] >> 24) & 0xff] & 0xff] ^
                    ...
                    Td2[Te1[(ekey[j] >> 8) & 0xff] & 0xff] ^ Td3[Te1[(ekey[j] >> 0) & 0xff] & 0xff];
            }
        }
