...
template <typename T> void memset(T *ptr, int value, size_t num)
{
MEMSET_L_1:
...
#ifdef __MENTOR_CATAPULT_HLS__
void gf_inner(ac_int<128, false> &_v, ac_int<128, false> &_z, uint32 x)
{
    ac_int<128, false> R = ac_int<128, false>(225) << 120;
#else
void gf_inner(ap_uint<128> &_v, ap_uint<128> &_z, uint32 x)
{
...
void gf_mult(uint32 x[AES_BLOCK_WORDS], uint32 y[AES_BLOCK_WORDS], uint32 z[AES_BLOCK_WORDS])
{
#pragma HLS inline
...
void ghash(uint32 h[AES_BLOCK_WORDS], uint32 x[AES_BLOCK_WORDS], uint32 y[AES_BLOCK_WORDS])
{
...
void gcm_init(uint32 key_bytes, uint32 iv_bytes, uint32 aad_bytes, uint32 ekey[AES_EXP_KEY_SIZE],
              uint32 iv[AES_MAX_IV_WORDS], uint32 aad[AES_MAX_IN_WORDS], uint32 S[AES_BLOCK_WORDS],
              uint32 H[AES_BLOCK_WORDS], uint32 L[AES_BLOCK_WORDS], uint32 J[AES_BLOCK_WORDS])
{
#pragma HLS inline
...
#pragma HLS unroll complete
                ...
                tmp[k] = (i + (k << 2) < iv_bytes) ? iv[(i >> 2) + k] : uint32(0);
            }
...
#pragma HLS unroll complete
            ...
            tmp[k] = (i + (k << 2) < aad_bytes) ? aad[(i >> 2) + k] : uint32(0);
        }
...
void gcm_core(uint32 encryption, uint32 key_bytes, uint32 input_bytes, uint32 total_blocks,
              uint32 in[AES_MAX_IN_WORDS], uint32 out[AES_MAX_IN_WORDS],
              uint32 ekey[AES_EXP_KEY_SIZE], uint32 cb[AES_BLOCK_WORDS], uint32 S[AES_BLOCK_BYTES],
              uint32 H[AES_BLOCK_BYTES], uint32 J[AES_BLOCK_BYTES], uint32 T[AES_BLOCK_BYTES])
{
#pragma HLS inline
...
#pragma HLS unroll complete
            ...
            if (i + (k << 2) < input_bytes) in_tmp[k] = in[(i >> 2) + k];
        }
...
#pragma HLS unroll complete
            ...
            if (i + (k << 2) < input_bytes) out[(i >> 2) + k] = out_tmp[k];
        }
...
#pragma HLS unroll complete
            ...
            hash_tmp[k] = (i + (k << 2) < input_bytes) ?
                ((encryption == AES_DECRYPTION_MODE) ? in_tmp[k] : out_tmp[k]) :
                uint32(0);
        }
...
uint32 gcm_final(uint32 encryption, uint32 tag_bytes, uint32 total_bytes,
                 uint32 tag[AES_MAX_IN_WORDS], uint32 S[AES_BLOCK_BYTES], uint32 H[AES_BLOCK_BYTES],
                 uint32 L[AES_BLOCK_BYTES], uint32 T[AES_BLOCK_BYTES])
{
#pragma HLS inline
...
int aes_gcm_cipher(uint32 encryption, uint32 key_bytes, uint32 input_bytes, uint32 iv_bytes,
                   uint32 aad_bytes, uint32 tag_bytes, uint32 ekey[AES_EXP_KEY_SIZE],
                   uint32 iv[AES_MAX_IV_WORDS], uint32 in[AES_MAX_IN_WORDS],
                   uint32 out[AES_MAX_IN_WORDS], uint32 aad[AES_MAX_IN_WORDS],
...
#pragma HLS array_partition variable = T complete
    ...
    if (!init_done) {
        gcm_init(key_bytes, iv_bytes, aad_bytes, ekey, iv, aad, S, H, L, J);
        init_done = true;
    }

    if (tag_bytes == 0) {
        gcm_core(encryption, key_bytes, input_bytes, total_blocks, in, out, ekey, cb, S, H, J, T);
    ...
    {
        check = gcm_final(encryption, tag_bytes, total_bytes, tag, S, H, L, T);
