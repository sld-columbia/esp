...
void sha256_init(uint32 h[SHA256_HBLOCK_WORDS],
                 uint32 out_bytes)
{
...
void sha256_load(uint32 data[SHA256_CBLOCK_WORDS],
...
void sha256_block(uint32 h[SHA256_HBLOCK_WORDS],
                  uint32 X[SHA256_CBLOCK_WORDS])
{
    uint32 tmp[8];
    uint32 s0, s1, T1, T2;
...
SHA256_BLOCK_L_2:
    for (unsigned i = 0; i < 16; i++)
    {
        ...
        T1 = X[i] + tmp[7] + SHA256_SIGMA1(tmp[4]) +
...
SHA256_BLOCK_L_3:
    for (unsigned i = 16; i < 64; i++)
    {
        ...
        T1 = X[(i & 0xf)] + tmp[7] + SHA256_SIGMA1(tmp[4]) +
                SHA256_Ch(tmp[4], tmp[5], tmp[6]) + K256[i];
...
void sha256_process(uint32 h[SHA256_HBLOCK_WORDS],
...
void sha512_init(uint64_t h[SHA512_HBLOCK_WORDS],
                 uint32 out_bytes)
{
...
void sha512_load(uint32 data[SHA512_CBLOCK_WORDS],
                 uint32 in[SHA2_MAX_BLOCK_SIZE])
{
    #pragma HLS inline off
...
void sha512_block(uint64_t h[SHA512_HBLOCK_WORDS],
                  uint32 T[SHA512_CBLOCK_WORDS])
{
    uint64_t A, E;
    uint64_t T1, T2;
    uint64_t X[9 + 80];
...
void sha512_process(uint64_t h[SHA512_HBLOCK_WORDS],
          ...
          uint32 out[SHA2_MAX_DIGEST_WORDS])
{
    ...
    switch (out_bytes)
    {
        ...
        case SHA256_DIGEST_LENGTH:
            sha256(in_bytes, out_bytes, in, out);
            break;
#endif // SYN_SHA256
        ...
        case SHA512_DIGEST_LENGTH:
            sha512(in_bytes, out_bytes, in, out);
            break;
#endif // SYN_SHA512
    }
