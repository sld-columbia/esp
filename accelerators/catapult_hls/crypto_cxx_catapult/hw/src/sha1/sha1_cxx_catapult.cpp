...
template <class T1, class T2> void compute_wrapper(uint32 in_bytes, T1 &input, T2 &output)
{
...
void CCS_BLOCK(sha1_cxx_catapult)(
    ...
    conf_info_t &conf_info, ac_channel<dma_info_t> &dma_read_ctrl,
    ...
    ac_channel<dma_data_t> &dma_write_chnl, ac_sync &acc_done)
{
    ...
    dma_read_info           = {dma_read_data_index, (dma_read_data_length + 2 - 1) / 2, SIZE_WORD,
                     0}; // ceil(dma_read_data_legnth / 2)
...
LOAD_CTRL_LOOP:
    ...
    ESP_REPORT_INFO(
        VON,
        "DMA read ctrl: data index = %u, data length = %u, size [0=8b, 1=16b, 2=32b, 3=64b] = %llu",
...
STORE_CTRL_LOOP:
    ...
    ESP_REPORT_INFO(VON,
                    "DMA write ctrl: data index = %u, data length = %u, size [0=8b, 1=16b, 2=32b, "
                    "3=64b] = %llu",
