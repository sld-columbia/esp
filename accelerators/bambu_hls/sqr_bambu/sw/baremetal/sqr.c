...
static void init_buf(token_t *in, token_t *gold)
{
    ...
    for (i = 0; i < 1; i++)
        for (j = 0; j < size; j++)
            gold[i * out_words_adj + j] = (token_t)j * (token_t)j;
}
...
int main(int argc, char *argv[])
{
    ...
    ndev = probe(&espdevs, VENDOR_SLD, SLD_SQR, DEV_NAME);
    if (ndev == 0) {
        printf("sqr not found\n");
        return 0;
    }
    ...
    for (n = 0; n < ndev; n++) {
...
#else
        {
            ...
            /* <<--regs-config-->> */
            iowrite32(dev, SQR_SIZE_REG, size);
