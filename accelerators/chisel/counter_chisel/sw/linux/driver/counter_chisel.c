...
static bool counter_xfer_input_ok(struct esp_device *esp, void *arg)
{
    ...
    if (a->ticks >= (1 << 16) || a->ticks < 1) return false;
    return true;
}
...
static int __init counter_init(void) { return esp_driver_register(&counter_driver); }

static void __exit counter_exit(void) { esp_driver_unregister(&counter_driver); }

module_init(counter_init) module_exit(counter_exit)
