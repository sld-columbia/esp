...
static inline size_t counter_size() { return 4096; }

static void counter_alloc_buf(struct test_info *info) {}

static void counter_alloc_contig(struct test_info *info)
{
    if (contig_alloc(counter_size(), &info->contig) == NULL) die_errno(__func__);
}
...
static bool counter_diff_ok(struct test_info *info) { return true; }
