...
* `CONFIG_GT_VORTEX_L2_EN`
* `CONFIG_GT_VORTEX_L3_EN`

ESP now derives the generated GT_VORTEX AXI ID width from these values during
socket generation, and the SoC GUI only offers combinations whose derived TID
width fits ESP's current third-party accelerator limit of 10 bits. In the ESP
single-cluster integration, L3 is still meaningful: it acts as the
single-cluster last-level cache between the cluster-side memory bus and the
