...
#define TAG_LOOKUP_FWD						       \
    HLS_CONSTRAIN_LATENCY("l2-tag-lookup-fwd-latency"); \
...
#define REQS_LOOKUP							\
    HLS_CONSTRAIN_LATENCY("l2-reqs-lookup-latency"); \
...
#define REQS_PEEK_FWD							\
    HLS_CONSTRAIN_LATENCY("l2-reqs-peek-fwd-latency"); \
...
#define FLUSH_READ_SET							\
    HLS_CONSTRAIN_LATENCY("l2-flush-read-set-latency");			\
    if (RPT_RTL) CACHE_REPORT_TIME(sc_time_stamp(), "Flush read set.")
    // bookmark_tmp |= BM_FLUSH_READ_SET;					
#define FLUSH_LOOP					\
    HLS_CONSTRAIN_LATENCY("l2-flush-loop-latency")
#define FLUSH_END					\
    HLS_CONSTRAIN_LATENCY("l2-flush-end-latency")
