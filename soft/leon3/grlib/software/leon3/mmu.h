 ...
 * XXX But for now...
 */
#define SRMMU_PAGE_NONE   __pgprot(SRMMU_VALID | SRMMU_CACHE | SRMMU_PRIV | SRMMU_REF)
#define SRMMU_PAGE_SHARED __pgprot(SRMMU_VALID | SRMMU_CACHE | SRMMU_EXEC | SRMMU_WRITE | SRMMU_REF)
#define SRMMU_PAGE_COPY   __pgprot(SRMMU_VALID | SRMMU_CACHE | SRMMU_EXEC | SRMMU_REF)
#define SRMMU_PAGE_RDONLY __pgprot(SRMMU_VALID | SRMMU_CACHE | SRMMU_EXEC | SRMMU_REF)
#define SRMMU_PAGE_KERNEL __pgprot(SRMMU_VALID | SRMMU_CACHE | SRMMU_PRIV | SRMMU_DIRTY | SRMMU_REF)
...
{
    unsigned int retval;
    asm volatile("lda [%1] %2, %0\n\t" : "=r"(retval) : "r"(SRMMU_CTXTBL_PTR), "i"(ASI_M_MMUREGS));
...
{
    register int retval;
    asm volatile("lda [%1] %2, %0\n\t" : "=r"(retval) : "r"(SRMMU_CTX_REG), "i"(ASI_M_MMUREGS));
    return retval;
}
...
static int srmmu_device_memory(unsigned long x) { return ((x & 0xF0000000) != 0); }
...
int srmmu_nocache_used;
    ...
    #define __nocache_pa(VADDR) \
        VADDR // (((unsigned long)VADDR) - SRMMU_NOCACHE_VADDR + __pa((unsigned
              // long)srmmu_nocache_pool))
    #define __nocache_va(PADDR) \
        PADDR // (__va((unsigned long)PADDR) - (unsigned long)srmmu_nocache_pool +
              // SRMMU_NOCACHE_VADDR)
    #define __nocache_fix(VADDR) VADDR // __va(__nocache_pa(VADDR))

static unsigned long srmmu_pgd_page(pgd_t pgd)
{
    return srmmu_device_memory(pgd_val(pgd)) ?
        ~0 :
...
{
    return srmmu_device_memory(pmd_val(pmd)) ?
        ~0 :
        (unsigned long)__nocache_va((pmd_val(pmd) & SRMMU_PTD_PMASK) << 4);
}

// static  struct page *srmmu_pte_page(pte_t pte)
//{ return (mem_map + (unsigned long)(srmmu_device_memory(pte_val(pte))?~0:(((pte_val(pte) &
//SRMMU_PTE_PMASK) << 4) >> PAGE_SHIFT))); }

static int srmmu_pte_none(pte_t pte) { return !(pte_val(pte) & 0xFFFFFFF); }

static int srmmu_pte_present(pte_t pte) { return ((pte_val(pte) & SRMMU_ET_MASK) == SRMMU_ET_PTE); }

static void srmmu_pte_clear(pte_t *ptep) { srmmu_set_pte(ptep, __pte(0)); }

static int srmmu_pmd_none(pmd_t pmd) { return !(pmd_val(pmd) & 0xFFFFFFF); }

static int srmmu_pmd_bad(pmd_t pmd) { return (pmd_val(pmd) & SRMMU_ET_MASK) != SRMMU_ET_PTD; }

static int srmmu_pmd_present(pmd_t pmd) { return ((pmd_val(pmd) & SRMMU_ET_MASK) == SRMMU_ET_PTD); }

static void srmmu_pmd_clear(pmd_t *pmdp) { srmmu_set_pte((pte_t *)pmdp, __pte(0)); }

static int srmmu_pgd_none(pgd_t pgd) { return !(pgd_val(pgd) & 0xFFFFFFF); }

static int srmmu_pgd_bad(pgd_t pgd) { return (pgd_val(pgd) & SRMMU_ET_MASK) != SRMMU_ET_PTD; }

static int srmmu_pgd_present(pgd_t pgd) { return ((pgd_val(pgd) & SRMMU_ET_MASK) == SRMMU_ET_PTD); }

static void srmmu_pgd_clear(pgd_t *pgdp) { srmmu_set_pte((pte_t *)pgdp, __pte(0)); }
