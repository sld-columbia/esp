...
char report_test_string[N_IDS_TEST][MAX_REPORT_STRING] = {
    "Test     start : ", "Leon3    test  : ", "Reg      test  : ", "Mul      test  : ",
    "Div      test  : ", "FPU      test  : ", "Fill B   test  : ", "Sharing  test  : ",
    "L2       test  : ", "Rand RW  test  : ", "Fill HW  test  : ", "MESI     test  : ",
    "Lock     test  : ", "Fill W   test  : ", "Test     end   : "};

char report_fail_string[N_IDS_FAIL][MAX_REPORT_STRING] = {
    "Reg      fail  : ", "Mul      fail  : ", "Div      fail  : ", "FPU      fail  : ",
    "Fill B   fail  : ", "Sharing  fail  : ", "Rand RW  fail  : ", "Fill HW  fail  : ",
    "MESI     fail  : ", "Lock     fail  : ", "Fill W   fail  : ", "MP       fail  : "};
...
{
    if (array_type == TEST) report_test_array[get_pid()][report_id] = 1;
    else
        report_fail_array[get_pid()][report_id] = 1;
}

void report_test(int report_id) { report(TEST, report_id); }

void report_fail(int report_id) { report(FAIL, report_id); }
...
void report_parse(int ncpu)
{
    ...
    for (j = 0; j < N_IDS_TEST; j++) {
        ...
        for (i = 0; i < ncpu; i++)
            if (report_test_array[i][j]) printf("cpu%d ", i);
