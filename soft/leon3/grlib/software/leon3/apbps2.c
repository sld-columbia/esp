...
int apbps2_test(int addr)
{
    ...
    if ((tmp >> APBPS2_S_RCNT_P) & 0x1F) { fail(0); }
    if ((tmp >> APBPS2_S_TCNT_P) & 0x1F) { fail(1); }
    if (tmp & APBPS2_S_TF) { fail(2); }
    if (tmp & APBPS2_S_RF) { fail(3); }
    if (tmp & APBPS2_S_KI) { fail(4); }
    if (tmp & APBPS2_S_FE) { fail(5); }
    if (tmp & APBPS2_S_PE) { fail(6); }
    if (tmp & APBPS2_S_DR) { fail(7); }
    ...
    for (i = 0; i < 2; i++) {
        ...
        while (!((reg->status >> APBPS2_S_RCNT_P) & 0x1F))
            ;
        tmp = reg->data;
        if ((i == 0 && tmp != 0x5A) || (i == 1 && tmp != 0xA5)) { fail(i); }
    }
