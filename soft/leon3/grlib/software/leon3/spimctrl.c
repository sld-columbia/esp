...
int spimctrl_test(unsigned int addr)
{
    ...
    if (regs->stat & SPIM_TO) fail(0);
    if (regs->stat & SPIM_ERR) fail(1);
    if (!(regs->stat & SPIM_INIT)) fail(2);
    if (regs->stat & SPIM_BUSY) fail(3);
    if (regs->stat & SPIM_DONE) fail(4);
    if (!(regs->ctrl & SPIM_CSN)) fail(5);
    if (regs->ctrl & SPIM_EAS) fail(6);
    if (regs->ctrl & SPIM_IEN) fail(7);
    if (regs->ctrl & SPIM_USRC) fail(8);
    ...
    if (regs->rx != 0xFF) fail(3);

    regs->ctrl = 0;

    if (regs->ctrl != SPIM_CSN) fail(4);
