...
void l34stat_test(unsigned int addr, unsigned char l3)
{
    ...
    if (l3) { report_device(0x01098000); }
    else {
    ...
    *ctrl = 0; /* Initialize before reading # counters */
    ...
    for (i = 0; i < ncnt; i++) {
        ...
        val[i]  = 0;
        ctrl[i] =
            (L4STAT_CTRL_EL | L4STAT_CTRL_CD | (L4STAT_CTRL_SU_MSK << L4STAT_CTRL_SU) |
             (L4STAT_CTRL_ID_MSK << L4STAT_CTRL_ID) | (L4STAT_CTRL_CPU_MSK << L4STAT_CTRL_CPU));
        tmp = ctrl[i];
        ...
        if (((tmp & (L4STAT_CTRL_EL | L4STAT_CTRL_CD)) && !(tmp & L4STAT_CTRL_MC)) ||
            ...
            fail(0);
        if (((tmp >> L4STAT_CTRL_SU) & L4STAT_CTRL_SU_MSK) != L4STAT_CTRL_SU_MSK) fail(1);
        ...
        if (((tmp & (L4STAT_CTRL_DS | L4STAT_CTRL_EE)) &&
            ...
            fail(4);
        if (((tmp >> L4STAT_CTRL_CPU) & L4STAT_CTRL_CPU_MSK) != L4STAT_CTRL_CPU_MSK) fail(5);
...
void l4stat_test(unsigned int addr) { l34stat_test(addr, 0); }

void l3stat_test(unsigned int addr) { l34stat_test(addr, 1); }
