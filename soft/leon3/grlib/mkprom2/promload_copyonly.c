...
void clean(paddr, len) double *paddr;
int len;
{
    len >>= 3;
    while (len >= 0) {
...
void mmov(raddr, paddr, len) int *raddr, *paddr;
int len;
{
    len >>= 2;
    while (len >= 0) {
...
void putsx(s) unsigned char *s;
{
    ...
    if (_iserc32) {
        ...
        if (((_prom_getpsr() >> 24) & 0x0f) != 3) { ubase = (unsigned int *)0x80000070; }
        else {
            ubase = _uaddr;
        }
        while (s[0] != 0) {
...
int main(void)
{
    ...
    while (sections[secnum].paddr) {
        if (sections[secnum].paddr != 0x800) {
            ...
            mmov(sections[secnum].raddr, sections[secnum].paddr, sections[secnum].len);
        }
        secnum++;
    }
    /* reset cwp to 0 */
