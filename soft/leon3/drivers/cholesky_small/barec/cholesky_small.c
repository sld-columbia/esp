...
#define SLD_CHOLESKY_SMALL 0x056
#define DEV_NAME "sld,cholesky_small"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_CHOLESKY_SMALL, DEV_NAME);
	if (ndev == 0) {
#ifndef __riscv
		printf("cholesky_small not found\n");
#else
		print_uart("cholesky_small not found\n");
#endif
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
		...
		/* <<--regs-config-->> */
		iowrite32(dev, CHOLESKY_SMALL_INPUT_ROWS_REG, input_rows);
		iowrite32(dev, CHOLESKY_SMALL_OUTPUT_ROWS_REG, output_rows);
