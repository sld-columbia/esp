...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_CHOLESKY_6X6, DEV_NAME);
	if (ndev == 0) {
#ifndef __riscv
		printf("cholesky_6x6 not found\n");
#else
		print_uart("cholesky_6x6 not found\n");
#endif
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
		...
		/* <<--regs-config-->> */
		iowrite32(dev, CHOLESKY_6X6_INPUT_ROWS_REG, input_rows);
		iowrite32(dev, CHOLESKY_6X6_OUTPUT_ROWS_REG, output_rows);
