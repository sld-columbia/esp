...
static inline struct gemm_device *to_gemm(struct esp_device *esp)
{
	return container_of(esp, struct gemm_device, esp);
}

static void gemm_prep_xfer(struct esp_device *esp, void *arg)
{
	struct gemm_access *a = arg;
...
{
	/* struct gemm_device *gemm = to_gemm(esp); */
	/* struct gemm_access *a = arg; */
...
static struct esp_driver gemm_driver = {
	...
	.ioctl_cm	= GEMM_IOC_ACCESS,
	.arg_size	= sizeof(struct gemm_access),
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("gemm driver");
