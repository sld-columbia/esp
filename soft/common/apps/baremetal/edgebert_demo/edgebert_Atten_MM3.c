...
static void EdgeBert_Attension_MM3 (struct esp_device *dev, struct esp_device *plic_dev, token_t *mem)

{
   ...
   key_mat_1  = aligned_malloc(2*N0*N1);
   vaule_mat_1 = aligned_malloc(2*N0*N1);
   memset(vaule_mat_1, 14, 2*N0*N1*sizeof(token_t));  
   ...
   N1 = 128;
   token_t *query_mat_2; //the input for softmax: 128X128
   query_mat_2 = aligned_malloc(N0*N1);  
   memset(query_mat_2, 28, N0*N1*sizeof(token_t));  
...
int main(int argc, char * argv[])
{
    ...
    attention_heads = aligned_malloc(128*768);
            ...
            EdgeBert_Attension_MM3 (&dev, &plic_dev, mem);
