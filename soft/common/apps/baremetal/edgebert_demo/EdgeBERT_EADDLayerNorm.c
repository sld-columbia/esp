...
int main(int argc, char * argv[])
{
    ...
    for (int i=0; i<12; i++)
        {
            ...
            EdgeBert_Attension (&dev, &plic_dev, mem);
    ...
    printf("\nSTARTing 12 Attention Heads Prcessing...\n"); 
    ...
    for (int i=0; i<2; i++)
   {
    ...
    {   
        ...
        EdgeBert_MatMul (&dev, &plic_dev, N0, N1, M_mat, is_relu, mem, Mask_mat, input_1, input_2, 0);
        //memcpy(We2_output + N0*N1*i*j, mem+Mask_buffer_size+2*input_buffer_size+aux_buffer_size, N0*N1*sizeof(token_t));
        ...
        for (int l=0; l<N0; l++)
            ...
            for (int k=0; k<N1; k++)
                {
                attention_head_out[k+(l+i*N0)*768 + j*N1] = mem[Mask_buffer_size+2*input_buffer_size+aux_buffer_size+k+l*N0];
                }
            }
    }
   ...
   printf("###(%"PRIu64" clock cycles)###\n", count2-count1);
    ...
    count1 = get_counter();
    EdgeBert_FeedForward (&dev, &plic_dev, mem, attention_out);
...
int main(int argc, char * argv[])
{
   ...
   unsigned M_mat;
    ...
    for (int i=0; i<12; i++)
        ...
        {   
            ...
            for (int k=0; k<64; k++)
                {
                //attention_heads[l*768+k+i*64] = mem[Mask_buffer_size+2*input_buffer_size+aux_buffer_size+k+l*64];
                attention_heads[l*768+k+i*64] = k+l*64;
                }
            }
    ...
    int *We_heads;
    We_heads = aligned_malloc(768*768*sizeof(int));
    int *attention_head_out;
    attention_head_out = aligned_malloc(128*768*sizeof(int));
    ...
    CPU_multiply(attention_heads, We_heads, N0, M_mat, N1, attention_head_out); 
