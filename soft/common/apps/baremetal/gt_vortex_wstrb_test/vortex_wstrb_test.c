...
/*
 * Bare-metal Vortex write-strobe test:
 * - input.h provides the initial memory/program image.
 * - Kernel writes patterned values to specific output words.
 * - Host samples those words before/after to confirm write behavior.
 */
...
{
 /* input.h embeds the full Vortex image for this scenario. */
 #include "input.h"
}
...
int main(int argc, char * argv[])
{
	...
	for (n = 0; n < ndev; n++) {
		...
		/* Output words checked for expected post-kernel write patterns. */
		output_1 = (token_t*)(mem_top+0x00001b58);
		output_2 = (token_t*)(mem_top+0x00001b54);
		output_3 = (token_t*)(mem_top+0x00001b50);
...
#else
		{
			...
			printf(" Value of pointer 1 in Vortex (expected 0xA) = %x \n", *(output_1));
			printf(" Value of pointer 2 in Vortex (expected 0xB) = %x \n", *(output_2));
			printf(" Value of pointer 3 in Vortex (expected 0xC) = %x \n", *(output_3));
			...
			// Set memory offset
			/* Program-visible memory base for Vortex load/store traffic. */
			...
			// START_VORTEX
			/* Start signal consumed by wrapper state machine. */
			...
			printf(" Value of pointer 1 in Vortex (expected 0xAAAAAAAA) = %x \n", *(output_1));
			printf(" Value of pointer 2 in Vortex (expected 0xBBBBBBBB) = %x \n", *(output_2));
			printf(" Value of pointer 3 in Vortex (expected 0xCCCCCCCC) = %x \n", *(output_3));
