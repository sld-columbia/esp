...
/*
 * Bare-metal Vortex smoke test flow:
 * 1) Copy a prebuilt Vortex memory image (from input.h) into DDR.
 * 2) Point the accelerator at that image via VX_BASE_ADDR.
 * 3) Pulse VX_SOFT_RESET to start Vortex and poll VX_BUSY_INT until done.
 * 4) Read result words from fixed offsets inside the same memory image.
 */
...
{
 /* input.h contains the byte array image for this test program + data. */
 #include "input.h"
}
...
int main(int argc, char * argv[])
{
	...
	out_offset  = 0;
	unsigned int _fibonacci_bin_len_in_words = 8096/4; 
	...
	for (n = 0; n < ndev; n++) {
		...
		printf("CPU Calculations Finished\n");
	    ...
	    input_n  =   (token_t*) (mem_top+0x1f98);
		*input_n = input_n_val; // Assigning input value to memory
		printf("  set input value to  = %d\n", *input_n);
			
		/* The kernel writes its result here in the pre-initialized image. */
		output_computed = (token_t*)(mem_top+0x1fa0);
...
#else
		{
			...
			// Set memory offset
			/* BASE_ADDR is the start of the shared Vortex memory window. */
			...
			// START_VORTEX
			/* Writing START_VORTEX triggers the reset/start sequence in the wrapper. */
			...
			/*
			 * Refresh cached DDR state before CPU-side validation so cache-enabled
			 * runs observe the data written back by Vortex.
			 */
			esp_flush(coh);
