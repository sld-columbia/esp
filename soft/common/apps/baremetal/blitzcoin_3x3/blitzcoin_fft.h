...
static void fft_init_buf(fft_token_t *in, fft_token_t *gold)
{
    ...
    if (!do_bitrev) fft_bit_reverse((float *)gold, len, log_len);
...
static void fft_probe(struct esp_device **espdevs_fft)
{
    ...
    if (ndev == 0) printf("fft not found\n");
    else
        printf("Found %d FFT instances\n", ndev);
}

void setup_fft(struct esp_device *dev, fft_token_t *gold_fft, fft_token_t *mem_fft,
               unsigned **ptable)
{
    int i;
