...
int main(int argc, char * argv[])
{
    ...
    espdev = &espdevs[0];
    write_config0(espdev, enable_const, max_tokens_EXP0[0], refresh_rate_min_const[0], refresh_rate_max_const[0]);
    espdev = &espdevs[2];
    write_config0(espdev, enable_const, max_tokens_EXP0[2], refresh_rate_min_const[2], refresh_rate_max_const[2]);
    ...
    write_config0(&espdevs[0], enable_const, 0, refresh_rate_min_const[0], refresh_rate_max_const[0]);
...
//Test running all viterbi, NVDLA and FFT tiles
   ...
   uint64_t cycles_start = 0, cycles_end_n0 = 0, cycles_end_f0 = 0,cycles_end_v0 = 0, cycles_end_f1 = 0, cycles_end_v1 = 0, cycles_end_f2 = 0;
   int ndev;
   ...
   for (i = 0; i < N_ACC; i++) {
       ...
       write_config3(espdev, pm_network_const[i]);
		if(i>0){
			write_config1(espdev, no_activity_const, random_rate_const, 0, token_counter_override[i]);
			write_config1(espdev, no_activity_const, random_rate_const, 0, 0);
		}
		else {
			write_config1(espdev, no_activity_const,random_rate_const_0, 0, token_counter_override[i]);
			write_config1(espdev, no_activity_const, random_rate_const_0, 0, 0);	
		}

       write_config0(espdev, enable_const, max_tokens[i], refresh_rate_min_const[i], refresh_rate_max_const[i]);
   }
   ...
   //////Viterbi setup//////
	...
	printf("  Viterbi Execution cycles : v0=%llu, v1=%llu\n", cycles_end_v0 - cycles_start,cycles_end_v1 - cycles_start);
	printf("  FFT Execution cycles : f0=%llu, f1=%llu, f2=%llu\n", cycles_end_f0 - cycles_start,cycles_end_f1 - cycles_start,cycles_end_f2 - cycles_start);
