...
/*
 * Bare-metal Vortex memory-pattern test:
 * - input.h seeds memory with test values and program code.
 * - Vortex mutates the seeded values.
 * - Host compares values before/after execution at fixed offsets.
 */
...
{
 /* input.h embeds the prebuilt program/data image loaded into DDR. */
 #include "input.h"
}
...
int main(int argc, char * argv[])
{
	...
	out_offset  = 0;
	unsigned int _mem_bin_len_in_words = 7040/4; 
	mem_size = _mem_bin_len_in_words * sizeof(token_t);
	unsigned int coh;
	...
	for (n = 0; n < ndev; n++) {
		...
		/* Test output slots inside the loaded image (64-bit patterns). */
		output_1 = (token_t_l*)(mem_top+0x00001b78);
		output_2 = (token_t_l*)(mem_top+0x00001b70);
		output_3 = (token_t_l*)(mem_top+0x00001b68);
		printf("**************** Memory Details ****************\n");
		printf("  memory buffer base-address = %x\n", (intptr_t)(mem));
		printf(" Last word address: %x \n", (intptr_t)(mem+_mem_bin_len_in_words-1));
...
#else
		{
			...
			printf(" Value of pointer 1 in Vortex (expected 0xA) = %llx \n", *(output_1));
			printf(" Value of pointer 2 in Vortex (expected 0xB) = %llx \n", *(output_2));
			printf(" Value of pointer 3 in Vortex (expected 0xC) = %llx \n", *(output_3));
			...
			// Set memory offset
			/* BASE_ADDR tells the wrapper where Vortex global memory starts. */
			...
			// START_VORTEX
			/* Kick Vortex; wrapper handles DCR writes + reset sequencing. */
			...
			printf(" Value of pointer 1 in Vortex (expected 0xAAAAAAAAAAAAAAAA) = %llx \n", *(output_1));
			printf(" Value of pointer 2 in Vortex (expected 0xBBBBBBBBBBBBBBBB) = %llx \n", *(output_2));
			printf(" Value of pointer 3 in Vortex (expected 0xCCCCCCCCCCCCCCCC) = %llx \n", *(output_3));
