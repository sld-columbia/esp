...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < mcast_ndests + 1; i++) {
        ...
        iowrite32(&devs[i], DST_OFFSET_REG, store_count * dummy_buf_size / (mcast_ndests + 1));
        ...
        if (i == MCAST_SRC) {
            ...
            iowrite32(&devs[i], NDESTS_REG, mcast_ndests);
        }
        else {
            iowrite32(&devs[i], SOURCE_REG, MCAST_SRC + 1);
