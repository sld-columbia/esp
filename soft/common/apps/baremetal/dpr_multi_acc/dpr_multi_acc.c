...
#endif
void spawn_hw_thread(struct esp_device *router_dev, int server_idx, int pbs_id, int new_div_sel_idx) {
...
int main(int argc, char * argv[])
{
    ...
    ndev = probe(&espdevs_tile_1, VENDOR_SLD, SLD_FFT, DEV_NAME_FFT);
    if (ndev == 0) {
    ...
    for (k = 0; k < 7; k++) {
        ...
        spawn_hw_thread(dev_tile_1, 0, ACC_CFG_IDX_FFT_STRATUS_2, k);
