...
#define SLD_SVHN_MLP 0x089
#define DEV_NAME "sld,svhn_mlp_hls4ml"

#define INT_BITS 22
#define fl2fx(A) float_to_fixed32(A, INT_BITS)

/* <<--params-->> */
const int32_t nbursts = 2;

static unsigned in_words = 1024;
static unsigned out_words = 10;
...
static int validate_buf(token_t *out, token_t *gold)
{
	...
	for (i = 1; i < nbursts; i++) {
		for (j = 0; j < out_words; j++) {
				...
				printf("%d %d : gold %d : output %d : error\n", i, j,
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_SVHN_MLP, DEV_NAME);
	if (ndev == 0) {
		printf("svhn_mlp not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
        ...
        /* <<--regs-config-->> */
        iowrite32(dev, SVHN_MLP_NBURSTS_REG, nbursts);
