...
static void fft_init_buf(fft_token_t *in, fft_token_t *gold, unsigned log_len)
{
	int j;
	const float LO = -10.0;
	const float HI = 10.0;
...
static unsigned fft_init_params(unsigned log_len)
{
    len = 1 << log_len;
...
void setup_fft(struct esp_device *dev, fft_token_t *gold_fft, fft_token_t *mem_fft, unsigned **ptable, unsigned log_len)
{
	int i; 
	printf("Setup FFT: log_len = %u\n", log_len);
