...
typedef void *(*trap_handler_t)(unsigned hartid, unsigned mcause, void *mepc, void *sp);
...
static inline int atomic_xchg(atomic_t *v, int n)
{
    ...
    __asm__ __volatile__("amoswap.w.aqrl %0, %2, %1" : "=r"(c), "+A"(v->counter) : "r"(n));
    return c;
}

static inline void mb(void) { __asm__ __volatile__("fence"); }
...
int main()
{
    ...
    while (1) {
        ...
        if (total != shared_count) {
            ...
            printf("count is %d:", shared_count);
            for (int i = 0; i < NHARTS; i++)
                printf(" %d", hart_count[i]);
            printf("\n");
            break;
        }
        if (shared_count % 5 == 0) {
