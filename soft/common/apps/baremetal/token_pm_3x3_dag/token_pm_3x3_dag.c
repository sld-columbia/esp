...
int main(int argc, char * argv[])
{
	...
	*cpu_dco_ptr = 0x5000D;
	...
	done_nvdla0 = 1;
	while (!(done_fft0 && done_nvdla0 && done_fft1 && done_viterbi0 && done_viterbi1 && done_fft2)) {
		...
		if ((done_viterbi1 && !done_viterbi1_before) || (done_fft1 && !done_fft1_before)) {
			if (done_viterbi1 && !done_viterbi1_before) {
				espdev = &espdevs[4];
				write_config1(espdev, 0, random_rate_const, 0, 0);
				is_running_viterbi1 = 0;
				cycles_end_viterbi1 = get_counter();
				#ifdef DEBUG
