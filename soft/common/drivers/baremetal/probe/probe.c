...
const char *const coherence_label[5] = {"non-coherent DMA", "LLC-coherent DMA", "coherent-DMA",
                                        "fully-coherent access", 0};
...
void *aligned_malloc(int size)
{
    ...
    void **ptr =
        (void **)((uintptr_t)(mem + CACHELINE_SIZE + sizeof(void *)) & ~(CACHELINE_SIZE - 1));
    ptr[-1] = mem;
    return ptr;
}
...
void esp_flush(int coherence)
{
    ...
    if (coherence == ACC_COH_NONE) /* Look for LLC controller */
        nllc = probe(&llcs, VENDOR_CACHE, DEVID_LLC_CACHE, DEVNAME_LLC_CACHE);

    if (coherence < ACC_COH_RECALL) /* Look for L2 controller */
...
static void esp_prop(const struct fdt_scan_prop *prop, void *extra)
{
    ...
    if (!strcmp(prop->name, "compatible") && fdt_string_list_index(prop, name) >= 0)
        ...
        (*espdevs)[ndev].irq =
            fdt_get_value(prop, 0); // TODO sending proper index instead of always 0
}
