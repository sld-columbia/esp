...
static void esp_flush_l1(void *info) { leon3_flush_dcache_all(); }
...
static int esp_private_cache_probe(struct platform_device *pdev)
{
    ...
    if (esp_private_cache == NULL) return -ENOMEM;
...
module_init(esp_private_cache_init) module_exit(esp_private_cache_exit)
