...
int esp_cache_flush()
{
    ...
    {
        if (mutex_lock_interruptible(&esp_cache->lock)) return -EINTR;
...
static int esp_cache_probe(struct platform_device *pdev)
{
    ...
    if (esp_cache == NULL) return -ENOMEM;
...
static void __exit esp_cache_exit(void) { platform_driver_unregister(&esp_cache_driver); }

module_init(esp_cache_init) module_exit(esp_cache_exit)
