...
#define esp_dma_mode_enable_dst(_dev, idx) iowrite32be(ioread32be(_dev->iomem + DMA_IDX_REG + _idx) | MODE_MASK_DST_IS_P2P, _dev->iomem + DMA_IDX_REG + _idx)
#define esp_dma_mode_enable_src(_dev, _idx) iowrite32be(ioread32be(_dev->iomem + DMA_IDX_REG + _idx) | MODE_MASK_SRC_IS_P2P, _dev->iomem + DMA_IDX_REG + _idx)
#define esp_dma_mode_set_y(_dev, _idx, _n, _y) iowrite32be(ioread32be(_dev->iomem + DMA_IDX_REG + _idx) | ((MODE_MASK_SRCS_YX & _y) << MODE_SHIFT_SRCS_Y(_n)), _dev->iomem + DMA_IDX_REG + _idx)
#define esp_dma_mode_set_x(_dev, _idx, _n, _x) iowrite32be(ioread32be(_dev->iomem + DMA_IDX_REG + _idx) | ((MODE_MASK_SRCS_YX & _x) << MODE_SHIFT_SRCS_X(_n)), _dev->iomem + DMA_IDX_REG + _idx)
