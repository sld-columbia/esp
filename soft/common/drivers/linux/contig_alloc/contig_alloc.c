...
static int contig_alloc_preferred(struct contig_desc *desc,
                                  const struct contig_alloc_params *params)
{
...
static bool least_loaded_alloc_ok(unsigned int n_chunks)
{
    int i;
    for (i = 0; i < nddr; i++)
        if (mem_size[i] - mem_allocated[i] >= n_chunks * chunk_size) return true;
    return false;
}
...
static int get_least_loaded_ddr_node(unsigned int n_chunks, unsigned int threshold)
{
    ...
    if (mem_allocated[0] + th < min_allocated && mem_size[0] - mem_allocated[0] >= tba) return 0;
    else
        return least_loaded;
}

static int contig_alloc_least_loaded(struct contig_desc *desc,
                                     const struct contig_alloc_params *params)
{
    unsigned long allocated = 0;
    int ddr_node;
    ...
    if (unlikely(!least_loaded_alloc_ok(desc->n))) return -ENOMEM;
...
static struct contig_desc *__contig_alloc_chunks(const struct contig_alloc_params *params,
                                                 unsigned int n_chunks)
{
    ...
    if (unlikely(IS_ERR(desc))) return desc;

    switch (params->policy) {
        case CONTIG_ALLOC_PREFERRED: rc = contig_alloc_preferred(desc, params); break;
        case CONTIG_ALLOC_LEAST_LOADED: rc = contig_alloc_least_loaded(desc, params); break;
        case CONTIG_ALLOC_BALANCED: rc = contig_alloc_balanced(desc, params); break;
        default: BUG();
    }
...
static struct contig_desc *__contig_alloc(const struct contig_alloc_params *params,
                                          unsigned long size)
{
    ...
    if (size > mem_free) return ERR_PTR(-ENOMEM);
...
struct contig_desc *contig_alloc(const struct contig_alloc_params *params, unsigned long size)
{
    ...
    if (unlikely(size == 0)) return ERR_PTR(-EINVAL);
...
static int __init contig_phys_alloc(int n_chunks)
{
    ...
    if (bp_buf == NULL) return -ENOMEM;
    return 0;
}

static void contig_phys_free(void) { bigphysarea_free(bp_buf, mem_size); }
...
static int __init contig_phys_alloc(int n_chunks) { return 0; }
...
static int __init contig_chunks_create(int ddr_node, int n_chunks)
{
    ...
    if (contig_phys_alloc(n_chunks)) return -ENOMEM;
    ...
    switch (params->policy) {
        ...
        case CONTIG_ALLOC_LEAST_LOADED:
            if (params->pol.lloaded.threshold < 0 ||
                params->pol.lloaded.threshold > mem_size[0] / chunk_size)
                return false;
            break;
        case CONTIG_ALLOC_BALANCED:
            if (params->pol.balanced.threshold < 0 ||
                params->pol.balanced.threshold > mem_size[0] / chunk_size)
                return false;
            if (params->pol.balanced.cluster_size < 1) return false;
            break;
...
static long contig_alloc_ioctl(struct file *file, void __user *arg)
{
    ...
    if (req.arr == NULL) return -EFAULT;

    if (!contig_alloc_ok(&req.params)) return -EINVAL;
...
{
    if (put_user(contig_chunk_size_log, (unsigned long __user *)arg)) return -EFAULT;
    return 0;
}
...
{
    switch (cm) {
        case CONTIG_IOC_ALLOC: return contig_alloc_ioctl(file, arg);
        case CONTIG_IOC_FREE: return contig_free_ioctl(file, arg);
        case CONTIG_IOC_CHUNK_LOG: return contig_chunk_size_ioctl(file, arg);
...
static int __init contig_create_file(void)
{
    ...
    if (IS_ERR(device_create(contig_class, NULL, MKDEV(CONTIG_MAJOR, CONTIG_MINOR), NULL,
                             "contig_alloc")))
...
static int __init contig_init(void)
{
    ...
    if (contig_chunk_size_log >= 32) return -EINVAL;
    chunk_size = BIT(contig_chunk_size_log);

#ifndef CONFIG_BIGPHYS_AREA
    if (!mem_start[0]) return -EINVAL;
#endif
    ...
    for (i = 0; i < nddr; i++) {
        ...
        if (mem_size[i] % chunk_size) {
            pr_warn(PFX "chunk_size (0x%lx) does not divide evenly mem_size[%d] (0x%lx); "
                        "discarding %ld bytes\n",
                    chunk_size, i, mem_size[i], mem_size[i] % chunk_size);
            mem_size[i] -= mem_size[i] % chunk_size;
        }
        if (!mem_size[i] || !chunk_size) return -EINVAL;
...
module_init(contig_init) module_exit(contig_exit)
