...
static int contig_init(void)
{
    ...
    if (likely(atomic_read(&fd))) return 0;
    ...
    if (atomic_read(&fd)) {
        if (close(local_fd) < 0) goto unlock;
    }
    else {
        atomic_set(&fd, local_fd);
    }

    if (ioctl(fd, CONTIG_IOC_CHUNK_LOG, &chunk_log) < 0) goto unlock;
...
void *contig_alloc(unsigned long size, contig_handle_t *handle)
{
    ...
    if (unlikely(contig_init())) return NULL;
    req = calloc(1, sizeof(*req));
    if (unlikely(req == NULL)) return NULL;
    req->n_max = DIV_ROUND_UP(size, chunk_size);

    req->arr = calloc(req->n_max, sizeof(*req->arr));
    if (unlikely(req->arr == NULL)) goto err_arr;
    ...
    if (ioctl(fd, CONTIG_IOC_ALLOC, req) < 0) goto err_ioctl;

    req->mm = mmap(NULL, req->n * chunk_size, flags, MAP_SHARED, fd, req->arr[0]);
    if (req->mm == MAP_FAILED) { goto err_mmap; }

    *handle = (contig_handle_t)req;
    return req->mm;

err_mmap:
    if (munmap(req->mm, chunk_size)) fprintf(stderr, PFX "munmap failed for %p\n", req->mm);
err_ioctl:
    free(req->arr);
err_arr:
    free(req);
...
static void contig_copy(contig_handle_t handle, unsigned long offset, void *vaddr,
                        unsigned long size, bool to_contig)
{
    struct contig_alloc_req *req = (struct contig_alloc_req *)handle;
    uint8_t *curr;
    uint8_t *fr;
    ...
    if (to_contig) memcpy(curr, fr, size);
    else
        memcpy(fr, curr, size);
}
