...
static void __dev_init(struct test_info *info, bool pr)
{
    ...
    if (info->set_access) info->set_access(info);
    info->esp->contig = contig_to_khandle(info->contig);
    info->esp->run    = true;
}

static void dev_init(struct test_info *info) { __dev_init(info, false); }

static void pr_file(const char *filename, const char *pfx, const char *description,
...
static int cmd_test(struct test_info *info)
{
    ...
    gettime(&th_end);
    if (rc < 0) die_errno("IOCTL on %s failed", info->devname);
    printf("HW done");
    if (info->to_float) {
...
static int cmd_run(struct test_info *info)
{
    ...
    if (rc < 0) die_errno("ESP_IOC_RUN on %s failed", info->devname);
...
static int cmd_flush(struct test_info *info)
{
    ...
    if (rc < 0) die_errno("ESP_IOC_FLUSH on %s failed", info->devname);
...
int test_main(struct test_info *info, const char *coh, const char *cmd)
{
    ...
    if (!strcmp(coh, "coh")) coherence = ACC_COH_FULL;
    ...
    if (!strcmp(cmd, "config")) return cmd_config(info);
