...
make_alias_links() {
	...
	for src in "$host_bin"/"${buildroot_prefix}"-*; do
		[ -e "$src" ] || continue
		base=$(basename "$src")
		# Buildroot's own internals, not tools to expose under the alias prefix.
		case $base in *.br_real) continue ;; esac
		tool=${base#"${buildroot_prefix}-"}
		wrapper="$alias_bin/${alias_prefix}-${tool}"
		rm -f "$wrapper"
		# Exec the Buildroot name, not its realpath. Most of these are symlinks
		# to toolchain-wrapper, which picks the real binary by appending
		# .br_real to the name it was invoked as. Resolving the symlink first
		# makes it look for toolchain-wrapper.br_real, which does not exist.
		cat > "$wrapper" <<EOF
#!/usr/bin/env bash
exec "$src" "\$@"
EOF
	...
	"$alias_bin/${alias_prefix}-gcc" --version >/dev/null 2>&1 || \
		die "created ${alias_prefix}-gcc in $alias_bin but it does not run"
}
