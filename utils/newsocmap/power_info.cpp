          ...
          power_info_db_t &db)
{
    ...
    for (unsigned i = 0; i < info.size(); i++) {
        ...
        parent_layout->addItem  (op->hsp,   power_line_number, 7 * i + 1, 1, 1, Qt::AlignRight);
        parent_layout->addWidget(op->MHz,   power_line_number, 7 * i + 2, 1, 1, Qt::AlignRight);
        parent_layout->addWidget(op->MHz_l, power_line_number, 7 * i + 3, 1, 1, Qt::AlignLeft);
        parent_layout->addWidget(op->V,     power_line_number, 7 * i + 4, 1, 1, Qt::AlignRight);
        parent_layout->addWidget(op->V_l,   power_line_number, 7 * i + 5, 1, 1, Qt::AlignLeft);
        parent_layout->addWidget(op->mW,    power_line_number, 7 * i + 6, 1, 1, Qt::AlignRight);
        parent_layout->addWidget(op->mW_l,  power_line_number, 7 * i + 7, 1, 1, Qt::AlignLeft);
    }
}
