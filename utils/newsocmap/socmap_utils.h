...
#define ECLKDOMAINS TOSTRING(Clock domain IDs must be contiguous and each clock domain must have one and only one PLL)
...
#define round_down(x, y) ((x) & ~__round_mask(x, y))
    ...
    typedef enum tile_enum {TILE_EMPTY = 0, TILE_ACC, TILE_CPU, TILE_MEM, TILE_MEMDBG, TILE_MISC, TILE_AXI} tile_t;
