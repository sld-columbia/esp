...
#define CTRLCFG6_CFG_CS_CHIP(x)	((x) & 0xffff)

#define SOCFPGA_FW_F2SDRAM0_DDR_SCR_ADDRESS	0xf8020200
...
#define FW_MPFE_SCR_FPGA2SDRAM_SIDEBANDMGR	0x18

static void f2sdram0_ddr_scr_writel(u32 data, u32 reg)
{
	writel(data, SOCFPGA_FW_F2SDRAM0_DDR_SCR_ADDRESS + reg);
}
...
phys_size_t sdram_calculate_size(struct altera_sdram_platdata *plat)
{
	...
	u32 ctrlcfg6 = hmc_readl(plat, CTRLCFG6);
	u32 cs_rank = CTRLCFG6_CFG_CS_CHIP(ctrlcfg6);
	u32 cs_addr_width;
...
void sdram_set_firewall(struct bd_info *bd)
{
	...
	/*
	 * ESP reaches DDR through the FPGA-to-SDRAM0 AXI port, which has its
	 * own DDR SCR window separate from the MPU/non-MPU DDR firewall above.
	 */
	for (i = 0; i < CONFIG_NR_DRAM_BANKS; i++) {
		if (!bd->bi_dram[i].size)
			continue;
		...
		lower = lower_32_bits(value);
		upper = upper_32_bits(value);
		f2sdram0_ddr_scr_writel(lower,
					FW_F2SDRAM_DDR_SCR_REGION0ADDR_BASE +
					(i * 4 * sizeof(u32)));
		f2sdram0_ddr_scr_writel(upper & 0xff,
		...
		f2sdram0_ddr_scr_writel(BIT(i), FW_F2SDRAM_DDR_SCR_EN_SET);
	}

	/*
	 * The MPFE sideband manager is secure-only after reset. SPL can touch
	 * it, but U-Boot proper runs non-secure and currently hangs when it
	 * probes the F2SDRAM sideband status registers.
	 */
	writel(FIREWALL_MPFE_SCR_DISABLE_MPU,
	       SOCFPGA_FW_MPFE_SCR_ADDRESS +
	       FW_MPFE_SCR_FPGA2SDRAM_SIDEBANDMGR);
#endif
}
