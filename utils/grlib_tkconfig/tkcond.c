...
static void mark_variables(struct kconfig *scfg)
{
    ...
    for (cfg = scfg; cfg != NULL; cfg = cfg->next) {
        if (cfg->token == token_bool || cfg->token == token_choice_item ||
            cfg->token == token_define_bool || cfg->token == token_define_hex ||
            cfg->token == token_define_int || cfg->token == token_define_string ||
            cfg->token == token_define_tristate || cfg->token == token_dep_bool ||
            cfg->token == token_dep_mbool || cfg->token == token_dep_tristate ||
            cfg->token == token_hex || cfg->token == token_int || cfg->token == token_string ||
            cfg->token == token_tristate || cfg->token == token_unset) {
...
static struct condition *remove_bang(struct condition *condition)
{
    ...
    for (conda = condition; conda; conda = conda->next) {
        if (conda->op == op_bang && conda->next && (condb = conda->next->next)) {
            if (condb->op == op_eq || condb->op == op_neq) {
                condb->op = (condb->op == op_eq) ? op_neq : op_eq;
                conda->op = op_nuked;
                if (prev) { prev->next = conda->next; }
                else {
 ...
 */
static struct condition *join_condition_stack(struct condition *conditions[], int depth)
{
    ...
    for (i = 0; i < depth; i++) {
        ...
        if (conditions[i]->op == op_true) continue;
        ...
        if (conditions[i]->op != op_lparen) {
            ...
            memset(cnew, 0, sizeof(*cnew));
            cnew->op = op_lparen;
            if (cond_last == NULL) { cond_list = cond_last = cnew; }
            else {
                cond_last->next = cnew;
                cond_last       = cnew;
            }
     ...
     */
    {
        ...
        for (cond1 = cond_list; cond1 != NULL; cond1 = cond1->next) {
            if (cond1->op == op_lparen) {
                cond1b = cond1->next;
                if (cond1b == NULL) break;
                cond1c = cond1b->next;
                if (cond1c == NULL) break;
                cond1d = cond1c->next;
                if (cond1d == NULL) break;
                cond1e = cond1d->next;
                if (cond1e == NULL) break;
                cond1f = cond1e->next;
                if (cond1f == NULL) break;

                if (cond1b->op == op_variable && (cond1c->op == op_eq || cond1c->op == op_neq) &&
                    cond1d->op == op_constant && cond1e->op == op_rparen) {
                    ...
                    for (cond2 = cond1f->next; cond2 != NULL; cond2 = cond2->next) {
                        if (cond2->op == op_lparen) {
                            cond2b = cond2->next;
                            if (cond2b == NULL) break;
                            cond2c = cond2b->next;
                            if (cond2c == NULL) break;
                            cond2d = cond2c->next;
                            if (cond2d == NULL) break;
                            cond2e = cond2d->next;
                            if (cond2e == NULL) break;
                            cond2f = cond2e->next;

                            /* look for match */
                            if (cond2b->op == op_variable &&
                                cond2b->nameindex == cond1b->nameindex &&
                                cond2c->op == cond1c->op && cond2d->op == op_constant &&
                                strcmp(cond2d->str, cond1d->str) == 0 && cond2e->op == op_rparen) {
                                ...
                                if (cond1f->op == op_and ||
                                    ...
                                    cond1d->op = op_nuked;
                                    cond1e->op = op_nuked;
                                    if (cond1f->op == op_and) cond1f->op = op_nuked;
                                    else
                                        cond2f->op = op_nuked;
                                }
                            }
...
static struct condition *eliminate_other_arch(struct condition *list)
{
    ...
    if (current_arch == NULL) current_arch = getenv("ARCH");
    if (current_arch == NULL) {
        fprintf(stderr, "error: ARCH undefined\n");
        exit(1);
    }
    if (cond1a->op == op_variable && !strcmp(vartable[cond1a->nameindex].name, "ARCH")) {
        cond1b = cond1a->next;
        if (cond1b == NULL) goto done;
        cond1c = cond1b->next;
        if (cond1c == NULL) goto done;
        cond1d = cond1c->next;
    ...
    if (cond1a->op == op_variable && !vartable[cond1a->nameindex].defined) {
        ...
        if (cond1c->op == op_constant && (cond1d == NULL || cond1d->op == op_and)) /*???*/
        {
     ...
     */
    {
        ...
        for (cfg = scfg; cfg != NULL; cfg = cfg->next) {
            ...
            switch (cfg->token) {
                ...
                case token_if:
                    cond_stack[depth++] = remove_bang(eliminate_other_arch(cfg->cond));
                    cfg->cond           = NULL;
                    break;
                ...
                case token_else: {
                    ...
                    for (cond = cond_stack[depth - 1]; cond != NULL; cond = cond->next) {
                        switch (cond->op) {
                            default: break;
