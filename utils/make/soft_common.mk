...
sysroot/opt/drivers-esp/contig_alloc.ko: linux-build/vmlinux $(wildcard $(CONTIG_ALLOC_PATH)/*.c) $(wildcard $(CONTIG_ALLOC_PATH)/*.h) $(wildcard $(DRIVERS)/include/*.h)
	...
	$(QUIET_MAKE)ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE_LINUX) DRIVERS=$(DRIVERS) KSRC=$(PWD)/linux-build $(MAKE) -C $(BUILD_DRIVERS)/contig_alloc
...
sysroot/opt/drivers-esp/esp_cache.ko: linux-build/vmlinux $(wildcard $(ESP_CORE_PATH)/../esp_cache/*.c) $(wildcard $(ESP_CORE_PATH)/../esp_cache/*.h) $(wildcard $(DRIVERS)/include/*.h)
	@mkdir -p sysroot/opt/drivers-esp
	$(QUIET_MAKE)ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE_LINUX) DRIVERS=$(DRIVERS) KSRC=$(PWD)/linux-build $(MAKE) -C $(BUILD_DRIVERS)/esp_cache
...
sysroot/opt/drivers-esp/esp.ko: linux-build/vmlinux $(wildcard $(ESP_CORE_PATH)/*.c) $(wildcard $(ESP_CORE_PATH)/*.h) $(wildcard $(DRIVERS)/include/*.h)
	@mkdir -p sysroot/opt/drivers-esp
	$(QUIET_MAKE)ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE_LINUX) DRIVERS=$(DRIVERS) KSRC=$(PWD)/linux-build $(MAKE) -C $(BUILD_DRIVERS)/esp
...
sysroot-update: linux-build/vmlinux socmap.vhd soft-build
	...
	@mkdir -p sysroot/applications/dvi; cp $(BUILD_DRIVERS)/dvi/linux/app/*.exe sysroot/applications/dvi
	@$(MAKE) acc-driver
	@$(MAKE) acc-app
	@touch sysroot
...
sysroot-clean:
	...
	@$(MAKE) --quiet -C $(DRIVERS)/dvi/linux/app clean DRIVERS=$(DRIVERS)
...
baremetal-all: soft-build
	@mkdir -p baremetal/dvi
	...
	@cp $(BUILD_DRIVERS)/dvi/baremetal/*.bin baremetal/dvi
	@$(MAKE) acc-baremetal

baremetal-distclean:
	$(QUIET_CLEAN)$(RM) baremetal
	@CPU_ARCH=$(CPU_ARCH) BUILD_PATH=$(BUILD_DRIVERS)/dvi/baremetal $(MAKE) --quiet -C $(DRIVERS)/dvi/baremetal clean
