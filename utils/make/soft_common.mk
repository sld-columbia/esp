...
$(BAREMETAL_APPS-baremetal): $(BAREMETAL_BIN) soft-build $(ESP_CFG_BUILD)/socmap.vhd
	...
	if [ `ls -1 $$BAREMETAL_APPS_PATH/*.c 2>/dev/null | wc -l ` -gt 0 ]; then \
		echo '   ' MAKE $@; \
		mkdir -p $$BUILD_PATH; \
		CROSS_COMPILE=$(CROSS_COMPILE_ELF) CPU_ARCH=$(CPU_ARCH) CONFIG_PRC_EN=$(CONFIG_PRC_EN) DRIVERS=$(DRV_BARE) DESIGN_PATH=$(DESIGN_PATH)/$(ESP_CFG_BUILD) BUILD_PATH=$$BUILD_PATH $(MAKE) -C  $$BAREMETAL_APPS_PATH; \
