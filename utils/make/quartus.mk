...
# Platform Designer ships in a different directory from the Quartus binaries.
# Prefer whatever is on PATH; otherwise fall back to Intel's own QSYS_ROOTDIR,
...
quartus-check-tools:
	@if ! command -v quartus_sh >/dev/null 2>&1; then \
		...
		false; \
	elif test -n "$(strip $(QUARTUS_QSYS))" && test -z "$(QSYS_GENERATE)"; then \
		echo $(SPACES)"ERROR: qsys-generate not found in PATH or QSYS_ROOTDIR"; \
		false; \
	elif test -n "$(strip $(QUARTUS_QSYS))" && test -z "$(QSYS_SCRIPT)"; then \
		echo $(SPACES)"ERROR: qsys-script not found in PATH or QSYS_ROOTDIR"; \
		false; \
	fi
	...
	@if test -n "$(strip $(QUARTUS_QSYS))"; then \
		...
		cd "$(QUARTUS_DIR)" && \
			$(PIPEFAIL_SHELL) -o pipefail -c '{ \
				...
				$(QSYS_SCRIPT) --qpf="$(QUARTUS_QPF)" --script="$(QUARTUS_QSYS_TCL_PATH)" || run_status=$$?; \
				if test "$$run_status" = "0"; then \
					qsys_status=0; \
					$(QSYS_GENERATE) --quartus-project="$(QUARTUS_QPF)" --rev="$(QUARTUS_REVISION)" --clear-output-directory "$(QUARTUS_QSYS)" --synthesis=VERILOG || qsys_status=$$?; \
