...
def print_wrap_sp(file1, module1, address1, word1, macro_name):
    ...
    if os.path.exists(file1):
        ...
        out_file.write("\n")
        out_file.write(
            "module %s (CLK0, A0, D0, Q0, WE0, WEM0, CE0); \n" %
            module1)
        out_file.write("\n")
...
def print_wrap_sp_tb(file1, module1, address1, word1):
    ...
    if os.path.exists(file1):
        ...
        out_file.write(
            "    reg [DATA_WIDTH-1 : 0] tmp_mem [0:2**ADDR_WIDTH-1]; \n")
        out_file.write("\n")
        ...
        out_file.write("            repeat (1) @(posedge clk);\n")
        out_file.write(
            "            if (i!=0 && rd_data !== tmp_mem[i-1]) begin\n")
        out_file.write(
            "                $display(\"Mismatch at time=%0t a=0x%0h exp=0x%0h got=0x%0h\", $time, i-1, tmp_mem[i-1], rd_data);\n")
...
def print_wrap_dp(file1, module1, address1, word1, macro_name):
    ...
    if os.path.exists(file1):
        ...
        out_file.write("\n")
        out_file.write(
            "module %s (CLK0, A0, D0, WE0, WEM0, Q0, CE0, \n" %
            module1)
...
def print_wrap_dp_tb(file1, module1, address1, word1):
    ...
    if os.path.exists(file1):
        ...
        out_file.write("    initial begin\n")
        out_file.write(
            "        {clk, ce0, we0, wem0, addr0, wr_data0} <= 0;\n")
        out_file.write("        errors <= 0;\n")
        out_file.write("\n")
        out_file.write("        repeat (2) @(posedge clk);\n")
        out_file.write("\n")
        ...
        out_file.write("            repeat (1) @(posedge clk);\n")
        out_file.write(
            "            if (i!=0 && rd_data1 !== tmp_mem[i-1]) begin\n")
        out_file.write(
            "                $display(\"Mismatch at time=%0t a=0x%0h exp=0x%0h got=0x%0h\", $time, i-1, tmp_mem[i-1], rd_data1);\n")
        out_file.write("                errors <= errors + 1;\n")
        out_file.write("            end\n")
        out_file.write("        end\n")
        out_file.write("\n")
    ...
    for idx in range(len(mem_dict["llc"])):
        if mem_dict["llc"][idx]["address_size"] in val_addr:
            if int(mem_dict["llc"][idx]["word_size"]) == 64:
                ...
                file_tb = out_path + "/tb/LLC_SRAM_SP_LINE_" + \
                    mem_dict["llc"][idx]["address_size"] + "x" + mem_dict["llc"][idx]["word_size"] + "_tb.sv"
                ...
                file = out_path + "/LLC_SRAM_SP_MIXED_" + \
                    mem_dict["llc"][idx]["address_size"] + "x" + mem_dict["llc"][idx]["word_size"] + ".v"
                file_tb = out_path + "/tb/LLC_SRAM_SP_MIXED_" + \
                    mem_dict["llc"][idx]["address_size"] + "x" + mem_dict["llc"][idx]["word_size"] + "_tb.sv"
                ...
                file = out_path + "/LLC_SRAM_SP_SHARED_" + \
                    mem_dict["llc"][idx]["address_size"] + "x" + mem_dict["llc"][idx]["word_size"] + ".v"
                file_tb = out_path + "/tb/LLC_SRAM_SP_SHARED_" + \
                    mem_dict["llc"][idx]["address_size"] + "x" + mem_dict["llc"][idx]["word_size"] + "_tb.sv"
    ...
    for idx in range(len(mem_dict["l2"])):
        if mem_dict["l2"][idx]["address_size"] in val_addr:
            if int(mem_dict["l2"][idx]["word_size"]) == 64:
                ...
                file_tb = out_path + "/tb/L2_SRAM_SP_LINE_" + \
                    mem_dict["l2"][idx]["address_size"] + "x" + mem_dict["l2"][idx]["word_size"] + "_tb.sv"
                ...
                file = out_path + "/L2_SRAM_SP_MIXED_" + \
                    mem_dict["l2"][idx]["address_size"] + "x" + mem_dict["l2"][idx]["word_size"] + ".v"
                file_tb = out_path + "/tb/L2_SRAM_SP_MIXED_" + \
                    mem_dict["l2"][idx]["address_size"] + "x" + mem_dict["l2"][idx]["word_size"] + "_tb.sv"
...
def slm_sp_gen(out_path, mem_dict, val_word):
    ...
    for idx in range(len(mem_dict["slm"])):
        if mem_dict["slm"][idx]["word_size"] in val_word:
            ...
            file_tb = out_path + "/tb/SLM_SRAM_SP_" + \
                mem_dict["slm"][idx]["address_size"] + "x" + mem_dict["slm"][idx]["word_size"] + "_tb.sv"
            module = "SLM_SRAM_SP_" + \
    ...
    for idx in range(len(mem_dict["io"])):
        if mem_dict["io"][idx]["port_type"] == "1rw1r" or mem_dict["io"][idx]["port_type"] == "2rw":
            if (mem_dict["io"][idx]["address_size"],
                ...
                file_tb = out_path + "/tb/IO_DP_" + \
                    mem_dict["io"][idx]["address_size"] + "x" + mem_dict["io"][idx]["word_size"] + "_tb.sv"
                module = "IO_DP_" + \
                ...
                if os.path.exists(file_tb):
                    ...
                    print("\"io_tb dual port %s\" generated!" %
                          (' '.join(map(str, list(mem_dict["io"][idx].values())))))
            ...
            if (mem_dict["io"][idx]["address_size"],
                ...
                if os.path.exists(file_tb):
                    ...
                    print_wrap_sp_tb(file_tb, module, address, word)
                    print("\"io_tb single port %s\" generated!" %
                          (' '.join(map(str, list(mem_dict["io"][idx].values())))))
...
def acc_gen(out_path, mem_dict):
    ...
    for idx in range(len(mem_dict["acc"])):
        if mem_dict["acc"][idx]["port_type"] == "2rw":
            ...
            file_tb = out_path + "/tb/ACC_SRAM_DP_" + \
                mem_dict["acc"][idx]["address_size"] + "x" + mem_dict["acc"][idx]["word_size"] + "_tb.sv"
            module = "ACC_SRAM_DP_" + \
            ...
            if os.path.exists(file_tb):
                ...
                print("\"acc_tb dual port %s\" generated!" %
                      (' '.join(map(str, list(mem_dict["acc"][idx].values())))))

        elif mem_dict["acc"][idx]["port_type"] == "1rw":
            file = out_path + "/ACC_SRAM_SP_" + \
                mem_dict["acc"][idx]["address_size"] + "x" + mem_dict["acc"][idx]["word_size"] + ".v"
            file_tb = out_path + "/tb/ACC_SRAM_SP_" + \
                mem_dict["acc"][idx]["address_size"] + "x" + mem_dict["acc"][idx]["word_size"] + "_tb.sv"
            module = "ACC_SRAM_SP_" + \
                mem_dict["acc"][idx]["address_size"] + "x" + mem_dict["acc"][idx]["word_size"]
            address = int(
            ...
            if os.path.exists(file_tb):
                ...
                print_wrap_sp_tb(file_tb, module, address, word)
                print("\"acc_tb single port %s\" generated!" %
                      (' '.join(map(str, list(mem_dict["acc"][idx].values())))))
