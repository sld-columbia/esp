...
def print_usage():
    ...
    print(
        "                         : <[2|4|8|16]>w      -> 2 parallel write operations with known (modulo) address pattern.")
    ...
    print(
        "                         : <[2|4|8|16]>r      -> parallel read operations with known (modulo) address pattern.")
    print("                                                 Data are distributed across banks and the number of banks must")
    print("                                                 be a power of two to have low-overhead bank-selection logic")
...
class memory():
    ...
    def print(self):
        ...
        print("        " + str(self.words) + " words, " +
              str(self.width) + " bits, " + operations)
    ...
    def __find_hbanks(self):
        for op in self.ops:
            ...
            # dual_port
            if single_port:
                if self.need_parallel_rw or (
                        op.wn == 2 and op.wp == "unknown"):
                    print(
                        "Requested memory " +
                        self.name +
                        " requires a dual-port SRAM bank, which is not available for the target technology")
                    sys.exit(0)
                self.need_dual_port = False
            else:
                self.need_dual_port = (
                    self.need_dual_port or self.need_parallel_rw or (
                        op.wn == 2 and op.wp == "unknown") or (
                        (not self.need_parallel_rw) and (
                            op.rn > 1 or op.wn > 1)))
        ...
        for op in self.ops:
            ...
            self.duplication_factor = max(
                self.duplication_factor, op_duplication_factor)
        ...
        for op in self.ops:
            ...
            self.distribution_factor = max(
                self.distribution_factor, op_distribution_factor)
    ...
    def __write_ctrl_assignment(
        ...
        fd.write(
            a_str[duplicated_bank_set][port] +
            str(iface) +
            bank_addr_range_str +
            ";\n")
        if is_write:
            fd.write("              if (hh != " +
                     str(self.hhbanks - 1) + ") begin\n")
            fd.write(
                d_str[duplicated_bank_set][port] +
                str(iface) +
                hh_range_str +
                ";\n")
            fd.write(
                wem_str[duplicated_bank_set][port] +
                str(iface) +
                hh_range_str +
                ";\n")
            fd.write("              end\n")
            ...
            fd.write(
                ...
                last_hh_range_str +
                ";\n")
            fd.write(wem_str[duplicated_bank_set][port] +
                     str(iface) + last_hh_range_str + ";\n")
            fd.write("              end\n")
    ...
    def write_verilog(self, out_path):
        ...
        for i in range(0, self.write_interfaces):
            ...
            fd.write("  " +
                     "input " +
                     "[" +
                     str(int(math.ceil(math.log(self.words, 2))) -
                         1) +
                     ":0] A" +
                     str(i) +
                     ";\n")
            fd.write("  " + "input " +
                     "[" + str(self.width - 1) + ":0] D" + str(i) + ";\n")
            fd.write("  " + "input WE" + str(i) + ";\n")
            fd.write("  " + "input " +
                     "[" + str(self.width - 1) + ":0] WEM" + str(i) + ";\n")
        for i in range(
                self.write_interfaces,
                self.write_interfaces +
                self.read_interfaces):
            ...
            fd.write("  " +
                     ...
                     ";\n")
            fd.write("  " + "output " +
                     "[" + str(self.width - 1) + ":0] Q" + str(i) + ";\n")
        fd.write("  genvar d, h, v, hh;\n")
        fd.write("\n")

        # Wire for banks
        bank_wire_addr_width = int(
            math.ceil(
                math.log(
                    self.bank_type.words,
                    2)))
        ...
        bank_wire_dims_str = "[" + str(self.dbanks - 1) + ":0]" + "[" + str(self.hbanks - 1) + ":0]" + "[" + str(
            self.vbanks - 1) + ":0]" + "[" + str(self.hhbanks - 1) + ":0]" + "[" + str(self.bank_type.ports - 1) + ":0]"
        ...
        ctrl_wire_dims_str = "[" + \
            str(self.write_interfaces + self.read_interfaces - 1) + ":0]"
        sel_reg_dims_str = "[" + str(self.write_interfaces +
                                     self.read_interfaces - 1) + ":" + str(self.write_interfaces) + "]"
        if self.dbanks > 1:
            sel_dbank_reg_width_str = "[" + \
                str(sel_dbank_reg_width - 1) + ":0]"
        else:
            sel_dbank_reg_width_str = "[0:0]"
        sel_dbank_reg_width_str = format(sel_dbank_reg_width_str, ">12")
        if self.hbanks > 1:
            sel_hbank_reg_width_str = "[" + \
                str(sel_hbank_reg_width - 1) + ":0]"
        else:
            sel_hbank_reg_width_str = "[0:0]"
        sel_hbank_reg_width_str = format(sel_hbank_reg_width_str, ">12")
        if self.vbanks > 1:
            sel_vbank_reg_width_str = "[" + \
                str(sel_vbank_reg_width - 1) + ":0]"
        else:
        ...
        for ri in range(
                ...
                self.write_interfaces +
                self.read_interfaces):
            # For ru type of operations we guarantee to insist on different
            # copies of the memory structure
            if self.dbanks > 1:
                fd.write("  assign ctrld[" + str(ri) +
                         "] = " + str(ri % self.dbanks) + ";\n")
            else:
                fd.write("  assign ctrld[" + str(ri) + "] = 0;\n")
        for ri in range(0, self.write_interfaces + self.read_interfaces):
            if self.hbanks > 1:
                fd.write("  assign ctrlh[" +
                         str(ri) +
                         "] = A" +
                         str(ri) +
                         "[" +
                         str(sel_hbank_reg_width -
                             1) +
                         ":" +
                         "0" +
                         "];\n")
        ...
        for ri in range(0, self.write_interfaces + self.read_interfaces):
            if self.vbanks > 1:
                fd.write("  assign ctrlv[" +
                         str(ri) +
                         "] = A" +
                         str(ri) +
                         "[" +
                         str(bank_wire_addr_width +
                             sel_hbank_reg_width +
                             sel_vbank_reg_width -
                             1) +
                         ":" +
        ...
        last_hh_msb_str = str((self.width - 1) %
                              bank_wire_data_width) + " + " + hh_lsb_str
        last_hh_lsb_str = hh_lsb_str
        last_hh_range_str = "[" + last_hh_msb_str + ":" + last_hh_lsb_str + "]"
        bank_addr_msb_str = str(min(int(math.ceil(
            math.log(self.words, 2))) - 1, bank_wire_addr_width + sel_hbank_reg_width - 1))
        bank_addr_lsb_str = str(sel_hbank_reg_width)
        bank_addr_range_str = "[" + bank_addr_msb_str + \
            ":" + bank_addr_lsb_str + "]"
        fd.write("  generate\n")
        fd.write("  for (h = 0; h < " + str(self.hbanks) +
                 "; h = h + 1) begin : gen_ctrl_hbanks\n")
        fd.write("    for (v = 0; v < " + str(self.vbanks) +
                 "; v = v + 1) begin : gen_ctrl_vbanks\n")
        fd.write("      for (hh = 0; hh < " + str(self.hhbanks) +
                 "; hh = hh + 1) begin : gen_ctrl_hhbanks\n")
        fd.write("\n")
        ...
        for op in self.ops:
            ...
            if op.wp == "unknown" and op.wn == 2:
                for d in range(0, self.dbanks):
                    fd.write(
                        "          // Duplicated bank set " + str(d) + "\n")
                    for wi in range(0, op.wn):
                        p = self.write_ports[wi]
                        self.__write_ctrl_assignment(
                            fd, bank_addr_range_str, hh_range_str, last_hh_range_str, d, p, wi, True, 0)
            ...
            if op.wn == 0 and op.rp == "modulo":
                # All duplicated sets would return the same data. 0 is correct
                # even with no duplication
                d = 0
                fd.write(
                    "          // Always choose duplicated bank set " +
                    str(d) +
                    "\n")
            ...
            # Handle <N>w:<M>r with N and M power of 2. In this case hbanks
            # matches max(op.rn, op.wn) foreach op in the list of operations
        ...
        fd.write("  generate\n")
        fd.write("  for (hh = 0; hh < " + str(self.hhbanks) +
                 "; hh = hh + 1) begin : gen_q_assign_hhbanks\n")
        q_last_hh_msb_str = str(
            int(min(self.width - 1, self.hhbanks * self.bank_type.width - 1)))
        ...
        for ri in range(
            ...
            fd.write("    if (hh == " +
                     str(self.hhbanks -
                         1) +
                     " && (hh + 1) * " +
                     str(self.bank_type.width) +
                     " > " +
                     str(self.width) +
                     ") begin : gen_q_assign_hhbanks_last_" +
                     str(ri) +
                     " \n ")
            fd.write("      assign Q" +
                     str(ri) +
                     ...
                     str((self.width -
                          1) %
                         self.bank_type.width) +
                     ":0];\n")
            fd.write(
                "    end else begin : gen_q_assign_hhbanks_others_" +
                str(ri) +
                " \n")
            fd.write(
                "      assign Q" +
        ...
        # Bank instances
        fd.write("  generate\n")
        fd.write("  for (d = 0; d < " + str(self.dbanks) +
                 "; d = d + 1) begin : gen_wires_dbanks\n")
        fd.write("    for (h = 0; h < " + str(self.hbanks) +
                 "; h = h + 1) begin : gen_wires_hbanks\n")
        fd.write("      for (v = 0; v < " + str(self.vbanks) +
                 "; v = v + 1) begin : gen_wires_vbanks\n")
        fd.write("        for (hh = 0; hh < " + str(self.hhbanks) +
                 "; hh = hh + 1) begin : gen_wires_hhbanks\n")
        fd.write("\n")
        ...
        for p in range(0, self.bank_type.ports):
            if p == 0:
                ...
                fd.write(",\n              .CLK" + str(p) + "(CLK)")
            fd.write(",\n              .CE" + str(p) +
                     "(bank_CE[d][h][v][hh][" + str(p) + "])")
            fd.write(",\n              .A" + str(p) +
                     "(bank_A[d][h][v][hh][" + str(p) + "])")
            fd.write(",\n              .D" + str(p) +
                     "(bank_D[d][h][v][hh][" + str(p) + "])")
            fd.write(",\n              .WE" + str(p) +
                     "(bank_WE[d][h][v][hh][" + str(p) + "])")
            fd.write(",\n              .WEM" + str(p) +
                     "(bank_WEM[d][h][v][hh][" + str(p) + "])")
            fd.write(",\n              .Q" + str(p) +
                     "(bank_Q[d][h][v][hh][" + str(p) + "])")
        fd.write("\n            );\n")
        fd.write("\n")
            ...
            if self.bank_type.ports == 1:
                ...
                if self.hbanks != 1:
                    self.write_ports.append(
                        (int(wi / self.hbanks) + (wi % self.bank_type.ports)) % self.bank_type.ports)
                else:
            ...
            if self.bank_type.ports == 1:
                ...
                if self.hbanks != 1:
                    self.read_ports.append(
                        (int(ri / self.hbanks) + (ri % self.bank_type.ports)) % self.bank_type.ports)
                else:
    ...
    def gen(self, lib):
        ...
        print("        " +
              "Write interfaces are assigned to ports " +
              str(self.write_ports))
        print("        " +
              "Read interfaces are assigned to ports " +
              str(self.read_ports))
    ...
    def write_tb(self, tb_path):
        ...
        if wumax != 0:
            ...
            # Check what's written with any read-only op (even if not in the
            # original list of ops)
            if rumax != 0:
        ...
        for op in tb_ops:
            fd.write("  $display(\"\");\n")
            fd.write(
                "  #500000 $display(\"* Testing parallel access " +
                str(op) +
                " *\");\n")
            ...
            if op.wn != 0:
                ...
                for addr in range(0, self.words):
                    wi = addr % op.wn
                    fd.write(
                        "  @ (posedge CLK) $display(\"Reset addr " + str(addr) + "\");\n")
            ...
            ri_step = 1
            # Reversing the interface order makes them access different banks
            # than the ones assigned for pattern modulo
            if op.wp == "unknown":
                wi_start = op.wn - 1
                wi_end = -1
                wi_step = -1
            ...
            while True:
                fd.write(
                    "  @ (posedge CLK) $display(\"Current waddr and raddr are " +
                    str(waddr) +
                    ", " +
                    str(raddr) +
                    "\");\n")
                if waddr >= self.words:
                    ...
                    if op.rn == 0:
                        fd.write("  $display(\"\");\n")
                        fd.write(
                            "  $display(\"--- End of Test " + str(op) + " PASSED ---\");\n")
                        fd.write("  $display(\"\");\n")
                        break
                if raddr >= self.words:
                    if op.rn != 0:
                    ...
                    if waddr < self.words:
                        if op.wn != 0:
                            ...
                            fd.write(
                                "  A" + str(wi) + " = " + str(waddr) + ";\n")
                            addr_width = int(
                                math.ceil(math.log(self.words, 2)))
                            data = waddr
                            if (addr_width > self.width):
                                data = waddr % (
                                    int(math.pow(2, self.width)) - 1)
                            if waddr % 2 != 0:
                                data = (
                                    data << (self.width - int(math.log(max(data, 1), 2)) - 1)) | data
                            data_str = str(self.width) + "'h" + \
                                format(data, format_str)
                            fd.write(
                                "  D" + str(wi) + " = " + data_str + ";\n")
                            fd.write("  WE" + str(wi) + " = 1'b1;\n")
                            fd.write("  WEM" + str(wi) +
                ...
                wcycle = wcycle + 1
                if op.wn != 0:
                    if waddr < (min(raddr + op.rn - 1, self.words - 1)
                                ) or (wcycle - 1) / rcycle <= math.ceil(op.rn / op.wn):
                        continue
                for ri in range(ri_start, ri_end, ri_step):
                    if raddr < self.words:
                        ...
                        if op.rn != 0:
                            fd.write("  CE" + str(ri) + " = 1'b0;\n")
                rcycle = rcycle + 1
                if raddr < (min(caddr + op.rn - 1, self.words - 1)
                            ) or rcycle - ccycle <= 1:
                    continue
                for ri in range(ri_start, ri_end, ri_step):
                    if caddr < self.words:
                        if op.rn != 0:
                            ...
                            fd.write("  #200 ;\n")
                            fd.write(
                                "  if (Q" + str(ri) + " != " + str(data_str) + ") begin\n")
                            fd.write(
                                "    $display(\"Memory failure on interface " +
                                str(ri) +
                                " at address " +
                                str(caddr) +
                                ": reading %h, but expecting %h\", Q" +
                                str(ri) +
                                ", " +
                                data_str +
                                ");\n")
                            ...
                            fd.write("  else begin\n")
                            fd.write(
                                "    $display(\"Memory read on interface " +
                                str(ri) +
                                " at address " +
                                str(caddr) +
                                ": reading %h\", Q" +
    ...
    def write_hpp(self):
        try:
            ...
            die_werr(e)
        print(
            "  INFO: Generating SystemC explicit memory definition for " +
            self.name)
        fd.write("//\n")
        fd.write("// Created with the ESP Memory Generator\n")
        fd.write("//\n")
        ...
        fd.write(
            "  " +
            self.name +
            "_t(const sc_module_name& name = sc_gen_unique_name(\"" +
            self.name +
            "\"))\n")
        fd.write("  : sc_module(name)\n")
        fd.write("  , clk(\"clk\")\n")
        for iface in range(1, self.read_interfaces +
                           self.write_interfaces + 1):
            fd.write("  , port" + str(iface) +
                     "(\"port" + str(iface) + "\")\n")
        fd.write("  {\n")
        ...
        for iface in range(1, self.read_interfaces +
                           self.write_interfaces + 1):
            # TODO: there is a bug in Stratus that prevents a port interface to
            # have a single dimension (so use [1] for now)
            fd.write(
                "  typedef " +
                self.name +
                "::port_" +
    ...
    def write_bdm(self, out_path, tech_path):
        ...
        fd.write("setNumAccessPorts " +
                 str(self.read_interfaces + self.write_interfaces) + "\n")
...
def parse_op(op, mem_words):
    ...
    if re.match(r'ru', read_pattern_abbrv, re.M | re.I):
        ...
        if not is_power2z(read_number):
            die_werr(
                "Operation " +
                op +
                " implies known access patter (modulo), but the number of accesses is not a power of 2")
    else:
    ...
    if re.match(r'wu', write_pattern_abbrv, re.M | re.I):
        ...
        if write_number > 2:
            die_werr(
                "Too many parallel write accesses with unknown pattern for " + op)
        if write_number == 2 and read_number != 0:
            die_werr(
                "2 parallel write accesses with unknown pattern for " +
                op +
                " have non-zero parallel read access")
...
def read_techfile(tech_path, sram_list, tile):
    global mem_delay
    global mem_setup
    global single_port
    try:
        fd = open(tech_path + "/" + tile + "_lib.txt", 'r')
    except IOError as e:
        die_werr(e)
    for line in fd:
        line.strip()
