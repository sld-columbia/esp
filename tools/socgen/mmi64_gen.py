...
def create_mmi64_regs(soc):
    ...
    mem_offset = ddr_offset + \
        ((num_llc - 1) * mem_mon_regs + mem_mon_regs) * soc.noc.monitor_mem.get()
    if soc.noc.monitor_inj.get() == 1:
        fp.write("#define NOC_INJECT_offset " + str(mem_offset) + "\n")
    inj_offset = mem_offset + \
        (((num_nocs - 1) * num_tiles) + num_tiles) * soc.noc.monitor_inj.get()
    if soc.noc.monitor_routers.get() == 1:
        fp.write("#define NOC_QUEUES_offset " + str(inj_offset) + "\n")
    routers_offset = inj_offset + ((num_nocs - 1) * num_tiles * directions + (
        (num_tiles - 1) * directions) + directions) * soc.noc.monitor_routers.get()
    if soc.noc.monitor_accelerators.get() == 1:
        fp.write("#define ACC_offset " + str(routers_offset) + "\n")
    accelerators_offset = routers_offset + \
        ((num_acc - 1) * acc_mon_regs + acc_mon_regs) * soc.noc.monitor_accelerators.get()
    if soc.noc.monitor_l2.get() == 1:
        fp.write("#define L2_offset " + str(accelerators_offset) + "\n")
    l2_offset = accelerators_offset + \
        ((num_l2 - 1) * cache_mon_regs + cache_mon_regs) * soc.noc.monitor_l2.get()
    if soc.noc.monitor_llc.get() == 1:
        fp.write("#define LLC_offset " + str(l2_offset) + "\n")
    llc_offset = l2_offset + \
        ((num_llc - 1) * cache_mon_regs + cache_mon_regs) * soc.noc.monitor_llc.get()
    if soc.noc.monitor_dvfs.get() == 1:
        fp.write("#define DVFS_offset " + str(llc_offset) + "\n")
    dvfs_offset = llc_offset + \
        ((num_tiles - 1) * vf_points + vf_points) * soc.noc.monitor_dvfs.get()

    fp.write("#define MONITOR_REG_COUNT " + str(dvfs_offset) + "\n")
    fp.write("#define MONITOR_RESET_offset " + str(dvfs_offset) + "\n")
    fp.write("#define MONITOR_WINDOW_SIZE_offset " +
             str(dvfs_offset + 1) + "\n")
