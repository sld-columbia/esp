...
class StyledComponents:
    ...
    def CheckBoxWithLabel(
        ...
        label = ctk.CTkLabel(
            check_frame, text=text, font=(
                "Arial", 11), width=140, anchor="w")
...
class SocConfigFrame:
    ...
    def __init__(self, soc, left_panel, main_frame):
        ...
        self.soc_config_label = StyledComponents.Header(
            self.soc_config_frame, "SoC Configuration", 0, 0)

        # Target Technology
        self.tech_label = StyledComponents.KeyLabel(
            self.soc_config_frame, "Target Technology", 1, 0)
        self.tech_value_frame = ctk.CTkFrame(self.soc_config_frame)
        self.tech_value_frame.grid(row=1, column=1, pady=5, padx=15)
        self.tech_value = StyledComponents.ValueLabel(
            self.tech_value_frame, self.soc.TECH)

        # FPGA Board
        self.fpga_board_text = self.soc.FPGA_BOARD[:22] + \
            (self.soc.FPGA_BOARD[22:] and ' ..')
        self.fpga_label = StyledComponents.KeyLabel(
            self.soc_config_frame, "FPGA Board", 2, 0)
        self.fpga_value_frame = ctk.CTkFrame(self.soc_config_frame)
        self.fpga_value_frame.grid(row=2, column=1, pady=5, padx=15)
        self.fpga_value = StyledComponents.ValueLabel(
        ...
        self.cpu_value_menu = StyledComponents.OptionMenu(
            self.cpu_value_frame,
            self.soc.CPU_ARCH,
            self.cpu_choices,
            command=main_frame.update_noc_config)

        # FPU
        self.fpu_label = StyledComponents.KeyLabel(
            self.soc_config_frame, "Floating-Point Unit", 4, 0)
        self.fpu_value_frame = ctk.CTkFrame(self.soc_config_frame)
        self.fpu_value_frame.grid(row=4, column=1, pady=5, padx=15)
        self.fpu_value = StyledComponents.ValueLabel(
            self.fpu_value_frame, "None")
        self.set_cpu_specific_labels(self.soc)

        # Data Allocation Strategy
        self.das_label = StyledComponents.KeyLabel(
            self.soc_config_frame, "Data Allocation Strategy", 5, 0)
        self.das_value_frame = ctk.CTkFrame(self.soc_config_frame)
        self.das_value_frame.grid(row=5, column=1, pady=5, padx=15)
        self.das_value_menu = StyledComponents.OptionMenu(
            self.das_value_frame, self.soc.transfers, [
                "Big physical area", "Scatter/Gather"])

        # SLM KB per Tile
        self.slm_kbytes_choices = [
            "64", "128", "256", "512", "1024", "2048", "4096"]
        self.slm_label = StyledComponents.KeyLabel(
            self.soc_config_frame, "SLM KB Per Tile", 6, 0)
        self.slm_value_frame = ctk.CTkFrame(self.soc_config_frame)
        self.slm_value_frame.grid(row=6, column=1, pady=(5, 20), padx=15)
        self.slm_value_menu = StyledComponents.OptionMenu(
            self.slm_value_frame,
            self.soc.slm_kbytes,
            self.slm_kbytes_choices,
            command=main_frame.update_noc_config)
...
class PeripheralsConfigFrame:
    def __init__(self, soc, left_panel, main_frame):
        ...
        self.peripherals_frame = StyledComponents.Panel(
            self.left_panel, fg_color="#ebebeb")
        self.peripherals_frame.pack(fill="x", padx=(8, 3), pady=(0, 10))
        self.peripherals_label = StyledComponents.Header(
            self.peripherals_frame, "Peripherals", 0, 0)

        # UART
        self.uart_label, self.uart_checkbox = StyledComponents.CheckBoxWithLabel(
            self.peripherals_frame, None, "UART", "disabled", 1, 0)
        self.uart_checkbox.select()

        # JTAGssh
        self.jtag_label, self.jtag_checkbox = StyledComponents.CheckBoxWithLabel(
            self.peripherals_frame, self.soc.jtag_en, "SoCProbe", "normal", 1, 1, command=main_frame.update_noc_config)
        if soc.TECH_TYPE == "asic" or soc.TECH == "inferred" or soc.ESP_EMU_TECH != "none":
            ...
            self.jtag_checkbox.configure(state="disabled")
            self.jtag_label.configure(text_color="grey")

        # Ethernet
        self.eth_label, self.eth_checkbox = StyledComponents.CheckBoxWithLabel(
            self.peripherals_frame, self.soc.eth_en, "Ethernet", "normal", 2, 0, command=main_frame.update_noc_config)
        if not (soc.TECH_TYPE == "asic" or soc.TECH ==
                "inferred" or soc.ESP_EMU_TECH != "none"):
            self.eth_checkbox.select()
            self.eth_checkbox.configure(state="disabled")

        # Custom IO Link
        iolink_width_choices = ["8", "16", "32"]
        self.custom_io_label, self.custom_io_checkbox = StyledComponents.CheckBoxWithLabel(
            self.peripherals_frame, self.soc.iolink_en, "Custom IO Link", "normal", 4, 0, command=main_frame.update_noc_config)
        if soc.TECH_TYPE == "asic" or soc.TECH == "inferred" or soc.ESP_EMU_TECH != "none":
            self.menu_frame = ctk.CTkFrame(
                self.peripherals_frame, fg_color="transparent")
            self.menu_frame.grid(row=4, column=1, padx=(20, 10))
            self.custom_io_menu = ctk.CTkOptionMenu(
                self.menu_frame,
                values=iolink_width_choices,
                variable=self.soc.iolink_width,
                fg_color="white",
                bg_color="#e8e8e8",
                button_color="#e8e8e8",
                width=200,
        ...
        self.fpga_mem_label = StyledComponents.KeyLabel(
            self.peripherals_frame, "FPGA Memory Link", 3, 0, font=("Arial", 11))
        if soc.TECH_TYPE == "asic" or soc.TECH == "inferred" or soc.ESP_EMU_TECH != "none":
            self.mem_menu_frame = ctk.CTkFrame(
                self.peripherals_frame, fg_color="transparent")
            self.mem_menu_frame.grid(row=3, column=1, padx=(20, 10))
            self.fpga_mem_menu = ctk.CTkOptionMenu(
                self.mem_menu_frame,
                values=mem_link_width_choices,
                variable=self.soc.mem_link_width,
                fg_color="white",
                bg_color="#e8e8e8",
                button_color="#e8e8e8",
                width=200,
                ...
                dropdown_hover_color="#e8e8e8",
                command=main_frame.update_noc_config)
            self.fpga_mem_menu.pack()
        else:
            self.fpga_mem_label.configure(text_color="grey")

        # SVGA
        self.svga_label, self.svga_checkbox = StyledComponents.CheckBoxWithLabel(
            self.peripherals_frame, self.soc.svga_en, "SVGA", "normal", 2, 1, command=main_frame.update_noc_config)
        if not (soc.FPGA_BOARD.find("profpga") != -
                1 and (soc.TECH == "virtex7" or soc.TECH == "virtexu")):
...
class DebugLinkConfigFrame:
    def __init__(self, soc, left_panel):
        ...
        self.debug_link_frame = ctk.CTkFrame(
            self.left_panel, fg_color="#ebebeb")
        self.debug_link_frame.pack(fill="x", padx=(8, 3), pady=10)
        self.debug_link_label = StyledComponents.Header(
            self.debug_link_frame, "Debug Link", 0, 0)

        # IP Address (dec)
        self.ip_addr_label = StyledComponents.KeyLabel(
            self.debug_link_frame, "IP Address (dec)", 1, 0)
        self.ip_addr_value_frame = ctk.CTkFrame(self.debug_link_frame)
        self.ip_addr_value_frame.grid(row=1, column=1, pady=5, padx=15)
        self.ip_addr_value = StyledComponents.ValueLabel(
            self.ip_addr_value_frame, self.soc.IP_ADDR)

        # IP Address (hex)
        self.ip_addr_label = StyledComponents.KeyLabel(
            self.debug_link_frame, "IP Address (hex)", 2, 0)
        self.ip_addr_value_frame = ctk.CTkFrame(self.debug_link_frame)
        self.ip_addr_value_frame.grid(row=2, column=1, pady=5, padx=15)
        self.ip_addr_value = StyledComponents.Entry(
            self.ip_addr_value_frame, "")

        # MAC Address (hex)
        self.mac_addr_label = StyledComponents.KeyLabel(
            self.debug_link_frame, "MAC Address (hex)", 3, 0)
        self.mac_addr_value_frame = ctk.CTkFrame(self.debug_link_frame)
        self.mac_addr_value_frame.grid(row=3, column=1, pady=5, padx=15)
        self.mac_addr_value = StyledComponents.Entry(
            self.mac_addr_value_frame, "")
...
class CachesConfigFrame:
    def __init__(self, soc, left_panel, main_frame):
        ...
        self.title_label = StyledComponents.Header(
            self.caches_frame, "Caches Configuration", 0, 0)

        self.enable_caches_cb = ctk.CTkCheckBox(
            self.caches_frame,
            variable=self.soc.cache_en,
            text="Enable Caches",
            fg_color="green",
            border_color="grey",
            width=0,
        ...
        self.esp_rtl_dd = StyledComponents.OptionMenu(
            self.esp_rtl_frame,
            self.soc.cache_impl,
            self.cache_choices,
            width=160,
        ...
        self.cache_line_dd = StyledComponents.OptionMenu(
            self.cache_line_frame,
            self.soc.cache_line_size,
            self.cache_line_choices,
            width=160,
            command=main_frame.update_noc_config)
...
class EspCreator:
    def __init__(self, root, _soc):
        ...
        self.main_frame.grid_columnconfigure(
            0, weight=0)  # Fixed width for left panel
        self.main_frame.grid_columnconfigure(
            1, weight=1)  # Expandable for right panel
        ...
        self.right_canvas.create_window(
            (0, 0), window=self.right_panel_frame, anchor="nw")
        ...
        self.peripherals_config_frame = PeripheralsConfigFrame(
            self.soc, self.left_panel, self)
        self.debug_link_config_frame = DebugLinkConfigFrame(
        ...
        self.noc_config_frame = NoCConfigFrame(
            self.soc, self.left_panel, self.right_panel_frame)

        self.message_frame = ctk.CTkFrame(self.left_panel, fg_color="#ebebeb")
        self.message_frame.pack(padx=(8, 3), pady=(10, 20), fill="x")
        self.title_label = ctk.CTkLabel(
            self.message_frame, text="Messages", font=(
                "Arial", 10, "bold"))
        self.title_label.pack(fill="x", expand=True, pady=10)
        self.message = ctk.CTkTextbox(
            self.message_frame, height=150, wrap=WORD)
        self.message.pack(fill="x", expand=True)

        self.gen_soc_config = ctk.CTkButton(
            self.left_panel, text="Generate SoC Configuration", font=(
                "Arial", 12), height=40, command=self.generate_files)
    ...
    def generate_files(self):
        ...
        if os.path.isfile(".esp_config.bak"):
            shutil.move(".esp_config.bak", ".esp_config")

    def generate_socmap(self):
        self.soc.write_config(
            self.debug_link_config_frame.ip_addr_value.get(),
            self.debug_link_config_frame.mac_addr_value.get())
...
def on_closing():
    if messagebox.askokcancel("Quit", "Do you want to quit?"):
        soc.write_config(
            app.debug_link_config_frame.ip_addr_value.get(),
            app.debug_link_config_frame.mac_addr_value.get())
        root.destroy()
