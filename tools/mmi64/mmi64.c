...
int read_accelerators_mon(mmi64_module_t *user_module)
{
    ...
    for (i = 0; i < ACCS_NUM; i++)
        if (read_accelerator_mon(user_module, i)) relevant = 1;
    return relevant;
}
#endif
...
void read_mem_stats(mmi64_module_t *user_module, int mem_id)
{
    ...
    SET_GRN(fp);
    fprintf(fp, "%d\t%d\t%d\t%d\t", coh_req, coh_fwd, coh_rsp_rcv, coh_rsp_snd);

    SET_RED(fp);
    fprintf(fp, "%d\t%d\t", dma_req, dma_rsp);

    SET_YEL(fp);
    fprintf(fp, "%d\t%d\t", coh_dma_req, coh_dma_rsp);
...
void read_dvfs(mmi64_module_t *user_module)
{
    ...
    for (k = 0; k < TILES_NUM; k++)
        for (i = 0; i < VF_OP_POINTS; i++) {
            if (rdata[k][i] != 0 && tile_has_dvfs[k]) relevant_window = 1;
            fprintf(fp, "%d\t", rdata[k][i]);
        }
}
#endif
...
mmi64_error_t mmi64_main(int argc, char *argv[])
{
    ...
    if (status != E_PROFPGA_OK) { // cannot use NOW() macro because required MMI-64 domain handle
    ...
    for (i = 0; i < LLCS_NUM; i++)
        fprintf(fp,
                "coh-req-%d\tcoh-fwd-%d\tcoh_rsp_rcv-%d\tcoh_rsp_snd-%d\tdma_req-%d\tdma_rsp-%"
                "d\tcoh_dma_req-%d\tcoh_dma_rsp-%d\t",
                i, i, i, i, i, i, i, i);
#endif
