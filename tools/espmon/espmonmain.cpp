...
EspMonMain::EspMonMain(QWidget *parent) :
    QMainWindow(parent), ui(new Ui::EspMonMain), mmi(new mmi64_mon)
{
    ui->setupUi(this);
...
EspMonMain::~EspMonMain()
{
    ...
    if (mmi_is_open) status = mmi->close_system();
...
void EspMonMain::show_soc()
{
    for (int i = 0; i < TILES_NUM; i++) {
        ...
        if (t->type == (int)cpu_tile) { color_html = "#fd8276"; }
        ...
        for (int x = 0; x < XLEN * 6; x++)
            for (int h = 0; h < NOCS_NUM; h++) {
                ...
                if ((x0 % (6 * NOCS_NUM)) < (2 * NOCS_NUM)) {
                    if ((y0 % 2) == 0) layout_heat->addWidget(w1, y0, x0 + 1, 1, 1);
                    else
                        layout_heat->addWidget(w1, y0, x0 - 1, 1, 1);
                }
                else if ((x0 % (6 * NOCS_NUM)) < (4 * NOCS_NUM)) {
                    if ((y0 % 2) == 0) layout_heat->addWidget(w1, y0 + 1, x0, 1, 1);
                    else
...
void EspMonMain::show_dvfs()
{
    ...
    for (int i = 0; i < TILES_NUM; i++) {
        const struct tile_info *t = &tiles[i];
        if (t->type != accelerator_tile) continue;
...
#endif
    if (ui->check_mmi_acc->isChecked() || ui->check_mmi_dvfs->isChecked())
...
bool EspMonMain::read_probes()
{
    ...
    if (status != E_MMI64_OK) ErrorMessage(mmi64_strerror(status));

    if (current_time == mmi->current_time) return false;
...
static QColor get_color_heatmap(unsigned long long probe, float max)
{
    ...
    if (rate >= 100) return heat_1000;
    if (rate > 87.5) return heat_875;
    if (rate > 75) return heat_750;
    if (rate > 62.5) return heat_625;
    if (rate > 50) return heat_500;
    if (rate > 37.5) return heat_375;
    if (rate > 25) return heat_250;
    if (rate > 0) return heat_00;
    return heat_no;
#else
    if (rate > 90) return heat_90;
    if (rate > 80) return heat_80;
    if (rate > 70) return heat_70;
    if (rate > 60) return heat_60;
    if (rate > 50) return heat_50;
    if (rate > 40) return heat_40;
    if (rate > 30) return heat_30;
    if (rate > 20) return heat_20;
    if (rate > 10) return heat_10;
    if (rate > 0) return heat_00;
    return heat_no;
#endif
}
...
#ifdef NOC_QUEUES_offset
void EspMonMain::update_heatmap(int noc, float max,
                                uint32_t (&probes_queue)[NOCS_NUM][TILES_NUM][DIRECTIONS])
{
    ...
    for (int i = 0; i < TILES_NUM; i++) {
        ...
        if (t->position.y != 0) {
            ...
            router_is_active[noc][(t->position.y) * XLEN + t->position.x] +=
                probes_queue[noc][i][0];
            for (int y = 1; y < N - 1; y++) {
                QPalette Pal(palette());
    ...
    for (int i = 0; i < TILES_NUM; i++) {
        ...
        QColor col;
        if (noc % NOCS_NUM == 0) col = QColor("#bfbfbf");
        else
            col = QColor("#7f7f7f");
...
void EspMonMain::update_probes()
{
    ...
    if (first_sample) {
        ...
        if (new_time > current_time + 1) ui->label_mmi_warn->setText("Data loss!");
    }
    ...
    if (ui->check_mmi_dvfs->isChecked()) {
        for (int k = 0; k < TILES_NUM; k++) {
            ...
            if (pow > power_max[k]) power_max[k] = pow;

            assert(max != 0);
            float bar_value = pow / max;
            if (bar_value != 0 && bar_value < 1) bar_value = 1.0;
    ...
    if (ui->check_mmi_acc->isChecked()) {
        for (int k = 0; k < ACCS_NUM; k++) {
            ...
            if (mem_compute > mem_compute_max[k]) mem_compute_max[k] = mem_compute;
            if (exec_compute > exec_compute_max[k]) exec_compute_max[k] = exec_compute;
        ...
        if (mon->read_probes()) {
            if (mon->mmi->relevant_sample() || !auto_start) {
                ...
                if (relevant_sample) {
                    ...
                    if (no_activity_counter == 0) break;
                }
            }
        }
    }
...
{
    mmi_is_running = false;
    if (ui->check_mmi_auto->isChecked()) { ui->check_mmi_auto->setChecked(false); }
    else {
...
void EspMonMain::get_statistics()
{
    ...
    if (ui->check_mmi_dvfs->isChecked()) {
        ...
        for (int k = 0; k < TILES_NUM; k++) {
            ...
            if (t->type != accelerator_tile) continue;
            const float pow_max = energy_weight[k][VF_OP_POINTS - 1] / period[k][VF_OP_POINTS - 1];
            if (ave_not_max) pow = power_ave[k];
            else
                pow = power_max[k];
            ...
            float exec_time =
                (mmi->current_time - mmi->sample_start) * (1 << ui->dial_mmi_win->value());
            rpt_max << k << ". " << t->name << ": ";
            rpt_ave << k << ". " << t->name << ": ";
            rpt_max << std::setprecision(5) << "  " << (power_max[k] / max) * pow_max / 100.0
                    << "mW" << std::endl;
            rpt_ave << std::setprecision(5) << "  " << (power_ave[k] / max) * pow_max / 100.0
                    << "mW, ";
            float en = exec_time * (power_ave[k] / max) * pow_max / 100.0;
            rpt_ave << std::setprecision(5) << "  " << en << "pJ" << std::endl;
            total_energy += en;
        }
        rpt_ave << " ** Total Energy: " << std::setprecision(5) << "  " << total_energy << "pj **"
                << std::endl;
    }
    ...
    if (ui->check_mmi_traffic->isChecked()) {
        for (int h = 0; h < NOCS_NUM; h++) {
            if (ave_not_max) update_heatmap(h, max, mmi->probes_queue_ave);
            else
            ...
            for (int k = 0; k < TILES_NUM; k++) {
                ...
                for (int i = 0; i < DIRECTIONS; i++) {
                    rpt_max << std::setprecision(5)
                            << (100.0 * mmi->probes_queue_max[h][k][i]) / max << "%, ";
                    rpt_ave << std::setprecision(5)
                            << (100.0 * mmi->probes_queue_ave[h][k][i]) / max << "%, ";
        ...
        global_ave_bandwidth /=
            ((NOCS_NUM * TILES_NUM * DIRECTIONS) - (2 * (XLEN + YLEN) * NOCS_NUM));
        rpt_ave << " ** Average Bandwidth: " << std::setprecision(5) << "  " << global_ave_bandwidth
                << "% **" << std::endl;
    }
    rpt_max << std::endl;
    rpt_ave << std::endl;
    ...
    if (ui->check_mmi_acc->isChecked()) {
        ...
        for (int k = 0; k < TILES_NUM; k++) {
            ...
            if (ave_not_max) pbar_acc[accelerator]->setValue(mem_compute_ave[accelerator]);
            else
        ...
        rpt_max << std::endl
                << "=== Samping time ==="
                << (mmi->current_time - mmi->sample_start) * (1 << ui->dial_mmi_win->value()) * 10
                << " ns" << std::endl;
        rpt_ave << std::endl
                << "=== Samping time ==="
                << (mmi->current_time - mmi->sample_start) * (1 << ui->dial_mmi_win->value()) * 10
    ...
    if (ave_not_max) print_statistics(file_name_ave.str().c_str());
    else
