...
mmi64_error_t mmi64_mon::read_dvfs()
{
    ...
    for (int k = 0; k < TILES_NUM; k++)
        for (int i = 0; i < VF_OP_POINTS; i++) {
            ...
            if (tiles[k].type == accelerator_tile && probes_dvfs[k][i]) dvfs_relevant_sample = true;
        }
    return E_MMI64_OK;
}
#endif
...
mmi64_error_t mmi64_mon::read_acc(int accelerator)
{
    ...
    if (tlb > probes_acc_max[accelerator][0]) probes_acc_max[accelerator][0] = tlb;
    if (mem > probes_acc_max[accelerator][1]) probes_acc_max[accelerator][1] = mem;
    if (tot > probes_acc_max[accelerator][2]) probes_acc_max[accelerator][2] = tot;

    if (tot) acc_relevant_sample = true;
    ...
    for (k = 0; k < NOCS_NUM; k++)
        for (i = 0; i < TILES_NUM; i++)
            for (j = 0; j < DIRECTIONS - 1; j++) {
                ...
                unsigned global_lsb_bit  = k * bits_per_sample * (DIRECTIONS - 1) * TILES_NUM +
                    i * bits_per_sample * (DIRECTIONS - 1) + j * bits_per_sample;
                ...
                if (lsb_part == msb_part) {
                    ...
                    value              = (probes_signature[lsb_part] >> lsb_bit) |
                        ((probes_signature[msb_part] & msb_mask) << msb_shift);
                }
                ...
                probes_queue_ave[k][i][j] =
                    probes_queue_tot[k][i][j] / (current_time - sample_start);
