...
static long int parse_int(char *str)
{
    ...
    if (str[1] == 'x' || str[1] == 'X') val = strtol(str, &endptr, 16);
    else
...
static struct option long_options[] = {
    {"infile", required_argument, 0, 'i'},     {"outfile", required_argument, 0, 'o'},
    {"address", required_argument, 0, 'a'},    {"size", required_argument, 0, 's'},
    {"data", required_argument, 0, 'd'},       {"help", no_argument, 0, 'h'},
    {"wrhex", no_argument, 0, DO_WRITE},       {"rdhex", no_argument, 0, DO_READ},
...
int main(int argc, char *argv[])
{
    ...
    switch (action) {
        case DO_WRITE:
            if (infile == NULL) die("Invalid options for action --wrhex");
            load_memory(infile);
            break;
        ...
        case DO_WRITE_BIN:
            if ((address == INT_MAX) || (infile == NULL)) die("Invalid options for action --load");
        ...
        case DO_LOAD_BOOTROM:
            if (infile == NULL) die("Invalid options for action --brom");
        ...
        case DO_SET_WORD:
            if ((address == INT_MAX)) die("Invalid options for action --regw");
            set_word(address, data);
            break;

        case DO_GET_WORD:
            if ((address == INT_MAX)) die("Invalid options for action --regr");
            get_word(address);
            break;
