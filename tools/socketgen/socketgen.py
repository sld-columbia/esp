...
class Parameter():
    ...
    def __str__(self):
        return "            " + self.name + ", " + self.desc + ", " + \
            str(self.size) + "bits, register " + str(self.reg) + ", def. value " + str(self.value) + "\n"
...
class Accelerator():
    ...
    def __str__(self):
        ...
        cfgs = cfgs + "]"
        return "          " + self.name + "_" + cfgs + ": " + self.desc + \
            ", " + str(self.data) + "MB, ID " + self.device_id + str(params)
...
def write_axi_acc_interface(f, acc, noc_width):
    ...
    f.write(
        "  " +
        acc.axi_prefix +
        "awatop : out std_logic_vector(5 downto 0);\n")
    f.write(
        "  " +
        acc.axi_prefix +
...
def tie_unused_axi(f, acc, noc_width):
    f.write("  pad_id_gen : if XID_WIDTH > " +
            str(acc.id_width) + " generate\n")
    f.write("    mosi(0).aw.id(XID_WIDTH - 1 downto " +
            str(acc.id_width) + ") <= (others => '0');\n")
    f.write("    mosi(0).ar.id(XID_WIDTH - 1 downto " +
            str(acc.id_width) + ") <= (others => '0');\n")
    f.write("  end generate;\n")
    f.write("  pad_paddr_gen : if GLOB_PHYS_ADDR_BITS > " +
            str(acc.addr_width) + " generate\n")
    f.write("    mosi(0).aw.addr(GLOB_PHYS_ADDR_BITS - 1 downto " +
            str(acc.addr_width) + ") <= (others => '0');\n")
    f.write("    mosi(0).ar.addr(GLOB_PHYS_ADDR_BITS - 1 downto " +
            str(acc.addr_width) + ") <= (others => '0');\n")
    f.write("  end generate;\n")
    f.write("  pad_user_gen : if XUSER_WIDTH > " +
            str(acc.user_width) + " generate\n")
    f.write("    mosi(0).aw.user(XUSER_WIDTH - 1 downto " +
            str(acc.user_width) + ") <= (others => '0');\n")
    f.write("    mosi(0).w.user(XUSER_WIDTH - 1 downto " +
            str(acc.user_width) + ") <= (others => '0');\n")
    f.write("    mosi(0).ar.user(XUSER_WIDTH - 1 downto " +
            str(acc.user_width) + ") <= (others => '0');\n")
    f.write("  end generate;\n")
...
def write_acc_interface(
        ...
        rst,
        is_vivadohls_if,
        is_catapulthls_cxx_if,
        is_catapulthls_sysc_if):
    ...
    if is_catapulthls_cxx_if:
        ...
        f.write("\n")
        f.write("      conf_info_rsc_dat          : in  std_logic_vector(" +
                str(conf_info_size - 1) + " downto 0);\n")
        ...
        f.write("      " + rst + spacing + ": in  std_ulogic;\n")
        f.write("\n")
        f.write("      conf_info_msg          : in std_logic_vector(" +
                str(conf_info_size - 1) + " downto 0);\n")
        ...
        f.write("\n")
        f.write("      dma_read_chnl_msg          : in  std_logic_vector(" +
                str(noc_width - 1) + " downto 0);\n")
        f.write("      dma_read_chnl_val          : in  std_ulogic;\n")
        f.write("      dma_read_chnl_rdy          : out std_ulogic;\n")
        f.write("\n")
        f.write("      dma_write_chnl_msg         : out std_logic_vector(" +
                str(noc_width - 1) + " downto 0);\n")
        ...
        for param in acc.param:
            if not param.readonly:
                ...
                f.write("      conf_info_" + param.name + spacing +
                        ": in  std_logic_vector(" + str(param.size - 1) + " downto 0);\n")
        ...
        f.write(
            "      dma_read_ctrl_data_index   : out std_logic_vector(" +
            str(31) +
            " downto 0);\n")
        f.write(
            "      dma_read_ctrl_data_length  : out std_logic_vector(" +
            str(31) +
            " downto 0);\n")
        f.write(
            "      dma_read_ctrl_data_size    : out std_logic_vector(" +
            str(2) +
            " downto 0);\n")
        f.write(
        ...
        f.write(
            "      dma_write_ctrl_data_index  : out std_logic_vector(" +
            str(31) +
            " downto 0);\n")
        f.write(
            "      dma_write_ctrl_data_length : out std_logic_vector(" +
            str(31) +
            " downto 0);\n")
        f.write(
            "      dma_write_ctrl_data_size   : out std_logic_vector(" +
            str(2) +
            " downto 0);\n")
        f.write(
...
def write_acc_port_map(
    ...
    if is_vivadohls_if:
        ...
        f.write("\n")
        f.write(
            "  ap_acc_fsm: process (ap_state, conf_done, ap_idle, ap_ready, ap_done) is\n")
        ...
        f.write("  ---- CTRL FSM\n")
        f.write("\n")
        f.write("  dma_read_ctrl_data_user    <= dma_read_ctrl_data(101 downto 96);\n")
        ...
        for param in acc.param:
            if not param.readonly:
                ...
                f.write("      conf_info_rsc_dat(" +
                        str(conf_info_rsc_data_size -
                            1) +
                        " downto " +
                        str(conf_info_rsc_data_size -
                            param.size) +
        ...
        f.write("\n")
        f.write(
            "      dma_read_ctrl_rsc_dat(72 downto 67) => dma_read_ctrl_data_user,\n")
        f.write(
...
def write_cache_interface(f, cac, is_llc):
    if (is_llc and 'spandex' not in cac.name):
        ...
        f.write("      llc_req_in_data_addr         : in std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_req_in_data_word_offset  : in std_logic_vector(" +
                str(word_offset_bits - 1) + " downto 0);\n")
        f.write("      llc_req_in_data_valid_words  : in std_logic_vector(" +
                str(word_offset_bits - 1) + " downto 0);\n")
        f.write("      llc_req_in_data_line         : in std_logic_vector(" +
                str(bits_per_line - 1) + "  downto 0);\n")
        f.write("      llc_req_in_data_req_id       : in std_logic_vector(" +
                str(max_n_l2_bits - 1) + " downto 0);\n")
        ...
        f.write("      llc_dma_req_in_data_addr         : in std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_dma_req_in_data_word_offset  : in std_logic_vector(" +
                str(dma_word_offset_bits - 1) + " downto 0);\n")
        f.write("      llc_dma_req_in_data_valid_words  : in std_logic_vector(" +
                str(dma_word_offset_bits - 1) + " downto 0);\n")
        f.write("      llc_dma_req_in_data_line         : in std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      llc_dma_req_in_data_req_id       : in std_logic_vector(" +
                str(max_n_llc_bits - 1) + " downto 0);\n")
        f.write("      llc_rsp_in_valid             : in std_ulogic;\n")
        f.write(
            "      llc_rsp_in_data_coh_msg      : in std_logic_vector(1 downto 0);\n")
        f.write("      llc_rsp_in_data_addr         : in std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_rsp_in_data_line         : in std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      llc_rsp_in_data_req_id       : in std_logic_vector(" +
                str(max_n_l2_bits - 1) + " downto 0);\n")
        f.write("      llc_mem_rsp_valid            : in std_ulogic;\n")
        f.write("      llc_mem_rsp_data_line        : in std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        ...
        f.write("      llc_rsp_out_data_addr        : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_rsp_out_data_line        : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      llc_rsp_out_data_invack_cnt  : out std_logic_vector(" +
                str(invack_cnt_bits - 1) + "  downto 0);\n")
        f.write("      llc_rsp_out_data_req_id      : out std_logic_vector(" +
                str(max_n_l2_bits - 1) + " downto 0);\n")
        f.write(
            "      llc_rsp_out_data_dest_id     : out std_logic_vector(3 downto 0);\n")
        f.write("      llc_rsp_out_data_word_offset : out std_logic_vector(" +
                str(word_offset_bits - 1) + " downto 0);\n")
        f.write("      llc_dma_rsp_out_valid            : out std_ulogic;\n")
        f.write(
            "      llc_dma_rsp_out_data_coh_msg     : out std_logic_vector(1 downto 0);\n")
        f.write("      llc_dma_rsp_out_data_addr        : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_dma_rsp_out_data_line        : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      llc_dma_rsp_out_data_invack_cnt  : out std_logic_vector(" +
                str(invack_cnt_bits - 1) + " downto 0);\n")
        f.write("      llc_dma_rsp_out_data_req_id      : out std_logic_vector(" +
                str(max_n_llc_bits - 1) + " downto 0);\n")
        f.write(
            "      llc_dma_rsp_out_data_dest_id     : out std_logic_vector(3 downto 0);\n")
        f.write("      llc_dma_rsp_out_data_word_offset : out std_logic_vector(" +
                str(dma_word_offset_bits - 1) + " downto 0);\n")
        f.write("      llc_fwd_out_valid            : out std_ulogic;\n")
        f.write(
            "      llc_fwd_out_data_coh_msg     : out std_logic_vector(2 downto 0);\n")
        f.write("      llc_fwd_out_data_addr        : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_fwd_out_data_req_id      : out std_logic_vector(" +
                str(max_n_l2_bits - 1) + " downto 0);\n")
        f.write(
        ...
        f.write("      llc_mem_req_data_addr        : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_mem_req_data_line        : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        ...
        f.write("      llc_req_in_valid             : in std_ulogic;\n")
        f.write(
            "      llc_req_in_data_coh_msg      : in std_logic_vector(4 downto 0);\n")
        f.write(
            "      llc_req_in_data_hprot        : in std_logic_vector(1 downto 0);\n")
        f.write("      llc_req_in_data_addr         : in std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        ...
        f.write("      llc_req_in_data_line         : in std_logic_vector(" +
                str(bits_per_line - 1) + "  downto 0);\n")
        f.write("      llc_req_in_data_word_mask    : in std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        f.write(
            "      llc_req_in_data_req_id       : in std_logic_vector(3 downto 0);\n")
        f.write("      llc_dma_req_in_valid             : in std_ulogic;\n")
        f.write(
            "      llc_dma_req_in_data_coh_msg      : in std_logic_vector(4 downto 0);\n")
        f.write(
            "      llc_dma_req_in_data_hprot        : in std_logic_vector(1 downto 0);\n")
        f.write("      llc_dma_req_in_data_addr         : in std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        ...
        f.write("      llc_dma_req_in_data_word_mask    : in std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        f.write("      llc_rsp_in_valid             : in std_ulogic;\n")
        f.write(
            "      llc_rsp_in_data_coh_msg      : in std_logic_vector(3 downto 0);\n")
        f.write("      llc_rsp_in_data_addr         : in std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_rsp_in_data_line         : in std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      llc_rsp_in_data_word_mask    : in std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        f.write(
        ...
        f.write("      llc_rsp_out_valid            : out std_ulogic;\n")
        f.write(
            "      llc_rsp_out_data_coh_msg     : out std_logic_vector(3 downto 0);\n")
        f.write("      llc_rsp_out_data_addr        : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_rsp_out_data_line        : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      llc_rsp_out_data_word_mask   : out std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        f.write("      llc_rsp_out_data_invack_cnt  : out std_logic_vector(" +
                str(invack_cnt_bits - 1) + " downto 0);\n")
        f.write(
        ...
        f.write("      llc_dma_rsp_out_valid            : out std_ulogic;\n")
        f.write(
            "      llc_dma_rsp_out_data_coh_msg     : out std_logic_vector(3 downto 0);\n")
        f.write("      llc_dma_rsp_out_data_addr        : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      llc_dma_rsp_out_data_line        : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        ...
        f.write("      llc_dma_rsp_out_data_word_offset : out std_logic_vector(" +
                str(dma_word_offset_bits - 1) + " downto 0);\n")
        f.write("      llc_dma_rsp_out_data_word_mask   : out std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        f.write("      llc_fwd_out_valid            : out std_ulogic;\n")
        f.write(
            "      llc_fwd_out_data_coh_msg     : out std_logic_vector(4 downto 0);\n")
        f.write("      llc_fwd_out_data_addr        : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write(
        ...
        f.write(
            "      llc_fwd_out_data_dest_id     : out std_logic_vector(3 downto 0);\n")
        f.write("      llc_fwd_out_data_word_mask   : out std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        f.write("      llc_fwd_out_data_line        : out std_logic_vector(" +
                str(bits_per_line - 1) + "  downto 0);\n")
        f.write("      llc_mem_req_valid            : out std_ulogic;\n")
        f.write("      llc_mem_req_data_hwrite      : out std_ulogic;\n")
        f.write(
        ...
        f.write("      l2_cpu_req_data_addr      : in  std_logic_vector(" +
                str(phys_addr_bits - 1) + " downto 0);\n")
        f.write("      l2_cpu_req_data_word      : in  std_logic_vector(" +
                str(arch_bits - 1) + " downto 0);\n")
        ...
        f.write("      l2_fwd_in_data_addr       : in  std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      l2_fwd_in_data_req_id     : in  std_logic_vector(3 downto 0);\n")
        f.write("      l2_fwd_in_data_word_mask  : in std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        f.write("      l2_fwd_in_data_line       : in std_logic_vector(" +
                str(bits_per_line - 1) + "  downto 0);\n")
        f.write("      l2_rsp_in_valid           : in  std_ulogic;\n")
        f.write("      l2_rsp_in_data_coh_msg    : in  std_logic_vector(3 downto 0);\n")
        f.write("      l2_rsp_in_data_addr       : in  std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      l2_rsp_in_data_line       : in  std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      l2_rsp_in_data_word_mask  : in std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        f.write("      l2_rsp_in_data_invack_cnt : in  std_logic_vector(" +
                str(invack_cnt_bits - 1) + " downto 0);\n")
        ...
        f.write("      l2_rd_rsp_data_line       : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      l2_inval_valid            : out std_ulogic;\n")
        f.write("      l2_inval_data_addr        : out std_logic_vector(" +
                str(phys_addr_bits - 1) + " downto 0);\n")
        ...
        f.write("      l2_req_out_data_addr      : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      l2_req_out_data_line      : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      l2_req_out_data_word_mask : out std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        ...
        f.write("      l2_rsp_out_data_addr      : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      l2_rsp_out_data_line      : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      l2_rsp_out_data_word_mask : out std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        ...
        f.write("      l2_fwd_out_data_addr      : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      l2_fwd_out_data_line      : out std_logic_vector(" +
                str(bits_per_line - 1) + " downto 0);\n")
        f.write("      l2_fwd_out_data_word_mask : out std_logic_vector(" +
                str(words_per_line - 1) + "  downto 0);\n")
        ...
        f.write("      l2_fwd_in_data_addr       : in  std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
        f.write("      l2_fwd_in_data_req_id     : in  std_logic_vector(" +
                str(max_n_l2_bits - 1) + " downto 0);\n")
        f.write("      l2_rsp_in_valid           : in  std_ulogic;\n")
        ...
        f.write("      l2_rsp_out_data_req_id    : out std_logic_vector(" +
                str(max_n_l2_bits - 1) + " downto 0);\n")
        f.write("      l2_rsp_out_data_to_req    : out std_logic_vector(1 downto 0);\n")
        f.write("      l2_rsp_out_data_addr      : out std_logic_vector(" +
                str(phys_addr_bits - offset_bits - 1) + " downto 0);\n")
            ...
            for acc in accelerator_list:
                for impl in acc.hlscfg:
                    ...
                    if acc.hls_tool == 'stratus_hls' or acc.hls_tool == 'rtl':
                        ...
                        f.write("    port (\n")
                        write_acc_interface(
                            f, acc, noc_width, impl.datatype, "rst", False, False, False)
                        ...
                        f.write("    port (\n")
                        write_acc_interface(
                            f, acc, noc_width, impl.datatype, "rst", False, True, False)
                        ...
                        f.write("    port (\n")
                        write_acc_interface(
                            f, acc, noc_width, impl.datatype, "rst", False, False, True)
                    else:
                        f.write(
                            "  component " +
                            acc.name +
                        ...
                        f.write("    port (\n")
                        write_acc_interface(
                            f, acc, noc_width, impl.datatype, "ap_rst", True, False, False)
                    f.write("    );\n")
    ...
    with open(template_dir + '/allcaches.vhd', 'r') as ftemplate:
        for tline in ftemplate:
            if tline.find("-- <<invack_cnt_width>>") >= 0:
                f.write(
                    "  constant INVACK_CNT_WIDTH      : integer := " +
                    str(invack_cnt_bits) +
                    ";\n")
    ...
    with open(template_dir + '/genacc.vhd', 'r') as ftemplate:
        for tline in ftemplate:
            ...
            for acc in accelerator_list:
                ...
                f.write("    port (\n")
                write_acc_interface(
                    f, acc, noc_width, "", "acc_rst", False, False, False)
                f.write("    );\n")
                f.write("  end component;\n\n")
                f.write("\n")
...
# Mapping from generic components to technology and implementation
# dependent ones
def gen_tech_indep_impl(
        accelerator_list,
        cache_list,
        noc_width,
        template_dir,
                    ...
                    for impl in acc.hlscfg:
                        ...
                        if acc.hls_tool == 'stratus_hls':
                            ...
                            f.write(
                                ...
                                "_top\n")
                            write_acc_port_map(
                                f, acc, noc_width, impl.datatype, "rst", False, True, False, False)
                        f.write(
                            "  end generate impl_" +
                            impl.name +
                            "_gen;\n\n")
                ...
                for impl in cac.hlscfg:
                    ...
                    for item in info:
                        if re.match(r'[0-9]+sets', item, re.M | re.I):
                            ...
                            this_offset_bits = int(
                                int(math.log2((int(item.replace("line", ""))) / 8) + word_offset_bits))
                    ...
                    if sets * ways == 0:
                        print(
                            "    ERROR: hls config must report number of sets and ways, both different from zero")
                        sys.exit(1)
                    if this_word_offset_bits != word_offset_bits or this_offset_bits != offset_bits or this_addr_bits != phys_addr_bits or this_endian != little_endian:
                        print(
                            "    INFO: skipping cache implementation " +
                            impl +
                            " incompatible with SoC architecture")
                        continue
                    f.write("\n")
                    if is_llc:
                        f.write(
    ...
    with open(template_file, 'r') as ftemplate:
        for tline in ftemplate:
            if is_axi and tline.find("library ieee;") >= 0:
                ...
                f.write(
                    "  constant devid         : devid_t                := SLD_" +
                    acc.name.upper() +
                    ";\n")
            elif tline.find("-- <<tlb_entries>>") >= 0:
                f.write(
                    "  constant tlb_entries   : integer                := " + str(acc.data) + ";\n")
            elif tline.find("-- <<user_registers>>") >= 0:
                for param in acc.param:
                    f.write("  -- bank(" + str(param.reg) + "): " +
                            param.desc + "\n")
                    f.write("  constant " +
                            acc.name.upper() +
                            "_" +
                            param.name.upper() +
                            "_REG : integer range 0 to MAXREGNUM - 1 := " +
                            str(param.reg) +
                            ";\n\n")
                ...
                if is_axi:
                    ...
                    f.write("    )\n")
                    write_acc_port_map(
                        f, acc, noc_width, "", "acc_rst", True, False, False, False)
            else:
                f.write(tline)
...
if "ESP_THIRDPARTY_ACCELERATORS" in os.environ:
    enabled_axi_accelerators = os.environ[
        "ESP_THIRDPARTY_ACCELERATORS"].split()
    axi_accelerators = [
        acc for acc in axi_accelerators if acc in enabled_axi_accelerators]
accelerators.sort()
...
for acc in axi_accelerators:
    ...
    if not str(noc_width) in buf:
        print("    INFO: System DMA_WIDTH is " +
              str(noc_width) + "; skipping " + acc)
        continue

    accd = AxiAccelerator()
    accd.name = acc

    elem = xml.etree.ElementTree.parse(
        axi_acc_dir + "/" + acc + "/" + acc + ".xml")
    e = elem.getroot()
...
for acc in accelerators:
    ...
    for xmlacc in e.findall('accelerator'):
        ...
        if accd.data == 0:
            print(
                "    WARNING: memory footprint (MB) for " +
                acc +
                " is 0; defaulting to 4")
            accd.data = 4
        if "device_id" in xmlacc.attrib:
            accd.device_id = xmlacc.get('device_id')
        else:
        ...
        if "hls_tool" in xmlacc.attrib:
            ...
            # Default to stratus_hls for Chisel, because the interface matches
            # the Stratus HLS flow
