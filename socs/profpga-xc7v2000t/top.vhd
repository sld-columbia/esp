...
architecture rtl of top is
  ...
  -- Switches
  signal sel0 : std_ulogic;
  signal sel1 : std_ulogic;
  signal sel2 : std_ulogic;
  signal sel3 : std_ulogic;
  signal sel4 : std_ulogic;
...
begin
  ...
  c0_diagnostic : process (clkm, clkm_sync_rst) is
  ...
  c0_led_diag_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x15v)
    port map (c0_diagnostic_led, c0_diagnostic_toggle);

  c1_diagnostic : process (clkm_2, clkm_2_sync_rst) is
  ...
  c1_led_diag_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x15v)
    port map (c1_diagnostic_led, c1_diagnostic_toggle);
  ...
  lock_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x18v)
    port map (led_green, lock);

  -- From CPU 0 (on chip)
  cpuerr_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x18v)
    port map (led_red, cpuerr);

  -- pragma translate_off
  process (clkm, rstn) is
  begin  -- process
    if (rstn = '1') then
      assert cpuerr = '0'
  ...
  calib0_complete_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x15v)
    port map (c0_calib_complete, c0_calib_done);

  calib1_complete_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x15v)
    port map (c1_calib_complete, c1_calib_done);
  ...
  uart_rxd_pad : inpad
    generic map (
      level => cmos, voltage => x18v, tech => CFG_FABTECH)
    port map (uart_rxd, uart_rxd_int);

  uart_txd_pad : outpad
    generic map (
      level => cmos, voltage => x18v, tech => CFG_FABTECH)
    port map (uart_txd, uart_txd_int);

  uart_ctsn_pad : inpad
    generic map (
      level => cmos, voltage => x18v, tech => CFG_FABTECH)
    port map (uart_ctsn, uart_ctsn_int);

  uart_rtsn_pad : outpad
    generic map (
      level => cmos, voltage => x18v, tech => CFG_FABTECH)
    port map (uart_rtsn, uart_rtsn_int);
  ...
  clkgenmigref0 : clkgen
    generic map (
      CFG_FABTECH, 16, 32, 0, 0, 0, 0, 0, 100000)
    port map (clkm, clkm, chip_refclk, open, open, open, open, cgi, cgo, open, open, open);

  gen_mig : if (SIMULATION /= true) generate

    ddr_inst : axi2mig_7series_profpga
      generic map (
        axidw        => MEM_AXIDW,
        num_mem_tile => CFG_NMEM_TILE
      )
      port map (
  ...
  eth0_inpads : if (CFG_GRETH = 1) generate
    ...
    etxc_pad : clkpad
      generic map (
        tech => CFG_FABTECH, level => cmos, voltage => x18v, arch => 2)
    ...
    erxd_pad : inpadv
      generic map (
        tech => CFG_FABTECH, level => cmos, voltage => x18v, width => 4
      )
  ...
  tft_data_pad : outpadv
    generic map (
      width => 24, tech => CFG_FABTECH, level => cmos, voltage => x18v)
    port map (tft_data, dvi_data);

  tft_hsync_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x18v
    )
    port map (tft_hsync, dvi_hsync);

  tft_vsync_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x18v
    )
  ...
  tft_ctl1_a1_dk1_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x18v
    )
    port map (tft_ctl1_a1_dk1, dvi_ctl1_a1_dk1);

  tft_ctl2_a2_dk2_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x18v)
    port map (tft_ctl2_a2_dk2, dvi_ctl2_a2_dk2);

  tft_a3_dk3_pad : outpad
    generic map (
      tech => CFG_FABTECH, level => cmos, voltage => x18v
    )
  ...
  profpga_mmi64_gen : if CFG_MON_DDR_EN + CFG_MON_NOC_INJECT_EN + CFG_MON_NOC_QUEUES_EN + CFG_MON_ACC_EN + CFG_MON_DVFS_EN /= 0 generate
    ...
    detect_ddr_access : process (ddr_axi_si) is
    begin  -- process detect_mem_access

      mon_ddr(0).word_transfer <= '0';

      if (((ddr_axi_si(0).aw.addr(31 downto 20) xor conv_std_logic_vector(ddr_haddr(0), 12))
          and conv_std_logic_vector(ddr_hmask(0), 12)) = zero32(31 downto 20)) then
        if (ddr_axi_si(0).w.valid = '1' and ddr_axi_so(0).w.ready = '1') then
    ...
    dual_mig_monitor : if CFG_NMEM_TILE = 2 generate
      ...
      detect_ddr1_access : process (ddr_axi_si) is
      begin  -- process detect_mem_access

        mon_ddr(1).word_transfer <= '0';

        if (((ddr_axi_si(1).aw.addr(31 downto 20) xor conv_std_logic_vector(ddr_haddr(1), 12))
            and conv_std_logic_vector(ddr_hmask(1), 12)) = zero32(31 downto 20)) then
          if (ddr_axi_si(1).w.valid = '1' and ddr_axi_so(1).w.ready = '1') then
