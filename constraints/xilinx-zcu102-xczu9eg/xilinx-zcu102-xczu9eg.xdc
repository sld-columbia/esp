# --- Input clock and reset are generated by the ZYNQ MP SoC processing system (no constraints here)


# Recover ESP main clock name

# --- False paths
set_false_path -to [get_ports {led[*]}]

# --- UART
set_property PACKAGE_PIN F13 [get_ports uart_txd]
set_property PACKAGE_PIN E13 [get_ports uart_rxd]
set_property PACKAGE_PIN E12 [get_ports uart_rtsn]
set_property PACKAGE_PIN D12 [get_ports uart_ctsn]

set_property IOSTANDARD LVCMOS33 [get_ports uart_*]

# Inputs
set_input_delay -clock [get_clocks [get_clocks -of_objects [get_nets -hierarchical chip_refclk]]] -max 1.500 [get_ports uart_rxd]
set_input_delay -clock [get_clocks [get_clocks -of_objects [get_nets -hierarchical chip_refclk]]] -min -add_delay 0.500 [get_ports uart_rxd]
set_input_delay -clock [get_clocks [get_clocks -of_objects [get_nets -hierarchical chip_refclk]]] -max 1.500 [get_ports uart_ctsn]
set_input_delay -clock [get_clocks [get_clocks -of_objects [get_nets -hierarchical chip_refclk]]] -min -add_delay 0.500 [get_ports uart_ctsn]

# Outputs
set_output_delay -clock [get_clocks [get_clocks -of_objects [get_nets -hierarchical chip_refclk]]] -max 0.500 [get_ports uart_txd]
set_output_delay -clock [get_clocks [get_clocks -of_objects [get_nets -hierarchical chip_refclk]]] -min -add_delay -0.500 [get_ports uart_txd]
set_output_delay -clock [get_clocks [get_clocks -of_objects [get_nets -hierarchical chip_refclk]]] -max 0.500 [get_ports uart_rtsn]
set_output_delay -clock [get_clocks [get_clocks -of_objects [get_nets -hierarchical chip_refclk]]] -min -add_delay -0.500 [get_ports uart_rtsn]


# --- LEDs
set_property PACKAGE_PIN AG14 [get_ports {led[0]}]
set_property PACKAGE_PIN AF13 [get_ports {led[1]}]
set_property PACKAGE_PIN AE13 [get_ports {led[2]}]
set_property PACKAGE_PIN AJ14 [get_ports {led[3]}]
set_property PACKAGE_PIN AJ15 [get_ports {led[4]}]
set_property PACKAGE_PIN AH13 [get_ports {led[5]}]
set_property PACKAGE_PIN AH14 [get_ports {led[6]}]
#set_property PACKAGE_PIN AL12     [get_ports "led[7]"]

set_property IOSTANDARD LVCMOS33 [get_ports led*]


# --- Switches
set_property PACKAGE_PIN AN14 [get_ports {switch[0]}]
set_property PACKAGE_PIN AP14 [get_ports {switch[1]}]
set_property PACKAGE_PIN AM14 [get_ports {switch[2]}]
set_property PACKAGE_PIN AN13 [get_ports {switch[3]}]
set_property PACKAGE_PIN AN12 [get_ports {switch[4]}]
set_property PACKAGE_PIN AP12 [get_ports {switch[5]}]
set_property PACKAGE_PIN AL13 [get_ports {switch[6]}]
set_property PACKAGE_PIN AK13 [get_ports {switch[7]}]

set_property IOSTANDARD LVCMOS33 [get_ports switch*]

